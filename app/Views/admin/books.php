...
<body>
  ...
  // layout.php reads $pendingBorrowings for the Library nav badge
  $pendingBorrowings = $pendingBorrows;

  /* ─── Render sidebar, mobile nav, dark-mode JS, and link admin_app.css ───── */
  ...
  <div id="pdfModal" class="modal-back" onclick="if(event.target===this)closeModal('pdfModal')">
    <div class="modal-card wide">
      ...
      <div id="aPdfStepError" style="display:none;text-align:center;padding:24px 0;">
        ...
        <div id="adminDebugPanel" style="display:none;margin-top:10px;padding:10px 14px;background:#fef2f2;border:1px solid #fca5a5;border-radius:var(--r-sm);font-size:.72rem;font-family:var(--mono);color:#991b1b;word-break:break-all;white-space:pre-wrap;max-height:120px;overflow-y:auto;"></div>
  ...
  <main class="main-area">
    ...
    <div class="topbar fade-up">
      <div>
        ...
        <div class="page-title">Library <span style="color:var(--indigo);">Management</span></div>
  ...
  <script>
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape')
        ['addModal', 'editModal', 'pdfModal', 'deleteModal', 'ragModal'].forEach(closeModal);
    });
    ...
    function initPagination(f) {
      ...
      document.getElementById('pageInfo').textContent =
        `Showing ${start}–${Math.min(_currentPage * PAGE_SIZE, total)} of ${total}`;
    }
    ...
    function applyBorrowFilter() {
      ...
      let visible = 0;
      const matches = el =>
        (_borrowStatus === 'all' || el.dataset.status === _borrowStatus) &&
    ...
    function aProcessPdfFile(file) {
      ...
      reader.onload = async ev => {
        try {
          ...
          const ok = _aPdfText && _aPdfText.trim().length >= 20;
          document.getElementById('aExtractBtn').disabled = !ok;
          if (!ok) {
    ...
    function aSetStep(n) {
      [1, 2, 3].forEach(i => {
        document.getElementById('aStepDot' + i).className =
          'step-dot ' + (i < n ? 'done' : i === n ? 'active' : 'pending');
      });
      [1, 2].forEach(i => {
        document.getElementById('aStepLine' + i).className =
          'step-line ' + (i < n ? 'done' : 'pending');
      });
