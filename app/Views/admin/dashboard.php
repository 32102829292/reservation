...
<body>
        ...
        <div class="topbar fade-up">
            <div>
                <div class="greeting-eyebrow"><?php $hh = (int)date('H');
                                                echo $hh < 12 ? 'Good morning' : ($hh < 17 ? 'Good afternoon' : 'Good evening'); ?>, <?= htmlspecialchars($admin_name ?? 'Administrator') ?></div>
            ...
            <div class="stat-card">
                ...
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;text-align:center;">
                    <div>
                        ...
                        <div style="font-size:.6rem;color:#94a3b8;font-weight:700;">Pending</div>
                    </div>
                    <div>
                        <div style="font-size:1.3rem;font-weight:800;color:#16a34a;font-family:var(--mono);"><?= $todayApproved ?? 0 ?></div>
                        <div style="font-size:.6rem;color:#94a3b8;font-weight:700;">Approved</div>
                    </div>
                    <div>
                        <div style="font-size:1.3rem;font-weight:800;color:#7c3aed;font-family:var(--mono);"><?= $todayClaimed ?? 0 ?></div>
                        <div style="font-size:.6rem;color:#94a3b8;font-weight:700;">Claimed</div>
                    </div>
                </div>
            </div>
            <div class="stat-card">
    ...
    <script>
        ...
        function isClaimed(r) {
            return [true, 1, 't', 'true', '1'].includes(r.claimed);
        }
        ...
        function loadNotifications() {
            allRes.filter(r => r.status === 'pending' && !readIds.includes(String(r.id))).slice(0, 10).forEach(r => notifs.push({
                id: r.id,
        ...
        function renderNotifs() {
            ...
            if (!notifs.length) {
                l.innerHTML = `<div style="text-align:center;padding:24px;"><i class="fa-regular fa-bell-slash" style="font-size:1.5rem;color:#e2e8f0;display:block;margin-bottom:8px;"></i><p style="font-size:.78rem;color:#94a3b8;">No notifications</p></div>`;
                return;
            }
        ...
        function tlIsLogged(id) {
            return tlGetLogged().includes(id);
        }
        ...
        function tlRender() {
            ...
            sessions.forEach(r => {
                ...
                const sf = r.start_time ? r.start_time.substring(0, 5) : '–',
                    ef = r.end_time ? r.end_time.substring(0, 5) : '–',
            ...
            (function() {
                ...
                if (hm) {
                    ...
                    for (let h = 0; h < 24; h++) {
                        ...
                        const alpha = 0.06 + (pct(hourArr[h], maxH) / 100) * 0.9;
                ...
                if (rk) {
                    ...
                    rk.innerHTML = !entries.length ? '<p style="font-size:.75rem;color:#94a3b8;text-align:center;padding:16px;">No data yet</p>' : entries.slice(0, 7).map(([name, cnt], i) => {
                        const w = pct(cnt, topMax),
                            c = colors[i] || '#94a3b8',
                            share = totalCount > 0 ? Math.round(cnt / totalCount * 100) : 0;
                        return `<div><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;gap:8px;"><div style="display:flex;align-items:center;gap:8px;min-width:0;"><span style="width:20px;height:20px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:800;color:white;background:${c};flex-shrink:0;">${i+1}</span><span style="font-size:.78rem;font-weight:600;color:#475569;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</span></div><div style="display:flex;align-items:center;gap:8px;flex-shrink:0;"><span style="font-size:.65rem;color:#94a3b8;">${share}%</span><span style="font-size:.78rem;font-weight:800;color:#0f172a;">${cnt}</span></div></div><div class="prog-bar"><div class="prog-fill" style="width:${w}%;background:${c};"></div></div></div>`;
                    }).join('');
                }
            })();
        });
