...
<head>
    ...
    <!-- ★ Shared stylesheet — replaces all inline styles -->
    ...
    <style>
        ...
        .bk-date { width: 38px; height: 38px; background: var(--input-bg); border-radius: 10px; display: flex; flex-direction: column; align-items: center; justify-content: center; flex-shrink: 0; border: 1px solid rgba(99, 102, 241, .09); }
        ...
        body.dark .insight-mini { background: var(--card); border-color: rgba(99,102,241,.1); }
        ...
        .print-modal-card { background: var(--card); border-radius: var(--r-xl); width: 100%; max-width: 400px; padding: 24px; margin: auto; animation: slideUp .2s ease; box-shadow: var(--shadow-lg); }
        body.dark .print-modal-card { background: var(--card); }

        .tl-print-toggle { display: flex; border-radius: var(--r-sm); overflow: hidden; border: 1px solid rgba(99,102,241,.15); background: var(--input-bg); }
        .tl-print-toggle button { flex: 1; padding: .75rem .5rem; font-size: .78rem; font-weight: 700; border: none; background: transparent; cursor: pointer; transition: all .15s; color: var(--text-sub); font-family: var(--font); }
        ...
        .tl-page-counter { display: flex; align-items: center; gap: 12px; background: var(--input-bg); border-radius: var(--r-sm); padding: .75rem 1rem; border: 1px solid rgba(99,102,241,.12); }
        .tl-page-counter button { width: 38px; height: 38px; border-radius: 9px; border: 1px solid rgba(99,102,241,.15); background: var(--card); font-weight: 800; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--indigo); transition: all .15s; flex-shrink: 0; }
        .tl-page-counter button:hover { background: var(--indigo-light); border-color: var(--indigo-border); }
        .tl-page-num { flex: 1; text-align: center; font-size: 1.5rem; font-weight: 800; color: var(--text); font-family: var(--mono); }
        body.dark .tl-page-counter { background: #101e35; border-color: rgba(99,102,241,.12); }
        body.dark .tl-page-counter button { background: var(--card); color: #818cf8; border-color: rgba(99,102,241,.18); }
        ...
        .tl-skip-btn { width: 100%; padding: .65rem; border-radius: var(--r-sm); border: none; background: transparent; color: var(--text-sub); font-size: .8rem; font-weight: 700; cursor: pointer; transition: color .15s; margin-top: .3rem; font-family: var(--font); }
        .tl-skip-btn:hover { color: var(--text-muted); }

        /* ══ Notification dropdown ══ */
        .notif-dd { position: fixed; top: 80px; right: 20px; width: 320px; background: var(--card); border-radius: var(--r-xl); box-shadow: var(--shadow-lg), 0 0 0 1px rgba(99,102,241,.09); z-index: 200; display: none; overflow: hidden; animation: dropIn .15s ease; }
        ...
        body.dark .notif-dd { background: var(--card); border-color: rgba(99,102,241,.15); box-shadow: 0 20px 48px -8px rgba(0,0,0,.5); }
        ...
        /* ══ Topbar extras ══ */
        .greeting-eyebrow { font-size: .7rem; font-weight: 700; letter-spacing: .2em; text-transform: uppercase; color: var(--text-sub); margin-bottom: 4px; }
        .greeting-name    { font-size: 1.75rem; font-weight: 800; color: var(--text); letter-spacing: -.04em; line-height: 1.1; }
        .greeting-date    { font-size: .78rem; color: var(--text-sub); margin-top: 4px; font-weight: 500; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
...
<body>
    ...
    <main class="main-area">
        ...
        <div class="topbar fade-up">
            <div>
                <div class="greeting-eyebrow"><?php $hh = (int)date('H'); echo $hh < 12 ? 'Good morning' : ($hh < 17 ? 'Good afternoon' : 'Good evening'); ?>, <?= htmlspecialchars($admin_name) ?></div>
        ...
        <div class="stats-grid fade-up-2">
            ...
            <div class="stat-card" style="border-left-color:#d97706;">
                ...
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;text-align:center;">
                    <div><div style="font-size:1.3rem;font-weight:800;color:#d97706;font-family:var(--mono);"><?= $todayPending ?? 0 ?></div><div style="font-size:.6rem;color:var(--text-sub);font-weight:700;">Pending</div></div>
                    <div><div style="font-size:1.3rem;font-weight:800;color:#16a34a;font-family:var(--mono);"><?= $todayApproved ?? 0 ?></div><div style="font-size:.6rem;color:var(--text-sub);font-weight:700;">Approved</div></div>
                    <div><div style="font-size:1.3rem;font-weight:800;color:#7c3aed;font-family:var(--mono);"><?= $todayClaimed ?? 0 ?></div><div style="font-size:.6rem;color:var(--text-sub);font-weight:700;">Claimed</div></div>
                </div>
            </div>
            ...
            <div class="card card-p">
                <div class="card-head">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div><div class="card-title">Reservations Trend</div><div class="card-sub">Last 7 days · All users</div></div>
                    </div>
            ...
            <div class="card card-p">
                <div class="card-head">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div><div class="card-title">Popular Resources</div><div class="card-sub">Most reserved · All users</div></div>
                    </div>
            ...
            <div class="card card-p-lg">
                <div class="card-head">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div><div class="card-title">Reservation Calendar</div><div class="card-sub">All users · Tap date to view</div></div>
                    </div>
                    ...
                    if (!empty($pl)): foreach (array_slice($pl, 0, 4) as $res): ?>
                        <a href="/admin/manage-reservations?id=<?= $res['id'] ?>" class="bk-row">
                            <?php if (!empty($res['reservation_date'])): $dt = new DateTime($res['reservation_date']); ?>
                                <div class="bk-date"><div class="bk-month"><?= $dt->format('M') ?></div><div class="bk-day"><?= $dt->format('j') ?></div></div>
        ...
        <div class="grid-lib fade-up-4">
            <div style="display:flex;flex-direction:column;gap:14px;">
                <div class="lib-banner">
                    <div style="position:relative;z-index:1;">
                        ...
                        <div style="display:flex;gap:8px;">
                            <div class="lib-stat-item"><div class="lib-stat-lbl">Borrow Reqs</div><div class="lib-stat-val"><?= $pendingBorrowings ?></div></div>
                            <?php $bpct = $bookTotalCount > 0 ? round($bookAvailCount / $bookTotalCount * 100) : 0; ?>
                            <div class="lib-stat-item"><div class="lib-stat-lbl">In Stock</div><div class="lib-stat-val"><?= $bpct ?>%</div></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-p" style="flex:1;">
                    <div class="card-head">
                        <div><div class="card-title">Borrow Requests</div><div class="card-sub">Pending approval</div></div>
            ...
            <div class="card card-p-lg">
                <div class="card-head">
                    <div><div class="card-title">Books Catalog</div><div class="card-sub">Availability at a glance</div></div>
        ...
        <div class="grid-three fade-up-4">
            <div class="card card-p">
                <div class="card-head">
                    <div><div class="card-title">Hourly Activity Heatmap</div><div class="card-sub">Booking density by hour</div></div>
        ...
        <div class="grid-two fade-up-4" style="margin-bottom:0;">
            <div class="card card-p">
                <div class="card-head">
                    <div><div class="card-title">Monthly Seasonality</div><div class="card-sub">Volume by calendar month</div></div>
            ...
            <div class="card card-p">
                <div class="card-head">
                    <div><div class="card-title">Resource Demand Ranking</div><div class="card-sub">All-time count per resource</div></div>
    ...
    <script>
        ...
        function renderNotifs() {
            ...
            if (!notifs.length) { l.innerHTML = `<div style="text-align:center;padding:24px;"><i class="fa-regular fa-bell-slash" style="font-size:1.5rem;color:#e2e8f0;display:block;margin-bottom:8px;"></i><p style="font-size:.78rem;color:var(--text-sub);">No notifications</p></div>`; return; }
        ...
        async function tlSavePrint() {
            ...
            btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin" style="margin-right:8px;"></i>Saving…';
            try {
                const r = await fetch(PRINT_EP, { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF_TOKEN}, body: JSON.stringify({ reservation_id: tlCurrentPrint.id, printed: tlPrinted, pages: tlPrinted ? tlPageCount : 0 }) });
                if (r.ok) tlMarkLogged(tlCurrentPrint.id);
            } catch(e) { console.error(e); }
            btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk" style="margin-right:8px;"></i>Save & Log';
        ...
        function tlRender() {
            ...
            sessions.forEach(r => {
                ...
                const sf = r.start_time?.substring(0,5) || '–', ef = r.end_time?.substring(0,5) || '–', usedMin = Math.max(0, Math.floor(elMs/60000)), logged = tlIsLogged(r.id);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            (function() {
                ...
                window.adminToggleDark = function() { origToggle(); updateChartsForTheme(document.body.classList.contains('dark')); };
                window.toggleDark = window.adminToggleDark;

                const rk = document.getElementById('ins-resource-ranking');
                if (rk) {
                    const entries = Object.entries(resourceMap).sort((a,b) => b[1]-a[1]), topMax = entries[0]?.[1]||1, colors = ['#3730a3','#d97706','#7c3aed','#16a34a','#ec4899','#06b6d4','#f87171'];
