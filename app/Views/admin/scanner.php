...
<head>
    ...
    <style>
        body {
            ...
            background-image:
                radial-gradient(ellipse 900px 500px at 75% -5%, rgba(37,99,235,0.06) 0%, transparent 65%),
                radial-gradient(ellipse 600px 400px at 0% 100%, rgba(37,99,235,0.04) 0%, transparent 60%);
        ...
        .sidebar-item:not(.active):hover { background: #eff6ff !important; color: #2563eb !important; transform: translateX(3px); }
        .sidebar-item.active { background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; box-shadow: 0 8px 20px rgba(37,99,235,0.28), 0 2px 6px rgba(37,99,235,0.15); }
        ...
        /* ── Scanner viewport ── */
        #scanner-viewport {
            width: 100%; border-radius: 20px; overflow: hidden; border: 2px solid #e2e8f0;
            min-height: 280px; background: #f8fafc;
            display: flex; align-items: center; justify-content: center;
            position: relative; transition: border-color 0.3s;
        }
        @media (min-width: 640px) { #scanner-viewport { min-height: 340px; } }
        #scanner-viewport.active { border-color: #93c5fd; }
        #qr-video { width: 100%; height: 100%; object-fit: cover; display: block; border-radius: 18px; }
        ...
        .scan-frame::before, .scan-frame::after { content: ''; position: absolute; width: 36px; height: 36px; border-color: #3b82f6; border-style: solid; }
        .scan-frame::before { top: 0; left: 0; border-width: 3px 0 0 3px; border-radius: 4px 0 0 0; }
        .scan-frame::after  { bottom: 0; right: 0; border-width: 0 3px 3px 0; border-radius: 0 0 4px 0; }
        .scan-frame-tr, .scan-frame-bl { position: absolute; width: 36px; height: 36px; border-color: #3b82f6; border-style: solid; }
        ...
        .scan-line { position: absolute; left: 8px; right: 8px; height: 2px; background: linear-gradient(90deg, transparent, #3b82f6, transparent); top: 8px; animation: scanLine 2s ease-in-out infinite; border-radius: 1px; }
        ...
        @keyframes flashBlue { 0% { box-shadow:0 0 0 0 rgba(37,99,235,0.5); } 70% { box-shadow:0 0 0 20px rgba(37,99,235,0); } 100% { box-shadow:0 0 0 0 rgba(37,99,235,0); } }
        .scan-flash { animation: flashBlue 0.6s ease; }

        /* Inputs */
        input[type="text"] {
            font-family: 'Plus Jakarta Sans', sans-serif; border: 1.5px solid #e2e8f0;
            border-radius: 12px; padding: 0.75rem 1rem; font-size: 0.875rem;
            background: #f8fafc; color: #1e293b; transition: all 0.2s; width: 100%;
        }
        input[type="text"]:focus { outline: none; border-color: #2563eb; background: white; box-shadow: 0 0 0 4px rgba(37,99,235,0.1); }
        ...
        .cam-dot.live { background: #3b82f6; box-shadow: 0 0 6px rgba(59,130,246,0.6); animation: livePulse 1.5s ease-in-out infinite; }
        @keyframes livePulse { 0%,100% { opacity:1; } 50% { opacity:0.5; } }

        /* Buttons */
        .btn-start { background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; box-shadow: 0 4px 12px rgba(37,99,235,0.3); transition: all 0.18s; }
        .btn-start:hover { box-shadow: 0 8px 20px rgba(37,99,235,0.4); transform: translateY(-1px); }
        ...
        .history-item:hover { border-color: #bfdbfe; background: #eff6ff; transform: translateX(2px); }

        /* ── Result overlay (bottom-sheet on mobile, inline on sm+) ── */
        ...
        .result-sheet {
            ...
            border-radius: 28px 28px 0 0; max-height: 88vh; overflow-y: auto; z-index: 1;
            animation: sheetUp 0.26s cubic-bezier(0.34,1.2,0.64,1) both;
        ...
        /* On sm+ suppress the overlay, show inline */
        @media (min-width: 640px) {
            .result-overlay      { display: none !important; }
            .result-panel-inline { display: block !important; }
        }
...
<div id="debugPanel">
    ...
    <button onclick="showAllReservations()" class="mt-2 text-xs bg-blue-100 px-2 py-1 rounded ml-1">Show All</button>
</div>
...
<main class="flex-1 min-w-0 p-4 lg:p-12 pb-32">
    ...
    <header class="flex justify-between items-center mb-6 lg:mb-10">
        <div>
            ...
            <p class="text-slate-500 font-medium text-sm mt-0.5">Verify and validate reservation e-tickets.</p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 lg:gap-8">
        ...
        <!-- Left: Camera + Manual + Inline result (sm+) -->
        <div class="xl:col-span-2 space-y-5">
            ...
            <div class="content-card">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 gap-3">
                    ...
                    <div class="flex gap-2 w-full sm:w-auto">
                        <button id="startBtn" onclick="startScanner()"
                            class="btn-start flex-1 sm:flex-none px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 justify-center">
                            <i class="fa-solid fa-camera"></i> Start Camera
                        </button>
                        <button id="switchBtn" onclick="switchCamera()" style="display:none"
                            class="btn-stop flex-none px-3 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center"
                            title="Switch Camera">
                            <i class="fa-solid fa-camera-rotate"></i>
                        </button>
                        <button id="stopBtn" onclick="stopScanner()" style="display:none"
                            class="btn-stop flex-1 sm:flex-none px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 justify-center">
                ...
                <div id="scanner-viewport">
                    <div class="text-center p-8" id="viewport-placeholder">
                ...
                <div class="mt-5 pt-5 border-t border-slate-100">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button onclick="processCode(document.getElementById('manualCode').value)"
                            class="btn-verify px-5 py-3 rounded-xl font-bold text-sm whitespace-nowrap flex-shrink-0">
                            Verify
                        </button>
                    </div>
                </div>
            ...
            <!-- Inline result panel — sm+ only -->
            <div id="resultPanelInline"
                 class="result-panel-inline content-card !p-0 border-2 result-idle overflow-hidden transition-all duration-300 hidden">
        ...
        <div class="space-y-5">
            ...
            <div class="content-card">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button onclick="clearHistory()"
                        class="text-red-400 text-xs font-bold hover:bg-red-50 hover:text-red-600 px-2 py-1 rounded-lg transition">
                        Clear
                    </button>
                </div>
            ...
            <div class="content-card">
                ...
                <div class="space-y-1">
                    ...
                    <div class="stat-row">
                        <span class="text-xs font-black uppercase tracking-widest text-blue-500">Valid</span>
                        <span id="statValid"   class="font-black text-blue-600">0</span>
                    </div>
                    <div class="stat-row">
                        <span class="text-xs font-black uppercase tracking-widest text-purple-500">Already Claimed</span>
                        <span id="statClaimed" class="font-black text-purple-600">0</span>
                    </div>
                    <div class="stat-row">
...
document.addEventListener('keydown', e => {
    if (e.ctrlKey && e.shiftKey && e.key === 'D') {
        ...
        if (debugMode) showAllReservations();
    }
});
...
/* ──────────────────────────────────────────
   START / OPEN / SWITCH / STOP
────────────────────────────────────────── */
async function startScanner(camIndex) {
    if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
        showCameraError('Camera requires HTTPS. Please access the site over a secure connection.'); return;
    }
    if (!navigator.mediaDevices?.getUserMedia) {
        showCameraError('Your browser does not support camera access. Please use Chrome, Firefox, or Safari.'); return;
    }
    ...
    catch { showCameraError('Could not access any camera. Please allow camera permissions and try again.'); return; }

    const devices = await navigator.mediaDevices.enumerateDevices();
    availableCameras = devices.filter(d => d.kind === 'videoinput');
    debug('Cameras found:', availableCameras.map(c => c.label || c.deviceId));
...
async function openCamera(index) {
    ...
    const constraint = device
        ? { video: { deviceId: { exact: device.deviceId }, width: { ideal:1280 }, height: { ideal:720 } } }
        : { video: true };

    let stream;
    try { stream = await navigator.mediaDevices.getUserMedia(constraint); }
    catch { try { stream = await navigator.mediaDevices.getUserMedia({ video:true }); } catch(e) { showCameraError('Could not open camera: ' + e.message); return; } }
    ...
    videoEl.onerror = e => showCameraError('Video error: ' + e.message);
}
...
function processCode(code) {
    ...
    if (!res) {
        ...
        showResult('error','fa-circle-xmark','text-red-500','bg-red-50',
            'Code Not Recognised', `"${code}" doesn't match any reservation.`, [], false);
        return;
    }

    debug('Found reservation:', { id:res.id, status:res.status, claimed:res.claimed });
    ...
    if      (effectiveStatus === 'approved') stats.valid++;
    ...
    if (parseInt(res.claimed) === 1 && res.claimed_at) {
        details.push({ label:'Claimed At', value: new Date(res.claimed_at).toLocaleString() });
    }
...
function findReservationByCode(code) {
    ...
    const m1 = code.match(/(?:SK|ADMIN)-(\d+)-/i);  if (m1) { r = reservations.find(r => r.id===parseInt(m1[1])); if (r) return r; }
    const m2 = code.match(/(?:SK|ADMIN)(\d+)/i);    if (m2) { r = reservations.find(r => r.id===parseInt(m2[1])); if (r) return r; }
...
/* ──────────────────────────────────────────
   RESULT DISPLAY  (bottom-sheet on mobile, inline on sm+)
────────────────────────────────────────── */
function buildResultHTML(state, icon, iconColor, iconBg, title, sub, details, showValidate, idSuffix) {
    const rows = details.map(d =>
        `<div class="flex justify-between items-start py-2.5 border-b border-slate-100 last:border-0 gap-3">
            <span class="text-[0.65rem] font-black uppercase tracking-widest text-slate-400 flex-shrink-0 mt-0.5">${d.label}</span>
            <span class="font-bold text-slate-800 text-sm text-right">${d.value}</span>
         </div>`
    ).join('');

    const validateBtnColor = 'bg-blue-600 hover:bg-blue-700';
    const validateSection  = showValidate
        ? `<div class="mt-5 pt-5 border-t border-slate-100">
               <button id="validateBtn${idSuffix}" onclick="validateTicket()"
                   class="w-full py-3.5 ${validateBtnColor} text-white rounded-2xl font-bold text-sm transition flex items-center justify-center gap-2">
    ...
    return `
        <div class="flex items-center gap-4 mb-5">
            <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-xl flex-shrink-0 ${iconBg} ${iconColor}">
                <i class="fa-solid ${icon}"></i>
            </div>
            <div class="flex-1 min-w-0">
                <p class="font-extrabold text-base text-slate-900 leading-tight">${title}</p>
                <p class="text-sm text-slate-500 font-medium mt-0.5">${sub}</p>
            </div>
        </div>
...
function renderHistory() {
    ...
    list.innerHTML = scanHistory.map(item => {
        ...
        return `<div class="history-item flex items-center justify-between p-3 bg-slate-50 rounded-2xl cursor-pointer"
                     onclick="processCode('${item.code.replace(/'/g,"\\'")}')">
                    <div class="overflow-hidden mr-3">
                        <p class="font-bold text-slate-800 text-xs truncate">${item.code}</p>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">${item.time}</p>
                    </div>
                    <div class="w-8 h-8 rounded-xl ${sc} flex items-center justify-center flex-shrink-0">
...
function clearHistory() {
    ...
    scanHistory = []; stats = { total:0, valid:0, claimed:0, pending:0, invalid:0 };
    ...
    closeResultSheet();
    currentReservation = null; currentCode = null;
}
