...
<head>
    ...
    <style>
        ...
        @keyframes flashIndigo {
            0% {
                box-shadow: 0 0 0 0 rgba(55, 48, 163, .5)
            }
        ...
        .cam-dot.live {
            ...
            box-shadow: 0 0 6px rgba(55, 48, 163, .5);
        ...
        .btn-start:hover {
            box-shadow: 0 8px 20px rgba(55, 48, 163, .4);
        ...
        body.dark .result-success {
            background: rgba(5, 46, 22, .5) !important;
            border-color: #16a34a !important
        }

        body.dark .result-warning {
            background: rgba(120, 53, 15, .4) !important;
        ...
        body.dark .result-claimed {
            background: rgba(59, 7, 100, .4) !important;
...
<body>
                ...
                <div class="card card-p">
                    <div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;margin-bottom:16px;gap:10px">
                        ...
                        <div style="display:flex;gap:8px">
                            <button id="startBtn" onclick="startScanner()" class="btn-start" style="padding:9px 16px;border-radius:var(--r-sm);font-weight:700;font-size:.8rem;display:inline-flex;align-items:center;gap:6px;border:none;cursor:pointer;font-family:var(--font)">
                                <i class="fa-solid fa-camera" style="font-size:.75rem"></i> Start Camera
                            </button>
                            <button id="switchBtn" onclick="switchCamera()" style="display:none;padding:9px 12px;border-radius:var(--r-sm);font-weight:700;font-size:.8rem;align-items:center;border:none;cursor:pointer;font-family:var(--font)" class="btn-stop" title="Switch Camera">
    ...
    <script>
        ...
        function showAllReservations() {
            debug('All:', reservations.map(r => ({
                id: r.id,
                code: r.e_ticket_code,
                status: r.status,
                claimed: r.claimed
            })));
        }
        ...
        async function openCamera(index) {
            ...
            for (const constraint of attempts) {
                try {
                    ...
                    debug('Constraint failed:', e.message);
                }
            }
            if (!stream) {
                showCameraError('Could not open camera. Please check permissions.');
                return;
            }
            videoStream = stream;
        ...
        async function switchCamera() {
            ...
            await openCamera(currentCamIndex);
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-solid fa-camera-rotate"></i>';
        }
            ...
            if (videoEl.readyState >= 2) {
                ...
                if (w && h) {
                    ...
                    try {
                        ...
                        debug('scanFrame error:', e.message);
                    }
                }
            }
        ...
        function processCode(code) {
            ...
            if (!res) {
                stats.invalid++;
                updateStats();
                showResult('error', 'fa-circle-xmark', '#dc2626', '#fee2e2', 'Code Not Recognised', `"${code}" doesn't match any reservation.`, [], false);
                return;
            }
            debug('Found:', {
                id: res.id,
            ...
            const details = [{
                ...
                {
                    label: 'E-Ticket Code',
                    value: res.e_ticket_code || '—'
                },
                {
                    label: 'Status',
                    value: effectiveStatus.charAt(0).toUpperCase() + effectiveStatus.slice(1)
                },
            ];
        ...
        function renderHistory() {
            ...
            if (!scanHistory.length) {
                list.innerHTML = '<p style="color:#94a3b8;font-size:.82rem;text-align:center;padding:24px 0;font-style:italic">No recent scans</p>';
                return;
            }
            const colorMap = {
                ...
                claimed: 'background:#ede9fe;color:#7c3aed'
            };
            const iconMap = {
                approved: 'fa-check',
                pending: 'fa-clock',
