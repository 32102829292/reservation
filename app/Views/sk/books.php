...
#booksTable th:nth-child(2){min-width:140px}
#booksTable th:nth-child(3){width:80px}
#booksTable th:nth-child(4){width:90px}
#booksTable th:nth-child(5){width:70px}
#booksTable th:nth-child(6){width:46px}
#booksTable th:nth-child(7){width:80px}
#booksTable th:nth-child(8){width:56px}
#booksTable th:nth-child(9){width:52px}
#booksTable th:nth-child(10){width:90px}
...
.book-avatar{width:42px;height:52px;border-radius:10px;background:linear-gradient(160deg,#dcfce7 0%,#f0fdf4 100%);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(22,163,74,.12)}
  ...
  <div id="paneBooks">
      ...
      <div class="overflow-x-auto">
        <table id="booksTable">
          ...
          <tr data-search="<?=strtolower(htmlspecialchars(($b['title']??'').' '.($b['author']??'').' '.($b['genre']??'').' '.($b['call_number']??''),ENT_QUOTES,'UTF-8'))?>">
            ...
            <td><?php if(!empty($b['preface'])):?><span class="badge badge-rag-yes text-[10px]"><i class="fa-solid fa-check text-[8px]"></i>Yes</span><?php else:?><span class="badge badge-rag-no text-[10px]">—</span><?php endif;?></td>
    ...
    <div class="md:hidden space-y-3" id="bookCardList">
      ...
      <div class="book-card"
        ...
        <!-- Top: avatar + title/author + status -->
        <div class="book-card-top">
          <div class="book-avatar">
            <i class="fa-solid fa-book-open text-green-500 text-lg"></i>
          </div>
        ...
        <div class="book-copies-row" onclick="event.stopPropagation()">
          ...
          <div>
            ...
            <span class="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-lg">
              <i class="fa-solid fa-wand-magic-sparkles text-[9px]"></i> AI Ready
            </span>
            <?php else:?>
        ...
        <table id="borrowingsTable">
          ...
          <tr data-status="<?=$s?>" data-search="<?=strtolower(htmlspecialchars(($bw['resident_name']??'').' '.($bw['book_title']??''),ENT_QUOTES,'UTF-8'))?>">
            ...
            <td><div class="actions-cell">
              <?php if($s==='pending'):?>
              <form method="post" action="/sk/borrowings/approve/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-2.5 py-1 bg-green-50 hover:bg-green-100 text-green-700 rounded-lg font-bold text-[11px] transition w-full"><i class="fa-solid fa-check text-[9px] mr-1"></i>Approve</button></form>
              <form method="post" action="/sk/borrowings/reject/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-bold text-[11px] transition w-full"><i class="fa-solid fa-xmark text-[9px] mr-1"></i>Reject</button></form>
              <?php elseif($s==='approved'):?>
              <form method="post" action="/sk/borrowings/return/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-bold text-[11px] transition w-full">Returned</button></form>
      ...
      <div class="borrow-card" data-status="<?=$s?>" data-search="<?=strtolower(htmlspecialchars(($bw['resident_name']??'').' '.($bw['book_title']??''),ENT_QUOTES,'UTF-8'))?>">
        ...
        <div class="borrow-card-book">
          <div class="w-7 h-7 bg-green-50 rounded-lg flex items-center justify-center flex-shrink-0">
            <i class="fa-solid fa-book text-green-500 text-[10px]"></i>
          </div>
        ...
        <div class="borrow-card-actions">
          <form method="post" action="/sk/borrowings/approve/<?=$bw['id']?>"><?=csrf_field()?>
            <button class="py-2.5 w-full bg-green-50 hover:bg-green-100 text-green-700 rounded-xl font-bold text-xs transition flex items-center justify-center gap-1.5">
              <i class="fa-solid fa-check text-[10px]"></i> Approve
            </button>
          </form>
          <form method="post" action="/sk/borrowings/reject/<?=$bw['id']?>"><?=csrf_field()?>
        ...
        <?php elseif($s==='approved'):?>
        <div class="borrow-card-actions">
          <form method="post" action="/sk/borrowings/return/<?=$bw['id']?>" style="grid-column:span 2"><?=csrf_field()?>
