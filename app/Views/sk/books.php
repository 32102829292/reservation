...
/**
 * Views/sk/books.php  — restyled to match dashboard.php design system
  ...
  <style>
    ...
    *::after {
    ...
    .tab-btn.active {
      background: var(--indigo);
      color: #fff;
      box-shadow: 0 3px 10px rgba(55, 48, 163, .28)
    }
    ...
    .fpill.fp-pending.active {
      background: #fef3c7;
      color: #92400e;
      border-color: #fde68a
    }

    .fpill.fp-approved.active {
      background: #dcfce7;
      color: #166534;
      border-color: #bbf7d0
    }

    .fpill.fp-returned.active {
      background: #dbeafe;
      color: #1d4ed8;
      border-color: #bfdbfe
    }

    .fpill.fp-rejected.active {
      background: #fee2e2;
      color: #991b1b;
      border-color: #fecaca
    }
    ...
    thead {
      background: #f8fafc;
      border-bottom: 2px solid rgba(99, 102, 241, .08)
    }
    ...
    .modal-submit {
      ...
      font-family: var(--font);
      transition: all var(--ease);
      box-shadow: 0 3px 10px rgba(55, 48, 163, .25)
    }
    ...
    body.dark .nav-link:hover:not(.active) .nav-icon {
    ...
    body.dark .logout-link:hover .nav-icon {
    ...
    body.dark .action-btn-outline:hover {
      background: rgba(99, 102, 241, .12)
    }
    ...
    body.dark .act-del:hover {
      background: rgba(239, 68, 68, .18)
    }

    body.dark .act-approve {
      background: rgba(22, 163, 74, .15);
      color: #4ade80
    }

    body.dark .act-reject {
      background: rgba(239, 68, 68, .1);
      color: #f87171
    }

    body.dark .act-return {
      background: #101e35;
      color: #7fb3e8
    }
    ...
    body.dark .book-meta-chip {
      background: #101e35;
      border-color: rgba(99, 102, 241, .12);
      color: #7fb3e8
    }

    body.dark .book-copies-row {
      border-color: rgba(99, 102, 241, .08)
    }

    body.dark .book-card-actions .act-edit {
      background: #101e35
    }

    body.dark .call-badge {
      background: rgba(92, 33, 182, .2);
      color: #a78bfa
    }

    body.dark .tag-rag-no {
      background: #101e35;
      color: #4a6fa5
    }
    ...
    body.dark .step-dot.pending {
      background: #101e35;
      color: #4a6fa5
    }

    body.dark .step-line.pending {
      background: #101e35
    }

    body.dark .ai-progress-bar {
      background: #101e35
    }

    body.dark .ai-spinner {
      border-color: #1a2a42;
  ...
  <div id="addModal" class="modal-back" onclick="if(event.target===this)closeModal('addModal')">
    <div class="modal-card">
      ...
      <form method="post" action="/sk/books/store">
        ...
        <div style="margin-bottom:14px;"><label class="form-lbl">Call Number <span style="color:#7c3aed;text-transform:none;letter-spacing:0;font-weight:500;">(shelf code)</span></label><input class="form-input" style="font-family:var(--mono);" name="call_number" placeholder="e.g. 823.914"></div>
    ...
    <div class="modal-card wide">
      ...
      <div id="aPdfStep1">
        ...
        <div id="aFilePreview" class="hidden" style="margin-top:12px;padding:12px 14px;background:#f8fafc;border-radius:var(--r-md);border:1px solid rgba(99,102,241,.08);display:flex;align-items:center;gap:12px;">
          ...
          <button onclick="aClearPdfFile()" style="background:none;border:none;color:#94a3b8;cursor:pointer;font-size:.85rem;" title="Remove"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <button id="aExtractBtn" onclick="aExtractFromPdf()" disabled style="margin-top:16px;width:100%;padding:12px;background:var(--indigo);color:white;border-radius:var(--r-sm);font-weight:700;font-size:.88rem;border:none;cursor:pointer;font-family:var(--font);display:flex;align-items:center;justify-content:center;gap:8px;transition:all var(--ease);box-shadow:0 3px 10px rgba(55,48,163,.25);" onmouseover="if(!this.disabled)this.style.background='#312e81'" onmouseout="if(!this.disabled)this.style.background='var(--indigo)'">
    ...
    <div class="modal-card">
      ...
      <form method="post" id="editForm" action="">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
          ...
          <div><label class="form-lbl">Status</label><select class="form-input" name="status" id="editStatus">
              ...
              <tbody>
                <?php foreach ($books as $i => $b): ?>
                  <tr data-search="<?= strtolower(htmlspecialchars(($b['title'] ?? '') . ' ' . ($b['author'] ?? '') . ' ' . ($b['genre'] ?? '') . ' ' . ($b['call_number'] ?? ''), ENT_QUOTES)) ?>">
                    ...
                    <td>
                      <p style="font-weight:700;font-size:.85rem;color:#0f172a;max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?= htmlspecialchars($b['title'] ?? '') ?></p>
                    ...
                    <td style="font-size:.82rem;color:#475569;font-weight:500;"><?= htmlspecialchars($b['genre'] ?? '—') ?></td>
                    <td style="font-size:.82rem;color:#475569;font-weight:500;font-family:var(--mono);"><?= htmlspecialchars($b['published_year'] ?? '—') ?></td>
                    <td>
              ...
              <div style="display:flex;align-items:flex-start;gap:12px;padding-bottom:12px;border-bottom:1px solid rgba(99,102,241,.05);margin-bottom:12px;">
                ...
                <div style="flex:1;min-width:0;">
                  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
                    <p style="font-weight:800;font-size:.88rem;color:#0f172a;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;"><?= htmlspecialchars($b['title'] ?? '') ?></p>
              ...
              <tbody>
                <?php foreach ($borrowings as $i => $bw): $s = strtolower($bw['status'] ?? 'pending'); ?>
                  <tr data-status="<?= $s ?>" data-search="<?= strtolower(htmlspecialchars(($bw['resident_name'] ?? '') . ' ' . ($bw['book_title'] ?? ''), ENT_QUOTES)) ?>">
                    <td style="color:#94a3b8;font-weight:700;font-size:.72rem;font-family:var(--mono);"><?= $i + 1 ?></td>
                    <td>
                      <p style="font-weight:700;font-size:.85rem;color:#0f172a;"><?= htmlspecialchars($bw['resident_name'] ?? 'Unknown') ?></p>
                      <p style="font-size:.7rem;color:#94a3b8;margin-top:2px;"><?= htmlspecialchars($bw['email'] ?? '') ?></p>
                    </td>
                    <td>
                      <p style="font-weight:700;font-size:.85rem;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:150px;"><?= htmlspecialchars($bw['book_title'] ?? '') ?></p>
                      <p style="font-size:.7rem;color:#94a3b8;margin-top:2px;"><?= htmlspecialchars($bw['book_author'] ?? '') ?></p>
                    </td>
                    <td style="font-size:.78rem;color:#475569;font-family:var(--mono);"><?= htmlspecialchars($bw['borrowed_at'] ?? '—') ?></td>
                    <td style="font-size:.78rem;color:#475569;font-family:var(--mono);"><?= htmlspecialchars($bw['due_date'] ?? '—') ?></td>
            ...
            <div class="borrow-card" data-status="<?= $s ?>" data-search="<?= strtolower(htmlspecialchars(($bw['resident_name'] ?? '') . ' ' . ($bw['book_title'] ?? ''), ENT_QUOTES)) ?>">
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;padding-bottom:12px;border-bottom:1px solid rgba(99,102,241,.05);margin-bottom:10px;">
                <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0;">
                  ...
                  <div style="min-width:0;">
                    <p style="font-weight:700;font-size:.88rem;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?= htmlspecialchars($bw['resident_name'] ?? 'Unknown') ?></p>
              ...
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                ...
                <div style="min-width:0;">
                  <p style="font-weight:700;font-size:.82rem;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?= htmlspecialchars($bw['book_title'] ?? '') ?></p>
  ...
  <script>
    ...
    function initPagination(f) {
      ...
      paginationRange(_currentPage, totalPages).forEach(p => {
        ...
        btn.className = 'page-btn' + (p === _currentPage ? ' active' : '') + (p === '…' ? '' : '');
        btn.textContent = p;
    ...
    function showPage(p, rows) {
      rows.forEach((r, i) => {
        r.style.display = (i >= (p - 1) * PAGE_SIZE && i < p * PAGE_SIZE) ? '' : 'none';
      });
    }
    ...
    function adjustCopies(bookId, delta, btn) {
      ...
      fetch('/sk/books/update-copies/' + bookId, {
        ...
        els.forEach(el => el.textContent = cur);
        btn.disabled = false;
      });
    }
    ...
    function aSetStep(n) {
      ...
      document.getElementById('aStepLabel').textContent = {
        1: 'Upload PDF',
        2: 'AI Extracting…',
        3: 'Review & Save'
      } [n] || '';
    ...
    function aAnimProgress(tp, dur) {
      ...
      const s = parseFloat(fill.style.width) || 0,
        t0 = Date.now();
      clearInterval(_aPI);
      _aPI = setInterval(() => {
    ...
    async function aExtractFromPdf() {
      if (!_aPdfText) return;
      aSetStep(2);
      aShowPanel('aPdfStep2');
      aAnimProgress(40, 2000);
      ...
      try {
        ...
        if (!ct.includes('application/json')) {
          ...
          aShowError({
            419: 'CSRF expired — refresh and try again.',
            403: 'Access forbidden.',
            401: 'Session expired.',
            404: 'Route not found.',
            500: 'Server error.'
          } [response.status] || 'HTTP ' + response.status, snippet);
          return;
        }
        const rb = await response.text();
        let result;
        try {
          ...
          aShowError('Server returned invalid JSON.', rb.slice(0, 300));
          return;
        }
        if (!result.ok) {
          aShowError(result.error || 'Extraction failed.', null);
          return;
        }
        aAnimProgress(100, 200);
        setTimeout(() => {
          aPopulate(result.data);
          aSetStep(3);
          aShowPanel('aPdfStep3');
        }, 400);
      } catch (err) {
        clearInterval(mint);
        aShowError('Network error: ' + err.message, null);
      }
    }
    ...
    function aPopulate(data) {
      ...
      Object.entries(fields).forEach(([elId, cfg]) => {
        const el = document.getElementById(elId),
          badge = document.getElementById(cfg.badge);
