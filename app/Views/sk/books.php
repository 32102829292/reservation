<?php
/**
 * Views/sk/books.php  (enhanced — mirrors admin/books.php)
 * Additions: call_number · pdf.js client extraction · inline copies +/− ·
 *            CSRF refresh · skeleton stat cards · styled delete confirm ·
 *            borrowings filter bar · pagination (20/page)
 */
?>
<!DOCTYPE html>
<html lang="en">
...
.card-copies-btn:hover{background:#dcfce7;border-color:#86efac;color:#16a34a}
...
.form-input:focus{border-color:#16a34a;background:white;box-shadow:0 0 0 3px rgba(22,163,74,.1)}
textarea.form-input{resize:vertical;min-height:90px}
.drop-zone{border:2.5px dashed #bbf7d0;border-radius:20px;padding:2.5rem 1.5rem;text-align:center;background:#f0fdf4;cursor:pointer;transition:all .2s;position:relative}
...
.ai-progress-fill{height:100%;background:linear-gradient(90deg,#16a34a,#4ade80,#16a34a);background-size:200% 100%;border-radius:999px;animation:shimmer 1.4s infinite;width:0%;transition:width .4s ease}
@keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
.field-filled{border-color:#16a34a!important;background:#f0fdf4!important}
.field-badge-ai{display:inline-flex;align-items:center;gap:4px;font-size:.65rem;font-weight:800;padding:.15rem .5rem;border-radius:999px;background:#dcfce7;color:#166534;margin-left:6px}
...
#skDebugPanel{display:none;margin-top:10px;padding:10px 14px;background:#fef2f2;border:1px solid #fecaca;border-radius:12px;font-size:.72rem;font-family:monospace;color:#991b1b;word-break:break-all;white-space:pre-wrap;max-height:120px;overflow-y:auto}
...
<div class="modal-backdrop" id="deleteModal">
  <div class="modal-box sm">
    ...
    <div class="px-7 pt-4 pb-6">
      ...
      <p class="text-xs text-slate-400 font-semibold text-center mb-6">This cannot be undone.</p>
...
<div class="modal-backdrop" id="addModal">
  <div class="modal-box">
    ...
    <form method="post" action="/sk/books/store" class="px-7 py-6 space-y-4">
      <?= csrf_field() ?>
      ...
      <div><label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Call Number <span class="ml-1 normal-case font-semibold text-violet-500">(shelf location code)</span></label><input class="form-input font-mono" name="call_number" placeholder="e.g. 823.914  or  PR6045.A97"></div>
      <div><label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Preface / Description <span class="ml-1 normal-case font-semibold text-green-600">(powers AI suggestions)</span></label><textarea class="form-input" name="preface" rows="4" placeholder="Describe the book…"></textarea></div>
      <div class="flex gap-3 pt-2">
        ...
        <button type="submit" class="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-green-200">Add Book</button>
      </div>
    </form>
  </div>
</div>
...
<div class="modal-backdrop" id="pdfModal">
  <div class="modal-box xlg">
    <div class="px-7 pt-5 pb-0"><div class="sheet-handle"></div></div>
    <div class="flex items-center justify-between px-7 pt-2 pb-4 border-b border-slate-100">
      <div><p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">AI Extraction</p><h3 class="text-lg font-black text-slate-900 flex items-center gap-2"><span class="w-8 h-8 bg-green-100 rounded-xl flex items-center justify-center"><i class="fa-solid fa-wand-magic-sparkles text-green-600 text-xs"></i></span>Upload PDF — AI will extract details</h3></div>
    ...
    <div class="px-7 py-6">
      ...
      <div id="aPdfStep1">
        <div class="drop-zone" id="aDropZone">
          ...
          <div class="w-14 h-14 bg-green-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fa-solid fa-file-pdf text-2xl text-green-600"></i></div>
        ...
        <div id="aFilePreview" class="hidden mt-4 px-4 py-3 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0"><p id="aFilePreviewName" class="font-bold text-sm text-slate-800 truncate"></p><p id="aFilePreviewSize" class="text-xs text-slate-400 mt-0.5"></p></div>
        ...
        <button id="aExtractBtn" onclick="aExtractFromPdf()" disabled class="mt-5 w-full py-3.5 bg-green-600 hover:bg-green-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-2xl font-bold text-sm transition flex items-center justify-center gap-2">
          <i class="fa-solid fa-wand-magic-sparkles"></i> Extract with AI
        </button>
      </div>
      <div id="aPdfStep2" style="display:none">
        <div class="text-center py-6">
          <div class="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fa-solid fa-robot text-green-500 text-2xl animate-pulse"></i></div>
      ...
      <div id="aPdfStep3" style="display:none">
        ...
        <form method="post" action="/sk/books/store" id="aPdfBookForm" class="space-y-4">
          <?= csrf_field() ?>
          <div class="grid grid-cols-2 gap-3">
          ...
          <div><label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Preface / Description <span class="field-badge-ai" id="aBadgePreface" style="display:none"><i class="fa-solid fa-wand-magic-sparkles text-[8px]"></i> AI</span> <span class="ml-1 normal-case font-semibold text-green-600">(powers AI suggestions)</span></label><textarea class="form-input" name="preface" id="aPdfPreface" rows="4" placeholder="AI-generated description…"></textarea></div>
          <div id="aAiConfidenceNote" class="px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-xl text-xs font-semibold text-amber-700 hidden"><i class="fa-solid fa-circle-info mr-1.5"></i>Title or author wasn't detected — please fill those in, then hit Save.</div>
          <div class="flex gap-3 pt-2">
            ...
            <button type="submit" class="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-green-200"><i class="fa-solid fa-check mr-1"></i> Save Book</button>
          </div>
        </form>
      </div>
...
<div class="modal-backdrop" id="editModal">
  <div class="modal-box">
    ...
    <form method="post" id="editForm" action="" class="px-7 py-6 space-y-4">
      ...
      <div><label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Preface / Description <span class="ml-1 normal-case font-semibold text-green-600">(AI context)</span></label><textarea class="form-input" name="preface" id="editPreface" rows="4"></textarea></div>
      <div class="flex gap-3 pt-2">
        ...
        <button type="submit" class="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-green-200">Save Changes</button>
      </div>
    </form>
  </div>
</div>
...
<aside class="hidden lg:block w-80 flex-shrink-0 p-6">
  <div class="sidebar-card">
    <div class="sidebar-header"><span class="text-xs font-black tracking-[0.2em] text-green-600 uppercase">Youth Portal</span><h1 class="text-2xl font-extrabold text-slate-800">SK<span class="text-green-600">.</span></h1></div>
...
<main class="flex-1 min-w-0 p-4 lg:p-10 pb-32">
  <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 fade-up">
    <div>
      <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Youth Portal</p>
      <h2 class="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight">Library <span class="text-green-600">Management</span></h2>
    ...
    <div class="flex items-center gap-3 flex-shrink-0">
      ...
      <button onclick="openModal('pdfModal')" class="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:border-green-400 hover:bg-green-50 text-slate-700 hover:text-green-700 rounded-2xl font-bold text-sm transition"><i class="fa-solid fa-wand-magic-sparkles text-green-500 text-xs"></i> Upload PDF</button>
      <button onclick="openModal('addModal')" class="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-green-200"><i class="fa-solid fa-plus text-xs"></i> Add Book</button>
    </div>
  </header>

  <?php if(session()->getFlashdata('success')):?><div class="mb-6 px-5 py-4 bg-green-50 border border-green-200 text-green-700 font-bold rounded-2xl flex items-center gap-3 text-sm"><i class="fa-solid fa-circle-check text-green-500"></i><?=session()->getFlashdata('success')?></div><?php endif;?>
  <?php if(session()->getFlashdata('error')):?><div class="mb-6 px-5 py-4 bg-red-50 border border-red-200 text-red-700 font-bold rounded-2xl flex items-center gap-3 text-sm"><i class="fa-solid fa-circle-exclamation text-red-400"></i><?=session()->getFlashdata('error')?></div><?php endif;?>
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 hidden" id="realStatCards">
    ...
    <div class="analytics-card" onclick="switchTab('books')">
      <div class="flex items-center justify-between mb-3"><div class="w-10 h-10 bg-green-50 rounded-2xl flex items-center justify-center"><i class="fa-solid fa-bolt text-green-600"></i></div><span class="text-xs font-black text-green-600 bg-green-50 px-2 py-0.5 rounded-full"><?=$totalBooks>0?round($ragReady/$totalBooks*100):0?>%</span></div>
  ...
  <div id="paneBooks">
    <div class="flex gap-3 items-center flex-wrap mb-4">
      <div class="relative flex-1 min-w-[200px]">
        ...
        <input id="bookSearch" type="text" placeholder="Search title, author, genre, call number…" oninput="applyFilter()" class="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 transition">
      </div>
      <div class="flex gap-2 sm:hidden">
        <button onclick="openModal('pdfModal')" class="px-3 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-2xl font-bold text-sm transition"><i class="fa-solid fa-wand-magic-sparkles text-green-500 text-xs"></i></button>
        <button onclick="openModal('addModal')" class="px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition flex items-center gap-2"><i class="fa-solid fa-plus text-xs"></i> Add</button>
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-16 text-center">
      ...
      <div class="flex gap-3 justify-center">
        <button onclick="openModal('pdfModal')" class="px-5 py-2.5 bg-white border border-slate-200 hover:border-green-400 text-slate-700 rounded-2xl font-bold text-sm transition flex items-center gap-2"><i class="fa-solid fa-wand-magic-sparkles text-green-500 text-xs"></i> Upload PDF</button>
        <button onclick="openModal('addModal')" class="px-5 py-2.5 bg-green-600 text-white rounded-2xl font-bold text-sm hover:bg-green-700 transition"><i class="fa-solid fa-plus mr-1.5"></i> Add Manually</button>
      </div>
    </div>
    <?php else:?>
      ...
      <div class="overflow-x-auto">
        <table id="booksTable">
          ...
          <tr data-search="<?=strtolower(htmlspecialchars(($b['title']??'').' '.($b['author']??'').' '.($b['genre']??'').' '.($b['call_number']??''),ENT_QUOTES,'UTF-8'))?>">
            ...
            <td><?php if(!empty($b['preface'])):?><span class="badge badge-rag-yes"><i class="fa-solid fa-check text-[9px]"></i>Yes</span><?php else:?><span class="badge badge-rag-no">No</span><?php endif;?></td>
    ...
    <div class="md:hidden space-y-3" id="bookCardList">
      <?php foreach($books as $i=>$b):?>
      <div class="book-card" data-search="<?=strtolower(htmlspecialchars(($b['title']??'').' '.($b['author']??'').' '.($b['genre']??'').' '.($b['call_number']??''),ENT_QUOTES,'UTF-8'))?>" onclick="openEditModal(<?=htmlspecialchars(json_encode($b),ENT_QUOTES,'UTF-8')?>)">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-book text-blue-500"></i></div>
          <div class="flex-1 min-w-0"><p class="font-bold text-sm text-slate-800 truncate"><?=htmlspecialchars($b['title']??'',ENT_QUOTES,'UTF-8')?></p><p class="text-xs text-slate-400 mt-0.5 truncate"><?=htmlspecialchars($b['author']??'',ENT_QUOTES,'UTF-8')?></p></div>
        ...
        <div class="flex items-center gap-2 flex-wrap text-xs text-slate-400 font-medium">
          ...
          <?php if(!empty($b['genre'])):?><span><?=htmlspecialchars($b['genre'],ENT_QUOTES,'UTF-8')?></span><?php endif;?>
          <?php if(!empty($b['published_year'])):?><span><?=htmlspecialchars($b['published_year'],ENT_QUOTES,'UTF-8')?></span><?php endif;?>
        </div>
  ...
  <div id="paneBorrowings" style="display:none">
    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-16 text-center">
      ...
      <button onclick="switchTab('books')" class="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white rounded-2xl font-bold text-sm hover:bg-green-700 transition"><i class="fa-solid fa-book text-xs"></i> View Catalog</button>
    </div>
    <?php else:?>
    ...
    <div class="flex items-center gap-2 flex-wrap mb-4">
      <div class="relative flex-1 min-w-[180px] max-w-xs">
        <i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none"></i>
        <input id="borrowSearch" type="text" placeholder="Search resident or book…" oninput="applyBorrowFilter()" class="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 outline-none focus:border-green-400 transition">
      </div>
        ...
        <table id="borrowingsTable">
          ...
          <tr data-status="<?=$s?>" data-search="<?=strtolower(htmlspecialchars(($bw['resident_name']??'').' '.($bw['book_title']??''),ENT_QUOTES,'UTF-8'))?>">
            ...
            <td><div class="actions-cell">
              <?php if($s==='pending'):?>
              <form method="post" action="/sk/borrowings/approve/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 rounded-xl font-bold text-xs transition">Approve</button></form>
              <form method="post" action="/sk/borrowings/reject/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl font-bold text-xs transition">Reject</button></form>
              <?php elseif($s==='approved'):?>
              <form method="post" action="/sk/borrowings/return/<?=$bw['id']?>" style="display:inline"><?=csrf_field()?><button class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs transition">Mark Returned</button></form>
    ...
    <div class="md:hidden space-y-3" id="borrowCardList">
      ...
      <div class="borrow-card" data-status="<?=$s?>" data-search="<?=strtolower(htmlspecialchars(($bw['resident_name']??'').' '.($bw['book_title']??''),ENT_QUOTES,'UTF-8'))?>">
        ...
        <div class="flex gap-4 text-[11px] text-slate-400 font-medium">
          <span>Borrowed: <?=htmlspecialchars($bw['borrowed_at']??'—',ENT_QUOTES,'UTF-8')?></span>
          <span class="text-rose-500 font-bold">Due: <?=htmlspecialchars($bw['due_date']??'—',ENT_QUOTES,'UTF-8')?></span>
        </div>
        <?php if($s==='pending'):?>
        <div class="borrow-card-actions">
          <form method="post" action="/sk/borrowings/approve/<?=$bw['id']?>"><?=csrf_field()?><button class="py-2.5 w-full bg-green-50 hover:bg-green-100 text-green-700 rounded-xl font-bold text-xs transition"><i class="fa-solid fa-check mr-1"></i> Approve</button></form>
          <form method="post" action="/sk/borrowings/reject/<?=$bw['id']?>"><?=csrf_field()?><button class="py-2.5 w-full bg-red-50 hover:bg-red-100 text-red-600 rounded-xl font-bold text-xs transition"><i class="fa-solid fa-xmark mr-1"></i> Reject</button></form>
        </div>
        <?php elseif($s==='approved'):?>
        <div class="borrow-card-actions"><form method="post" action="/sk/borrowings/return/<?=$bw['id']?>"><?=csrf_field()?><button class="py-2.5 w-full bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs transition"><i class="fa-solid fa-rotate-left text-[10px] mr-1"></i> Mark Returned</button></form></div>
        <?php endif;?>
      </div>
      <?php endforeach;?>
    </div>
...
const CLS_ACTIVE='bg-green-600 text-white shadow-sm shadow-green-200';
...
document.addEventListener('keydown',function(e){if(e.key==='Escape')['addModal','editModal','pdfModal','deleteModal'].forEach(closeModal)});

function confirmDelete(id,title){document.getElementById('deleteBookTitle').textContent=title;document.getElementById('deleteForm').action='/sk/books/delete/'+id;openModal('deleteModal')}
...
function applyFilter(){
  ...
  var rows=document.querySelectorAll('#booksTable tbody tr');var cards=document.querySelectorAll('#bookCardList .book-card');var visible=0;
  rows.forEach(function(r){var m=!q||r.dataset.search.includes(q);r.style.display=m?'':'none';if(m)visible++});
  ...
  if(q&&noMsg)noMsg.classList.toggle('hidden',visible>0); else if(noMsg)noMsg.classList.add('hidden');
  initPagination(q);
}
...
function initPagination(f){
  ...
  var ctrl=document.getElementById('paginationControls');if(!ctrl)return;
  if(totalPages<=1){ctrl.classList.add('hidden');showPage(1,rows);return}
  ctrl.classList.remove('hidden');_currentPage=Math.min(_currentPage,totalPages);showPage(_currentPage,rows);
  var btns=document.getElementById('pageButtons');btns.innerHTML='';
  var prev=document.createElement('button');prev.className='page-btn';prev.textContent='‹';prev.disabled=_currentPage===1;prev.onclick=function(){goToPage(_currentPage-1,f)};btns.appendChild(prev);
  paginationRange(_currentPage,totalPages).forEach(function(p){var btn=document.createElement('button');btn.className='page-btn'+(p===_currentPage?' active':'')+(p==='…'?' pointer-events-none opacity-50':'');btn.textContent=p;if(p!=='…')btn.onclick=function(){goToPage(p,f)};btns.appendChild(btn)});
  var next=document.createElement('button');next.className='page-btn';next.textContent='›';next.disabled=_currentPage===totalPages;next.onclick=function(){goToPage(_currentPage+1,f)};btns.appendChild(next);
  var start=(_currentPage-1)*PAGE_SIZE+1;document.getElementById('pageInfo').textContent='Showing '+start+'–'+Math.min(_currentPage*PAGE_SIZE,total)+' of '+total;
}
function showPage(p,rows){rows.forEach(function(r,i){r.style.display=(i>=(p-1)*PAGE_SIZE&&i<p*PAGE_SIZE)?'':'none'})}
function goToPage(p,f){_currentPage=p;initPagination(f)}
function paginationRange(c,t){if(t<=7)return Array.from({length:t},function(_,i){return i+1});var r=[1];if(c>3)r.push('…');for(var i=Math.max(2,c-1);i<=Math.min(t-1,c+1);i++)r.push(i);if(c<t-2)r.push('…');r.push(t);return r}

/* ★ Inline copies */
function adjustCopies(bookId,delta,btn){
  var sels=['#copiesVal-'+bookId,'#copiesVal-'+bookId+'-m'];
  var els=sels.map(function(s){return document.querySelector(s)}).filter(Boolean);if(!els.length)return;
  var cur=parseInt(els[0].textContent)||0;var nv=Math.max(0,cur+delta);
  els.forEach(function(el){el.textContent=nv});
  var csrfToken=document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')||'';
  btn.disabled=true;
  fetch('/sk/books/update-copies/'+bookId,{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({available_copies:nv})})
  .then(function(res){var t=res.headers.get('X-CSRF-TOKEN');if(t)document.querySelector('meta[name="csrf-token"]')?.setAttribute('content',t);btn.disabled=false;if(!res.ok)els.forEach(function(el){el.textContent=cur})})
...
function filterBorrowings(status){
  ...
  ['all','pending','approved','returned','rejected'].forEach(function(s){var p=document.getElementById('bpill-'+s);if(!p)return;p.classList.remove('active');if(s===status)p.classList.add('active')});
  applyBorrowFilter();
}
function applyBorrowFilter(){
  ...
  var noMsg=document.getElementById('noBorrowResultsMsg');if(noMsg)noMsg.classList.toggle('hidden',(status==='all'&&!q)||visible>0);
}

/* ★ PDF extraction — client-side via pdf.js */
var _aPdfText=null;var _aPI=null;
if(typeof pdfjsLib!=='undefined'){
  var dz=document.getElementById('aDropZone');
  if(dz){
    dz.addEventListener('dragover',function(e){e.preventDefault();dz.classList.add('dragover')});
    dz.addEventListener('dragleave',function(){dz.classList.remove('dragover')});
    dz.addEventListener('drop',function(e){e.preventDefault();dz.classList.remove('dragover');var f=e.dataTransfer.files[0];if(f)aProcessPdfFile(f)});
  }
}
...
function aProcessPdfFile(file){
  ...
  document.getElementById('aDropZone').style.borderColor='#16a34a';
  ...
  reader.onload=async function(ev){
    try{
      ...
      var pages=[];
      for(var p=1;p<=Math.min(pdf.numPages,8);p++){var page=await pdf.getPage(p);var c=await page.getTextContent();pages.push(c.items.map(function(s){return s.str}).join(' '))}
...
function aSetStep(n){[1,2,3].forEach(function(i){document.getElementById('aStepDot'+i).className='step-dot '+(i<n?'done':i===n?'active':'pending')});[1,2].forEach(function(i){document.getElementById('aStepLine'+i).className='step-line '+(i<n?'done':'pending')});document.getElementById('aStepLabel').textContent={1:'Upload PDF',2:'AI Extracting…',3:'Review & Save'}[n]||''}
function aShowPanel(id){['aPdfStep1','aPdfStep2','aPdfStep3','aPdfStepError'].forEach(function(p){document.getElementById(p).style.display=p===id?'':'none'})}
function aAnimProgress(tp,dur){var fill=document.getElementById('aAiProgressFill');var s=parseFloat(fill.style.width)||0;var t0=Date.now();clearInterval(_aPI);_aPI=setInterval(function(){var pct=Math.min(s+(tp-s)*((Date.now()-t0)/dur),tp);fill.style.width=pct+'%';if(pct>=tp)clearInterval(_aPI)},30)}
function aShowError(msg,dbg){clearInterval(_aPI);document.getElementById('aPdfErrorText').textContent=msg;var d=document.getElementById('skDebugPanel');if(dbg){d.textContent=dbg;d.style.display='block'}else{d.style.display='none'}aShowPanel('aPdfStepError');aSetStep(1)}
async function aExtractFromPdf(){
  if(!_aPdfText)return;
  aSetStep(2);aShowPanel('aPdfStep2');document.getElementById('skDebugPanel').style.display='none';aAnimProgress(40,2000);
  var msgs=['Sending text to AI…','Extracting title and author…','Identifying genre and year…','Generating description…','Finalizing…'];
  var mi=0;var me=document.getElementById('aAiStatusText');var mint=setInterval(function(){mi=(mi+1)%msgs.length;me.textContent=msgs[mi]},2200);
  try{
    aAnimProgress(75,8000);
    var csrfToken=document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')||'';
    var response=await fetch('/sk/books/extract-pdf',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({pdf_text:_aPdfText})});
    var newToken=response.headers.get('X-CSRF-TOKEN');if(newToken)document.querySelector('meta[name="csrf-token"]')?.setAttribute('content',newToken);
    clearInterval(mint);aAnimProgress(95,500);
    var ct=response.headers.get('Content-Type')||'';
    if(!ct.includes('application/json')){var rb=await response.text();var snippet=rb.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim().slice(0,300);aShowError({419:'CSRF expired — refresh and try again.',403:'Access forbidden.',401:'Session expired.',404:'Route not found.',500:'Server error.'}[response.status]||'HTTP '+response.status,snippet);return}
    ...
    setTimeout(function(){aPopulate(result.data);aSetStep(3);aShowPanel('aPdfStep3')},400);
...
function aPopulate(data){
  ...
  Object.entries(fields).forEach(function([elId,cfg]){var el=document.getElementById(elId);var badge=document.getElementById(cfg.badge);var val=(data[cfg.key]||'').trim();el.value=val;if(val){el.classList.add('field-filled');if(badge)badge.style.display='inline-flex'}else{el.classList.remove('field-filled');if(badge)badge.style.display='none';if(['title','author'].includes(cfg.key))anyMissing=true}});
