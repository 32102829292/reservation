...
<head>
    ...
    <style>
        ...
        #resDate,
        #startTime,
        #endTime { display: none !important; }
        ...
        body:not(.dark) .cal-foot-btn.today:hover { background: var(--indigo-light); }
...
<body>
            ...
            <form id="reservationForm" method="POST" action="<?= base_url('sk/create-reservation') ?>">
                ...
                <div style="margin-bottom:22px">
                    ...
                    <div class="type-toggle">
                        <button type="button" class="type-btn active" id="btnUser" onclick="setType('User')"><i class="fa-solid fa-user" style="font-size:.8rem"></i> Registered User</button>
                        <button type="button" class="type-btn" id="btnVisitor" onclick="setType('Visitor')"><i class="fa-solid fa-person-walking" style="font-size:.8rem"></i> Walk-in Visitor</button>
                    </div>
                </div>
                <hr class="divider">
                ...
                <div style="margin-bottom:22px">
                    ...
                    <!-- Date / time row — custom pickers replace native inputs -->
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px" id="dateTimeGrid">
                        ...
                        <div>
                            ...
                            <!-- Hidden input keeps original name/id so form POST works unchanged -->
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            function checkGrid() {
                ...
                const vf = document.getElementById('visitorFields');
                const cols = window.innerWidth < 640 ? '1fr' : '1fr 1fr';
                [uf, vf].forEach(el => { if (el) el.style.gridTemplateColumns = cols; });
    ...
    <!-- ════════════════════════════════════════
         CUSTOM DATE / TIME PICKER  — drop-in JS
         Works for both light & dark mode.
         All values are written to the original
         hidden inputs so the form POST is unchanged.
         ════════════════════════════════════════ -->
    <script>
    (function () {
        'use strict';
        ...
        function renderCal() {
            ...
            let html = `
              <div class="cal-head">
                <div class="cal-nav-btn" id="_calPrev">&#8249;</div>
                <div class="cal-month-label">${MONTHS[m]} ${y}</div>
                <div class="cal-nav-btn" id="_calNext">&#8250;</div>
              </div>
              <div class="cal-grid">
                ${DOWS.map(d => `<div class="cal-dow">${d}</div>`).join('')}`;

            for (let i = 0; i < firstDow; i++)
                html += `<div class="cal-day cal-other">${prevTotal - firstDow + 1 + i}</div>`;

            for (let d = 1; d <= daysInM; d++) {
                const isToday = d === TODAY.getDate() && m === TODAY.getMonth() && y === TODAY.getFullYear();
                const isSel   = selDate && selDate.d === d && selDate.m === m && selDate.y === y;
                const cls = ['cal-day',
                    isToday && !isSel ? 'cal-today' : '',
                    isSel             ? 'cal-selected' : ''
                ].filter(Boolean).join(' ');
            ...
            html += `</div>
              <div class="cal-footer">
                <span class="cal-foot-btn"     id="_calClear">Clear</span>
                <span class="cal-foot-btn today" id="_calToday">Today</span>
              </div>`;
            ...
            $('calDrop').querySelectorAll('.cal-day[data-d]').forEach(el =>
                el.addEventListener('click', e => { e.stopPropagation(); pickDay(+el.dataset.d); })
            );
        }
        ...
        function renderTime(which) {
            ...
            const hItems = hours.map(h =>
                `<div class="tim-item${st.h === h ? ' sel' : ''}" data-part="h" data-val="${h}">${String(h).padStart(2,'0')}</div>`
            ).join('');
            const mItems = MINS.map(mn =>
                `<div class="tim-item${st.min === mn ? ' sel' : ''}" data-part="min" data-val="${mn}">${String(mn).padStart(2,'0')}</div>`
            ).join('');
            ...
            $(dropId).innerHTML = `
              ...
              <div class="tim-cols">
                ...
                <div class="ampm-col">
                  <div class="ampm-btn${st.ampm==='am'?' sel':''}" data-ampm="am">AM</div>
                  <div class="ampm-btn${st.ampm==='pm'?' sel':''}" data-ampm="pm">PM</div>
                </div>
              </div>
              <button class="tim-set-btn" id="_timSet_${which}">Set Time</button>`;
            ...
            $(dropId).querySelectorAll('.ampm-btn').forEach(el =>
                el.addEventListener('click', e => {
                    e.stopPropagation();
        ...
        function applyTime(which) {
            const st       = tState[which];
            const hDisp    = String(st.h).padStart(2,'0');
            const mDisp    = String(st.min).padStart(2,'0');
            const label    = `${hDisp}:${mDisp} ${st.ampm.toUpperCase()}`;
            let h24 = st.h;
        ...
        /* pre-populate date label with today's date (value already set in hidden input) */
        (function () {
            const today = new Date();
            $('dateLabel').textContent = today.toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' });
            $('dateTrigger').classList.add('has-value');
            selDate = { d: today.getDate(), m: today.getMonth(), y: today.getFullYear() };
        })();
