...
<head>
    ...
    <style>
        ...
        body.dark .sheet-handle { background: #1a2a42 !important; }
        ...
        body.dark .btn-stop { background: #101e35 !important; border-color: rgba(99,102,241,.12) !important; color: #7fb3e8 !important; }
        body.dark .btn-icon { background: #101e35 !important; border-color: rgba(99,102,241,.12) !important; color: #7fb3e8 !important; }
        body.dark .btn-verify { background: #e2eaf8 !important; color: #0f172a !important; }
    </style>
</head>
<body>
            ...
            <div class="card card-p">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div><div class="card-title">Live Camera Feed</div><div class="card-sub">Point at a QR code to scan</div></div>
                    </div>
        ...
        <div style="display:flex;flex-direction:column;gap:14px;">
            <div class="card card-p">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                    ...
                    <div><div class="card-title">Session Stats</div><div class="card-sub">This scan session</div></div>
                </div>
            ...
            <div class="card card-p" style="flex:1;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                    <div style="display:flex;align-items:center;gap:10px;"><div class="card-icon" style="background:var(--indigo-light);"><i class="fa-solid fa-clock-rotate-left" style="color:var(--indigo);font-size:.9rem;"></i></div><div class="card-title">Recent Scans</div></div>
...
function buildScannerUI(){
    ...
    videoEl=document.createElement('video');videoEl.setAttribute('autoplay','');videoEl.setAttribute('muted','');videoEl.setAttribute('playsinline','');
    ...
    canvasEl=document.createElement('canvas');canvasEl.style.display='none';document.body.appendChild(canvasEl);
}
...
async function startScanner(camIndex){
    ...
    document.getElementById('startBtn').style.display='none';document.getElementById('stopBtn').style.display='inline-flex';
    buildScannerUI();
    try{const tmp=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});tmp.getTracks().forEach(t=>t.stop());}catch(e){showCameraError('Could not access camera. Please allow camera permissions.');return;}
...
async function openCamera(index){
    ...
    videoEl.onloadedmetadata=()=>{videoEl.play().then(()=>{scanning=true;setCamStatus(true);requestAnimationFrame(scanFrame);}).catch(()=>showCameraError('Could not play video.'));};
...
async function switchCamera(){
    ...
    currentCamIndex=(currentCamIndex+1)%availableCameras.length;await openCamera(currentCamIndex);
    btn.disabled=false;btn.innerHTML='<i class="fa-solid fa-camera-rotate" style="font-size:.85rem;"></i>';
}

function scanFrame(){
    if(!scanning||!videoEl)return;
    if(videoEl.readyState>=2){const w=videoEl.videoWidth,h=videoEl.videoHeight;if(w&&h){canvasEl.width=w;canvasEl.height=h;try{const ctx=canvasEl.getContext('2d');ctx.drawImage(videoEl,0,0,w,h);const code=jsQR(ctx.getImageData(0,0,w,h).data,w,h,{inversionAttempts:'dontInvert'});if(code?.data&&code.data!==lastScanned){lastScanned=code.data;setTimeout(()=>{lastScanned=null;},3000);const vp=document.getElementById('scanner-viewport');vp.classList.add('scan-flash');vp.addEventListener('animationend',()=>vp.classList.remove('scan-flash'),{once:true});processCode(code.data);}}catch(e){}}}
    rafId=requestAnimationFrame(scanFrame);
}

function stopScanner(){
    scanning=false;if(rafId){cancelAnimationFrame(rafId);rafId=null;}if(videoStream){videoStream.getTracks().forEach(t=>t.stop());videoStream=null;}if(canvasEl?.parentNode){canvasEl.parentNode.removeChild(canvasEl);canvasEl=null;}videoEl=null;availableCameras=[];
    document.getElementById('switchBtn').style.display='none';setCamStatus(false);resetViewport();
}
...
function showCameraError(msg){
    ...
    document.getElementById('startBtn').style.display='inline-flex';document.getElementById('stopBtn').style.display='none';setCamStatus(false);
}
...
function processCode(code){
    ...
    let pcLabel='—';
    if(res.pc_numbers){try{const a=typeof res.pc_numbers==='string'?JSON.parse(res.pc_numbers):res.pc_numbers;pcLabel=Array.isArray(a)?a.join(', '):a;}catch{pcLabel=res.pc_numbers;}}else if(res.pc_number){pcLabel=res.pc_number;}
    const eff=isClaimed(res)?'claimed':res.status;
    const cfgMap={approved:{state:'success',icon:'fa-circle-check',iconColor:'#16a34a',iconBg:'#dcfce7',title:'Access Granted',sub:'Reservation is approved and valid.'},pending:{state:'warning',icon:'fa-clock',iconColor:'#d97706',iconBg:'#fef3c7',title:'Pending Approval',sub:'This reservation has not been approved yet.'},declined:{state:'error',icon:'fa-ban',iconColor:'#dc2626',iconBg:'#fee2e2',title:'Access Denied',sub:'This reservation has been declined.'},canceled:{state:'error',icon:'fa-ban',iconColor:'#dc2626',iconBg:'#fee2e2',title:'Access Denied',sub:'This reservation was canceled.'},claimed:{state:'claimed',icon:'fa-check-double',iconColor:'#7c3aed',iconBg:'#ede9fe',title:'Already Claimed',sub:'This ticket has already been used.'}};
    const cfg=cfgMap[eff]||cfgMap.pending;
    if(eff==='approved')stats.approved++;else if(eff==='claimed')stats.claimed++;else if(eff==='pending')stats.pending++;else stats.invalid++;
    updateStats();
    const details=[{label:'Reservation ID',value:'#'+res.id},{label:'Name',value:res.visitor_name||res.full_name||'Guest'},{label:'Email',value:res.visitor_email||res.user_email||'—'},{label:'Asset',value:res.resource_name||`Resource #${res.resource_id}`},{label:'Workstation',value:pcLabel},{label:'Date',value:res.reservation_date},{label:'Time',value:`${res.start_time} – ${res.end_time}`},{label:'Purpose',value:res.purpose||'—'},{label:'Status',value:eff.charAt(0).toUpperCase()+eff.slice(1)}];
...
function showResult(state,icon,iconColor,iconBg,title,sub,details,showValidate){
    const isMob=window.innerWidth<640, html=buildResultHTML(state,icon,iconColor,iconBg,title,sub,details,showValidate,isMob?'Sheet':'Inline'), stateClass=`result-${state}`;
    if(isMob){document.getElementById('resultSheetBody').innerHTML=html;document.getElementById('resultOverlay').classList.add('open');document.body.style.overflow='hidden';}
    else{const panel=document.getElementById('resultPanelInline');panel.className=`result-panel-inline card border-2 ${stateClass}`;document.getElementById('resultBodyInline').innerHTML=html;panel.style.display='block';panel.scrollIntoView({behavior:'smooth',block:'nearest'});}
}
...
function validateTicket(){
    ...
    const isMob=window.innerWidth<640, btn=document.getElementById('validateBtn'+(isMob?'Sheet':'Inline'));
    if(!btn)return;
    btn.disabled=true;btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Checking in…';
    const codeToValidate=currentReservation.e_ticket_code||currentCode, body=new URLSearchParams();
    ...
    .then(r=>r.json()).then(data=>{
        if(data.status==='success'||data.updated){const lr=reservations.find(r=>r.id===currentReservation.id);if(lr){lr.claimed=true;lr.claimed_at=new Date().toISOString();}stats.approved=Math.max(0,stats.approved-1);stats.claimed++;updateStats();btn.innerHTML='<i class="fa-solid fa-circle-check"></i> Checked In!';btn.style.background='#10b981';btn.disabled=true;renderHistory();}
        else if((data.message||'').toLowerCase().includes('claimed')){const lr=reservations.find(r=>r.id===currentReservation.id);if(lr)lr.claimed=true;stats.approved=Math.max(0,stats.approved-1);stats.claimed++;updateStats();btn.disabled=true;btn.innerHTML='<i class="fa-solid fa-check-double"></i> Already claimed';btn.style.background='#7c3aed';}
...
function addToHistory(code){const time=new Date().toLocaleTimeString('en-PH',{hour:'2-digit',minute:'2-digit'});scanHistory=scanHistory.filter(h=>h.code!==code);scanHistory.unshift({code,time});if(scanHistory.length>10)scanHistory.pop();renderHistory();}
...
function updateStats(){stats.total=scanHistory.length;document.getElementById('statTotal').textContent=stats.total;document.getElementById('statApproved').textContent=stats.approved;document.getElementById('statClaimed').textContent=stats.claimed;document.getElementById('statPending').textContent=stats.pending;document.getElementById('statInvalid').textContent=stats.invalid;}
