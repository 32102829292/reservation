...
<head>
    ...
    <style>
        ...
        .result-sheet {
            ...
            border-radius: 28px 28px 0 0; padding: 0; max-height: 88vh;
        ...
        /* On sm+ the overlay reverts to a regular inline panel */
...
<!-- ══════════════════════════════════════
     RESULT BOTTOM-SHEET  (mobile only, <sm)
     ══════════════════════════════════════ -->
...
<main class="flex-1 min-w-0 p-4 lg:p-12 pb-32">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 lg:gap-8">
        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <!-- Inline result panel — visible on sm+ only -->
        ...
        <div class="space-y-5">
            ...
            <div class="content-card">
                ...
                <div class="space-y-1">
                    ...
                    <div class="stat-row">
                        <span class="text-xs font-black uppercase tracking-widest text-red-400">Invalid</span>
...
function showAllReservations() { debug('All:', reservations.map(r => ({id:r.id, e_ticket_code:r.e_ticket_code, status:r.status}))); }
...
async function startScanner(camIndex) {
    if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
        showCameraError('Camera requires HTTPS. Please access the site over a secure connection.');
        return;
    }
    if (!navigator.mediaDevices?.getUserMedia) {
        showCameraError('Your browser does not support camera access. Please use Chrome, Firefox, or Safari.');
        return;
    }
...
function processCode(code) {
    ...
    debug('Found reservation:', { id:res.id, status:res.status });
    ...
    const cfg = cfgMap[res.status] || cfgMap.pending;
    ...
    showResult(cfg.state, cfg.icon, cfg.iconColor, cfg.iconBg, cfg.title, cfg.sub, details, res.status==='approved');
...
function findReservationByCode(code) {
    ...
    const m1 = code.match(/SK-(\d+)-/);                if (m1) { r = reservations.find(r => r.id===parseInt(m1[1])); if (r) return r; }
    const m2 = code.match(/SK(\d+)/);                  if (m2) { r = reservations.find(r => r.id===parseInt(m2[1])); if (r) return r; }
...
function buildResultHTML(state, icon, iconColor, iconBg, title, sub, details, showValidate, idSuffix) {
    ...
    return `
        ...
        <div class="space-y-0">${detailRows}</div>
        ${validateSection}`;
}
...
function renderHistory() {
    ...
    const colorMap = { approved:'bg-emerald-100 text-emerald-600', pending:'bg-amber-100 text-amber-600', declined:'bg-red-100 text-red-500', canceled:'bg-red-100 text-red-400' };
    const iconMap  = { approved:'fa-check', pending:'fa-clock', declined:'fa-ban', canceled:'fa-ban' };
    list.innerHTML = scanHistory.map(item => {
        const res  = findReservationByCode(item.code);
        const sc   = res ? (colorMap[res.status] || 'bg-slate-100 text-slate-400') : 'bg-red-100 text-red-400';
        const si   = res ? (iconMap[res.status]  || 'fa-question')                 : 'fa-xmark';
...
function clearHistory() {
    ...
    scanHistory = []; stats = { total:0, approved:0, pending:0, invalid:0 };
