...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── ROOT LAYOUT (matches dashboard exactly) ── */
        body {
            ...
            display: flex;
            height: 100vh;
            overflow: hidden;     /* prevent body scroll — only .main-col scrolls */
        }

        .page-wrapper { display: flex; width: 100%; height: 100vh; overflow: hidden; }

        /* Sidebar column — hidden on mobile, 280px on desktop */
        ...
        /* ── MOBILE NAV PILL (matches dashboard) ── */
        .mobile-nav-pill {
            position: fixed;
        ...
        .field-input:focus { outline: none; border-color: var(--green); background: white; box-shadow: 0 0 0 4px rgba(22,163,74,.08); }
            ...
            <nav class="sidebar-nav space-y-1">
                <?php foreach ($navItems as $item): $a = ($page == $item['key']) ? 'active' : ''; ?>
                    <a href="<?= $item['url'] ?>" class="sidebar-item <?= $a ?>">
                        ...
                        <?= $item['label'] ?>
                        <?php if ($item['key'] === 'user-requests' && $pendingUserCount > 0): ?>
    ...
    <div class="main-col">
        <main class="w-full max-w-screen-xl mx-auto px-4 lg:px-8 pt-6 pb-36">
            ...
            <header class="flex justify-between items-center mb-8">
                ...
                <span class="hidden md:inline-block bg-green-50 text-green-600 px-4 py-2 rounded-xl font-bold text-xs uppercase tracking-widest border border-green-100">
                    SK Verified
                </span>
            </header>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-[32px] border border-slate-200 shadow-sm p-6 text-center lg:sticky lg:top-8">
                            ...
                            <h3 class="text-lg lg:text-xl font-black text-slate-800"><?= htmlspecialchars($user['name'] ?? 'SK Officer') ?></h3>
                    ...
                    <div class="lg:col-span-2 space-y-5">
                        <div class="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            <div class="p-6 lg:p-8 space-y-5">
                                <?php
                                $fields = [
                                    ['icon' => 'fa-id-badge', 'label' => 'Full Name',      'value' => $user['name']  ?? 'SK Officer'],
                                    ['icon' => 'fa-envelope', 'label' => 'Email Address',  'value' => $user['email'] ?? '—'],
                                    ['icon' => 'fa-phone',    'label' => 'Contact Number', 'value' => $user['phone'] ?? 'Not set'],
                                    ['icon' => 'fa-star',     'label' => 'Role',           'value' => 'Sangguniang Kabataan Officer'],
                                ];
                        ...
                        <!-- Info banner -->
                        <div class="bg-green-600 rounded-[32px] p-6 lg:p-8 text-white flex items-center gap-4 lg:gap-6">
...
<div id="editModal" class="modal-backdrop" onclick="handleBackdrop(event)">
    <div class="modal-box">
        ...
        <form action="<?= base_url('sk/profile/update') ?>" method="POST" class="space-y-5">
            <?= csrf_field() ?>
            <div>
