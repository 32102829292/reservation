...
<main class="w-full max-w-screen-xl mx-auto px-4 lg:px-8 pt-6 pb-32">
    ...
    <header class="flex items-start justify-between mb-6 gap-4 fade-up">
        ...
        <div class="flex items-center gap-3 flex-shrink-0 flex-wrap justify-end">
            ...
            <?php endif; ?>
            <div class="relative">
                <button id="bellBtn" onclick="toggleNotif()" class="w-10 h-10 bg-white border border-slate-200 rounded-2xl flex items-center justify-center shadow-sm hover:border-green-300 transition text-slate-500"><i class="fa-regular fa-bell"></i></button>
            ...
            <a href="/sk/new-reservation" class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-2xl font-bold text-xs transition shadow-lg shadow-green-200"><i class="fa-solid fa-plus text-xs"></i> Reserve</a>
        </div>
    </header>
    ...
    <div class="section-divider">
        ...
        <a href="/sk/books" class="text-xs font-black text-green-600 bg-green-50 border border-green-200 px-3 py-1.5 rounded-xl hover:bg-green-100 transition whitespace-nowrap flex-shrink-0">Browse All →</a>
    </div>
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="library-banner">
                <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <div class="bg-white/10 rounded-xl px-3 py-2.5 text-center"><?php $bpct=$totalBooks>0?round($availableCount/$totalBooks*100):0; ?><p class="text-xl font-black text-white"><?= $bpct ?>%</p><p class="text-[9px] font-black text-green-300 uppercase tracking-wider mt-0.5">In Stock</p></div>
                    </div>
                </div>
            </div>
            ...
            <div class="dash-card p-4">
                ...
                <div class="flex gap-2 mb-3">
                    <input id="ai-query" type="text" placeholder="What are you looking for?" class="flex-1 text-xs border border-slate-200 rounded-xl px-3 py-2.5 outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 font-medium bg-slate-50" onkeydown="if(event.key==='Enter')aiFind()">
                    <button onclick="aiFind()" class="px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold text-xs transition flex-shrink-0"><i class="fa-solid fa-magnifying-glass"></i></button>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    ?>
                        <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-2xl border border-slate-100">
                            ...
                            <span class="inline-flex items-center gap-1 text-[10px] font-black px-2 py-1 rounded-full <?= $overdue?'bg-red-50 text-red-600':($dueSoon?'bg-amber-50 text-amber-600':'bg-green-50 text-green-600') ?>"><?= $overdue?'Overdue':($dueSoon?'Due Soon':'Active') ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="rounded-2xl p-4 border border-emerald-200 bg-emerald-50">
                <div class="flex items-start gap-2.5">
                    <div class="w-8 h-8 bg-emerald-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5"><i class="fa-solid fa-lightbulb text-emerald-600 text-xs"></i></div>
...
async function aiFind(){
    ...
    try{
        ...
        if(d.books?.length){r.innerHTML=d.books.map(b=>`<a href="/sk/books?id=${b.id}" class="flex items-center gap-2 p-2 hover:bg-green-50 rounded-xl transition"><div class="w-6 h-full rounded bg-green-200 flex-shrink-0" style="min-height:28px;background:var(--green)"></div><div class="min-w-0"><p class="text-xs font-bold text-slate-800 truncate">${b.title}</p><p class="text-[10px] text-slate-400 truncate">${b.author||'—'}</p></div></a>`).join('');}
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    (function(){
        ...
        if(be&&le){be.innerHTML='';le.innerHTML='';dowArr.forEach((cnt,i)=>{const bar=document.createElement('div');bar.style.cssText=`flex:1;border-radius:6px 6px 0 0;background:${i===peakDowIdx?'#16a34a':'#bbf7d0'};height:${Math.max(pct(cnt,maxD),4)}%;min-height:4px;`;bar.title=`${DOW[i]}: ${cnt}`;be.appendChild(bar);const lbl=document.createElement('div');lbl.style.cssText=`flex:1;text-align:center;font-size:9px;font-weight:${i===peakDowIdx?'800':'600'};color:${i===peakDowIdx?'#16a34a':'#94a3b8'};`;lbl.textContent=DOW[i];le.appendChild(lbl);});}
        ...
        if(mCtx)new Chart(mCtx,{type:'bar',data:{labels:MONTH,datasets:[{data:monthArr,backgroundColor:monthArr.map((_,i)=>i===peakMonthIdx?'#16a34a':'rgba(22,163,74,0.15)'),borderRadius:8,borderSkipped:false}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'#1e293b',titleFont:{family:'Plus Jakarta Sans',weight:'700'},bodyFont:{family:'Plus Jakarta Sans'},padding:10,cornerRadius:10,callbacks:{label:ctx=>` ${ctx.raw} reservations`}}},scales:{x:{grid:{display:false},ticks:{font:{family:'Plus Jakarta Sans',size:10},color:'#94a3b8'}},y:{grid:{color:'#f1f5f9'},beginAtZero:true,ticks:{font:{family:'Plus Jakarta Sans',size:10},color:'#94a3b8',stepSize:1}}}}});
