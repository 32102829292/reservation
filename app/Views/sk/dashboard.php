...
<body>
    ...
    <script>
        ...
        function openDateModal(date, items) {
            /* FIX: format raw HH:MM:SS → 12-hour AM/PM (Philippine time display) */
            const fmtT = t => {
                if (!t) return '—';
                const [h, m] = t.split(':');
            ...
            <div style="flex:1;min-width:0;">
                ...
                <p style="font-family:var(--mono);font-size:10px;color:var(--text-sub);margin-top:1px;">${fmtT(r.start_time)} – ${fmtT(r.end_time)}</p>
            </div>
