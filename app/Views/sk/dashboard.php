...
$myRes  = array_map($normalizeResName, $myRes);
$sysRes = array_map($normalizeResName, $sysRes);
...
$thirtyDaysAgo      = (clone $nowPHT)->modify('-30 days')->format('Y-m-d');
...
foreach ($sysRes as $r) {
    ...
    if (!empty($r['reservation_date'])) {
        ...
        $d = (int)floor(($nowPHT->getTimestamp() - strtotime($r['reservation_date'])) / 86400);
        if ($d >= 0 && $d < 7)  $ins7++;
...
$insPH  = (int)array_search(max($insHourArr), $insHourArr);
$insPD  = (int)array_search(max($insDowArr),  $insDowArr);
$insPM  = (int)array_search(max($insMonArr),  $insMonArr);
$f12    = fn($h) => (($h % 12) ?: 12) . ' ' . ($h < 12 ? 'AM' : 'PM');
$insPHL = $f12($insPH) . '–' . $f12(min(23, $insPH + 1));
...
$chartData   = [];
for ($i = 6; $i >= 0; $i--) {
    $dayObj        = (clone $nowPHT)->modify("-$i days");
    $d             = $dayObj->format('Y-m-d');
    $chartLabels[] = $dayObj->format('D');
...
foreach ($myRes as $r) {
    ...
    if (($r['status'] ?? '') === 'approved' && !in_array($r['claimed'] ?? false, [true, 1, 't', 'true', '1'], true)) {
        $s = strtotime($r['reservation_date'] . ' ' . $r['start_time']);
        $e = strtotime($r['reservation_date'] . ' ' . $r['end_time']);
        if ($s <= $nowTs && $e >= $nowTs) {
            $activeBanner = $r;
            break;
        }
        if ($s > $nowTs && $s <= $nowTs + 3600) $upcomingBanner = $r;
    }
}
...
<head>
    ...
    <style>
        ...
        .lib-banner { background:linear-gradient(135deg,var(--indigo) 0%,#4338ca 60%,#6366f1 100%) !important; border-radius:var(--r-lg) !important; padding:22px 22px 18px !important; overflow:hidden !important; position:relative !important; box-sizing:border-box !important; width:100% !important; }
        .lib-banner::before { content:'' !important; position:absolute !important; inset:0 !important; background:url("data:image/svg+xml,%3Csvg width='40' height='40' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='20' cy='20' r='18' fill='none' stroke='rgba(255,255,255,.05)' stroke-width='1'/%3E%3C/svg%3E") repeat !important; opacity:.4 !important; pointer-events:none !important; }
        .lib-banner>div { position:relative !important; z-index:1 !important; }

        .lib-stats { display:flex !important; gap:8px !important; flex-wrap:nowrap !important; overflow:hidden !important; margin-top:12px !important; box-sizing:border-box !important; width:100% !important; }
        .lib-stat, .lib-stat-item { flex:1 1 0 !important; min-width:0 !important; box-sizing:border-box !important; overflow:hidden !important; background:rgba(255,255,255,.1) !important; border:1px solid rgba(255,255,255,.1) !important; border-radius:10px !important; padding:7px 10px !important; }
        .lib-stat-lbl { font-size:.52rem !important; font-weight:600 !important; color:rgba(255,255,255,.55) !important; text-transform:uppercase !important; letter-spacing:.06em !important; white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; display:block !important; }
        .lib-stat-val { font-size:.92rem !important; font-weight:800 !important; color:white !important; font-family:var(--mono) !important; line-height:1.2 !important; display:block !important; white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; }

        .lib-browse { display:inline-flex !important; align-items:center !important; gap:7px !important; padding:9px 14px !important; background:rgba(255,255,255,.18) !important; color:white !important; border-radius:9px !important; font-size:.78rem !important; font-weight:700 !important; text-decoration:none !important; border:1px solid rgba(255,255,255,.2) !important; transition:background var(--ease) !important; backdrop-filter:blur(4px) !important; touch-action:manipulation !important; white-space:nowrap !important; }
        .lib-browse:hover { background:rgba(255,255,255,.28) !important; }

        @media(max-width:639px) {
            .grid-lib { grid-template-columns:1fr !important; }
            .lib-banner { padding:14px 14px 12px !important; }
            .lib-stats { gap:5px !important; margin-top:10px !important; }
            .lib-stat, .lib-stat-item { padding:6px 7px !important; border-radius:8px !important; }
            .lib-stat-lbl { font-size:.46rem !important; }
            .lib-stat-val { font-size:.82rem !important; }
            .lib-browse { width:100% !important; justify-content:center !important; padding:9px 12px !important; font-size:.75rem !important; }
            .section-lbl { flex-wrap:wrap; }
            .grid-four { grid-template-columns:repeat(2,minmax(0,1fr)) !important; }
            .heatmap-cell { height:22px; }
            .main-area { padding-bottom:calc(var(--mob-nav-total) + 24px) !important; }
        }
...
<body>
    ...
    <div id="loginToast" class="login-toast">
        <div style="width:28px;height:28px;border-radius:8px;background:rgba(99,102,241,.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <?= sk_icon('bell', 13, '#818cf8') ?>
        </div>
        <div style="flex:1;min-width:0;">
            <p style="font-weight:700;font-size:12px;color:white;">Welcome back, <?= esc($user_name ?? 'Officer') ?>!</p>
            <p style="font-size:10px;color:rgba(255,255,255,.6);margin-top:2px;"><?= $nowPHT->format('l, F j') ?></p>
        </div>
    ...
    <main class="main-area">
            ...
            <div class="timer-banner <?= $isActive ? 'active' : 'upcoming' ?>" id="timerBanner"
                ...
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <div style="flex:1;min-width:140px;">
                        ...
                        <p style="font-size:.72rem;opacity:.7;margin-top:2px;font-family:var(--mono);"><?= date('g:i A', $bStartTs) ?> – <?= date('g:i A', $bEndTs) ?> PHT</p>
                    </div>
        ...
        foreach ($myRes as $r) {
            if (($r['status'] ?? '') === 'approved' && !in_array($r['claimed'] ?? false, [true, 1, 't', 'true', '1'], true)) {
                $startTs = strtotime($r['reservation_date'] . ' ' . ($r['start_time'] ?? '00:00'));
                if ($startTs > $nowTs) {
                    $upcomingForPill = $r;
                    break;
                }
        ...
        if ($upcomingForPill && !$activeBanner): ?>
            <div class="upcoming-pill fade-up-1">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="up-time">
                        <?= date('M j, Y', strtotime($upcomingForPill['reservation_date'])) ?>
                        &nbsp;·&nbsp;
                        <?= date('g:i A', strtotime($upcomingForPill['start_time'])) ?> – <?= date('g:i A', strtotime($upcomingForPill['end_time'])) ?> PHT
                    </div>
                </div>
        ...
        <div class="stats-grid fade-up-2">
            ...
            <div class="stat-card" style="border-left:4px solid #d97706;">
                ...
                <div class="stat-lbl" style="margin-bottom:8px;">Today <span style="font-size:.55rem;opacity:.6;">(PHT)</span></div>
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;text-align:center;">
                    <div>
                        <div style="font-size:1.3rem;font-weight:800;color:#d97706;font-family:var(--mono);"><?= $sysTodayPending ?></div>
                        <div style="font-size:.6rem;color:var(--text-sub);font-weight:700;">Pending</div>
                    </div>
                    <div>
                        <div style="font-size:1.3rem;font-weight:800;color:#16a34a;font-family:var(--mono);"><?= $sysTodayApproved ?></div>
                        <div style="font-size:.6rem;color:var(--text-sub);font-weight:700;">Approved</div>
                    </div>
                    <div>
                        <div style="font-size:1.3rem;font-weight:800;color:#7c3aed;font-family:var(--mono);"><?= $sysTodayClaimed ?></div>
                ...
                <div class="card card-p" style="flex:1;">
                    ...
                    <?php if (!empty($recentAll)): ?>
                        <?php foreach ($recentAll as $r):
                            ...
                            $isExpired = !empty($r['reservation_date']) && strtotime($r['reservation_date']) < strtotime($sysToday) && !$isCl;
                            $dt = new DateTime($r['reservation_date'] ?? $sysToday, $pht);
                        ?>
                        ...
                        <div style="display:flex;flex-direction:column;gap:2px;">
                            ...
                            ?>
                                <a href="/sk/books" style="display:flex;align-items:center;gap:10px;padding:7px 6px;border-radius:10px;text-decoration:none;color:inherit;transition:background .15s;">
                                    <div class="book-letter"><?= mb_strtoupper(mb_substr($book['title'] ?? 'B', 0, 1)) ?></div>
                                    <div style="flex:1;min-width:0;">
                                ...
                                <div style="display:flex;align-items:center;gap:10px;background:var(--input-bg);border-radius:10px;padding:9px 12px;border:1px solid var(--border-subtle);">
                                    ...
                                    <div style="flex:1;min-width:0;">
                                        ...
                                        <p style="font-size:.68rem;color:<?= $overdue ? '#ef4444' : ($dueSoon ? '#d97706' : 'var(--text-sub)') ?>;font-family:var(--mono);">
                                            <?= $dueTs ? (($overdue ? 'Overdue · ' : ($dueSoon ? 'Due soon · ' : '')) . date('M j, Y', $dueTs)) : 'No due date' ?>
                                        </p>
                                    </div>
        ...
        <div class="grid-four fade-up-4">
            <div class="insight-card" data-emoji="⏰">
                ...
                <div class="prog-bar" style="margin-top:10px;">
                    <div class="prog-fill" style="width:<?= max($insHourArr) > 0 ? min(100, round($insHourArr[$insPH] / max($insHourArr) * 100)) : 0 ?>%;background:#f59e0b;"></div>
                </div>
            </div>
            <div class="insight-card" data-emoji="📅">
    ...
    <script>
        ...
        const allResData = <?= json_encode($sysRes, JSON_HEX_TAG | JSON_HEX_APOS | JSON_UNESCAPED_UNICODE) ?>;
        const INS = {
            ...
            (function() {
                ...
                if (hm) {
                    ...
                    hourArr.forEach((cnt, h) => {
                        ...
                        cell.style.cssText = `background:rgba(55,48,163,${alpha.toFixed(2)});${h===peakHourIdx?'box-shadow:0 0 0 2px #3730a3;':''}`;
