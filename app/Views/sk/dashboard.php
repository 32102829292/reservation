...
<head>
    ...
    <style>
        ...
        .timer-banner.upcoming {
            background: var(--indigo-light);
            border-color: var(--indigo-border);
            color: #312e81;
        }

        body.dark .timer-banner.active {
            background: rgba(20, 83, 45, .25);
            border-color: rgba(134, 239, 172, .2);
            color: #86efac;
        }

        body.dark .timer-banner.upcoming {
        ...
        .session-card.s-warning {
            border-left-color: #f59e0b;
        }

        .session-card.s-critical {
            border-left-color: #ef4444;
        }

        .session-card.s-ended {
        ...
        .s-ok .session-countdown {
            background: #dcfce7;
            color: #166534;
        }

        .s-warning .session-countdown {
            background: #fef3c7;
            color: #92400e;
        }

        .s-critical .session-countdown {
            background: #fee2e2;
            color: #991b1b;
        }

        .s-ended .session-countdown {
        ...
        .s-ok .session-prog-fill {
            background: #10b981;
        }

        .s-warning .session-prog-fill {
            background: #f59e0b;
        }

        .s-critical .session-prog-fill {
            background: #ef4444;
        }

        .s-ended .session-prog-fill {
            background: #94a3b8;
        }
        ...
        .lib-banner {
            background: linear-gradient(135deg, var(--indigo) 0%, #4338ca 60%, #6366f1 100%) !important;
        ...
        .lib-banner::before {
            ...
            inset: 0 !important;
            background: url("data:image/svg+xml,%3Csvg width='40' height='40' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='20' cy='20' r='18' fill='none' stroke='rgba(255,255,255,.05)' stroke-width='1'/%3E%3C/svg%3E") repeat !important;
            opacity: .4 !important;
        ...
        <div class="topbar fade-up">
            <div>
                <div class="greeting-eyebrow"><?php $hh = (int)date('H');
                                                echo $hh < 12 ? 'Good morning' : ($hh < 17 ? 'Good afternoon' : 'Good evening'); ?></div>
        ...
        <div class="stats-grid fade-up-2">
            ...
            <div class="stat-card" style="border-left:4px solid #16a34a;">
                ...
                <div class="prog-bar" style="margin-top:8px;">
                    <div class="prog-fill" style="width:<?= $sysApprovalRate ?>%;background:#16a34a;"></div>
                </div>
            ...
            <div class="stat-card" style="border-left:4px solid #7c3aed;">
                ...
                <div class="prog-bar" style="margin-top:8px;">
                    <div class="prog-fill" style="width:<?= $sysUtilizationRate ?>%;background:#7c3aed;"></div>
                </div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:12px;">
                        <?php foreach (
                            ...
                            <div>
                                ...
                                <div class="prog-bar">
                                    <div class="prog-fill" style="width:<?= $val ?>%;background:<?= $clr ?>;"></div>
                                </div>
    ...
    <script>
        ...
        (function() {
            ...
            function tick() {
                ...
                if (!diff) {
                    disp.textContent = isActive ? 'Ended' : 'Now!';
                    return;
                }
        ...
        function renderSessions() {
            ...
            sessions.forEach(r => {
                ...
                if (!sessionState[r.id]) sessionState[r.id] = {
                    warned: false,
                    expired: false
                };
                const st = sessionState[r.id];
                if (!st.warned && remMs > 0 && remMs <= TL_WARN) {
                    st.warned = true;
                    tlToast('warning', `${name} — 5 min left`, `${res} ending soon`);
                }
                if (!st.expired && remMs <= 0) {
                    st.expired = true;
        ...
        function getChartColors() {
            const dark = document.body.classList.contains('dark');
            return {
                grid: dark ? '#101e35' : '#f1f5f9',
                tick: dark ? '#4a6fa5' : '#94a3b8'
            };
        }
