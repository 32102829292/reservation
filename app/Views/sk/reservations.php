...
<head>
    ...
    <style>
        ...
        .stat-card { background: var(--card); border: 1px solid rgba(99,102,241,.08); border-radius: var(--r-lg); padding: 16px 18px; border-left-width: 4px; box-shadow: var(--shadow-sm); transition: transform var(--ease), box-shadow var(--ease); cursor: pointer; }
        .stat-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); }
        .stat-card.ring { box-shadow: 0 0 0 2px var(--indigo); }

        .qtab { display: inline-flex; align-items: center; gap: 6px; padding: .4rem .9rem; border-radius: var(--r-sm); font-size: .78rem; font-weight: 700; transition: all var(--ease); cursor: pointer; border: 1px solid #e2e8f0; white-space: nowrap; color: #64748b; background: white; }
        .qtab:hover { border-color: var(--indigo); color: var(--indigo); }
        .qtab.active { background: var(--indigo); color: white; border-color: var(--indigo); box-shadow: 0 4px 12px -2px rgba(55,48,163,.3); }

        .search-field { background: white; border: 1px solid rgba(99,102,241,.12); border-radius: var(--r-sm); padding: .7rem 1rem .7rem 2.5rem; font-size: .875rem; font-family: var(--font); color: #0f172a; transition: all .2s; width: 100%; outline: none; }
        .search-field:focus { border-color: #818cf8; box-shadow: 0 0 0 3px rgba(99,102,241,.08); }

        .table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }
        table { width: 100%; border-collapse: collapse; min-width: 720px; }
        thead th { background: #f8fafc; font-weight: 800; text-transform: uppercase; font-size: .6rem; letter-spacing: .12em; color: #94a3b8; padding: .85rem 1rem; border-bottom: 1px solid #e2e8f0; white-space: nowrap; cursor: pointer; user-select: none; }
        thead th:hover { color: var(--indigo); }
        thead th .sort-icon { opacity: .35; margin-left: 4px; font-size: .6rem; }
        ...
        .res-card { background: var(--card); border-radius: var(--r-lg); border: 1px solid rgba(99,102,241,.08); padding: 1rem 1.1rem; cursor: pointer; transition: all .18s; position: relative; overflow: hidden; box-shadow: var(--shadow-sm); }
        ...
        .res-card[data-status="approved"]::before { background: #6366f1; }
        ...
        .badge { display: inline-flex; align-items: center; gap: 4px; padding: .28rem .7rem; border-radius: 9px; font-size: .62rem; font-weight: 800; text-transform: uppercase; letter-spacing: .05em; white-space: nowrap; }
        ...
        .btn-approve { background: #e0e7ff; color: #3730a3; border: none; border-radius: 9px; padding: .45rem .85rem; font-size: .72rem; font-weight: 800; cursor: pointer; transition: all .18s; display: inline-flex; align-items: center; gap: 5px; font-family: var(--font); }
        .btn-approve:hover { background: var(--indigo); color: white; }
        .btn-decline { background: #fee2e2; color: #991b1b; border: none; border-radius: 9px; padding: .45rem .6rem; font-size: .72rem; font-weight: 800; cursor: pointer; transition: all .18s; display: inline-flex; align-items: center; font-family: var(--font); }
        .btn-decline:hover { background: #ef4444; color: white; }

        .print-pill-yes { display: inline-flex; align-items: center; gap: 4px; padding: .18rem .55rem; border-radius: 999px; font-size: .62rem; font-weight: 800; background: #e0e7ff; color: #3730a3; white-space: nowrap; }
        .print-pill-no { display: inline-flex; align-items: center; gap: 4px; padding: .18rem .55rem; border-radius: 999px; font-size: .62rem; font-weight: 800; background: #f1f5f9; color: #64748b; white-space: nowrap; }
        ...
        .modal-box { position: relative; margin: auto; background: white; border-radius: var(--r-xl); width: 94%; max-width: 520px; max-height: 92vh; overflow-y: auto; box-shadow: var(--shadow-lg); animation: popIn .22s cubic-bezier(.34,1.56,.64,1) both; }
        .modal-box.sm { max-width: 380px; }
        .sheet-handle { display: none; width: 40px; height: 4px; background: #e2e8f0; border-radius: 999px; margin: 10px auto 0; }
        ...
        .drow { display: flex; align-items: flex-start; gap: 12px; padding: .7rem 0; border-bottom: 1px solid #f1f5f9; }
        .drow:last-child { border-bottom: none; }
        .dicon { width: 34px; height: 34px; border-radius: 10px; background: var(--indigo-light); color: var(--indigo); display: flex; align-items: center; justify-content: center; font-size: .8rem; flex-shrink: 0; }
        .dlabel { font-size: .6rem; font-weight: 800; text-transform: uppercase; letter-spacing: .08em; color: #94a3b8; margin-bottom: 3px; }
        .dvalue { font-size: .88rem; font-weight: 700; color: #0f172a; }

        .btn-confirm-approve { background: var(--indigo); color: white; border: none; border-radius: var(--r-sm); padding: .85rem; font-size: .875rem; font-weight: 800; cursor: pointer; transition: all .18s; display: flex; align-items: center; justify-content: center; gap: 7px; font-family: var(--font); flex: 1; }
        .btn-confirm-approve:hover:not(:disabled) { background: #312e81; }
        .btn-confirm-decline { background: #ef4444; color: white; border: none; border-radius: var(--r-sm); padding: .85rem; font-size: .875rem; font-weight: 800; cursor: pointer; transition: all .18s; display: flex; align-items: center; justify-content: center; gap: 7px; font-family: var(--font); flex: 1; }
        .btn-confirm-decline:hover:not(:disabled) { background: #dc2626; }
        .btn-cancel { background: #f1f5f9; color: #475569; border: none; border-radius: var(--r-sm); padding: .85rem; font-size: .875rem; font-weight: 800; cursor: pointer; transition: all .18s; display: flex; align-items: center; justify-content: center; gap: 7px; font-family: var(--font); flex: 1; }
        .btn-cancel:hover { background: #e2e8f0; }

        .ticket-section { border: 2px dashed var(--indigo-border); border-radius: var(--r-lg); padding: 1.5rem; background: var(--indigo-light); display: flex; flex-direction: column; align-items: center; }
        .unclaimed-banner { background: #fff7ed; border: 1.5px dashed #fdba74; border-radius: var(--r-md); padding: .75rem 1rem; display: flex; align-items: center; gap: 10px; margin: 0 1.75rem 1rem; }
        .unclaimed-banner .ub-icon { width: 32px; height: 32px; background: #fed7aa; border-radius: 9px; display: flex; align-items: center; justify-content: center; color: #c2410c; font-size: .8rem; flex-shrink: 0; }

        #dPrintLogForm { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--r-lg); padding: 1rem 1.25rem; margin: 0 1.75rem 1rem; }
        #dPrintLogForm label { font-size: .6rem; font-weight: 800; text-transform: uppercase; letter-spacing: .08em; color: #94a3b8; display: block; margin-bottom: 6px; }
        #dPrintLogForm input[type=number] { width: 100%; border: 1px solid #e2e8f0; border-radius: 9px; padding: .55rem .8rem; font-size: .875rem; font-family: var(--font); color: #0f172a; }
        #dPrintLogForm input[type=number]:focus { outline: none; border-color: #818cf8; box-shadow: 0 0 0 3px rgba(99,102,241,.08); }
        .btn-save-print { background: var(--indigo); color: white; border: none; border-radius: 10px; padding: .6rem 1.1rem; font-size: .78rem; font-weight: 800; cursor: pointer; transition: all .18s; display: flex; align-items: center; gap: 6px; font-family: var(--font); white-space: nowrap; }
        .btn-save-print:hover:not(:disabled) { background: #312e81; }
        .btn-save-print:disabled { opacity: .6; cursor: not-allowed; }
        #printSaveMsg { font-size: .72rem; font-weight: 700; margin-top: 6px; min-height: 18px; }

        .empty-state { padding: 5rem 2rem; text-align: center; }
        .card-empty { padding: 3rem 1.5rem; text-align: center; background: var(--card); border-radius: var(--r-lg); border: 1px dashed #e2e8f0; }

        @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: none; } }
        .fade-up { animation: fadeUp .35s ease both; }

        #autoRefreshIndicator { position: fixed; bottom: calc(90px + env(safe-area-inset-bottom, 16px)); right: 16px; background: rgba(15,23,42,.88); backdrop-filter: blur(8px); color: white; font-family: var(--font); font-size: .68rem; font-weight: 700; padding: 6px 12px; border-radius: 999px; z-index: 90; display: flex; align-items: center; gap: 6px; box-shadow: var(--shadow-md); cursor: pointer; }

        @media (min-width: 768px) { #mobileCardList { display: none !important; } #mobileEmpty { display: none !important; } }
        @media (max-width: 767px) { #desktopTableWrap { display: none !important; } }
        ...
        body.dark .search-field { background: #101e35 !important; border-color: rgba(99,102,241,.18) !important; color: #e2eaf8 !important; }
        body.dark thead th { background: #0f1a2e !important; color: #4a6fa5 !important; }
        ...
        body.dark .badge-declined, body.dark .badge-canceled { background: rgba(239,68,68,.15) !important; color: #f87171 !important; }
...
<div id="detailModal" class="overlay" role="dialog" aria-modal="true">
    ...
    <div class="modal-box">
        ...
        <div class="px-7 pb-2">
            <div class="drow"><div class="dicon"><i class="fa-solid fa-user"></i></div><div><p class="dlabel">Requestor</p><p id="dName" class="dvalue"></p><p id="dEmail" class="text-xs text-slate-400 font-semibold mt-0.5"></p></div></div>
            ...
            <div class="drow" id="dApprovedByRow" style="display:none">
                ...
                <div><p class="dlabel" id="dApprovedByLabel">Approved By</p><p id="dApprovedByName" class="dvalue"></p><p id="dApprovedByEmail" class="text-xs text-slate-400 font-semibold mt-0.5"></p><p id="dApprovedAt" class="text-xs text-slate-400 font-semibold mt-0.5"></p></div>
            </div>
...
<main class="main-area">
            ...
            <table id="resTable">
                ...
                <tbody id="tableBody">
                    <?php if (empty($processed)): ?>
                        <tr><td colspan="9"><div class="empty-state"><i class="fa-solid fa-calendar-xmark" style="font-size:3rem;color:#e2e8f0;display:block;margin-bottom:14px"></i><p style="font-weight:800;color:#94a3b8;font-size:1rem">No reservations yet</p></div></td></tr>
                    <?php else: ?>
                        ...
                        ?>
                            <tr class="res-row" data-id="<?= $res['id'] ?>" data-status="<?= $s ?>" data-unclaimed="<?= $isUnclaimed?'1':'0' ?>" data-search="<?= strtolower("$name $resource $purpose $email $approverName") ?>" data-date="<?= $rawDate ?>" data-pl-printed="<?= $plPrinted===null?'':($plPrinted?'Yes':'No') ?>" data-pl-pages="<?= $plPrinted?$plPages:'' ?>" data-pl-at="<?= htmlspecialchars($plAt,ENT_QUOTES) ?>" onclick='openDetail(<?= htmlspecialchars($mdata,ENT_QUOTES) ?>)'>
                                ...
                                <td><p style="font-weight:700;font-size:.85rem;color:#0f172a;line-height:1.3"><?= $name ?></p><?php if ($email): ?><p style="font-size:.7rem;color:#94a3b8;margin-top:2px"><?= $email ?></p><?php endif; ?></td>
                                <td><p style="font-weight:700;font-size:.85rem;color:#0f172a;line-height:1.3"><?= $resource ?></p><?php if ($pc): ?><p style="font-size:.7rem;color:#94a3b8;margin-top:2px"><i class="fa-solid fa-desktop" style="font-size:.65rem"></i> <?= $pc ?></p><?php endif; ?></td>
                                <td><p style="font-size:.85rem;font-weight:700;color:#0f172a"><?= $date ?></p><p style="font-size:.7rem;color:var(--indigo);font-weight:600;margin-top:2px"><?= $start ?> – <?= $end ?></p></td>
                                ...
                                <td onclick="event.stopPropagation()">
                                    <?php if ($approverName && in_array($s, ['approved','claimed','declined','expired','unclaimed'])): ?>
                                        <div style="display:flex;align-items:center;gap:7px">
                                            ...
                                            <div style="min-width:0"><p style="font-size:.78rem;font-weight:700;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100px"><?= $approverName ?></p><?php if ($approvedAt): ?><p style="font-size:.65rem;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= $approvedAt ?></p><?php endif; ?></div>
                                        </div>
    ...
    <div id="mobileCardList" style="display:flex;flex-direction:column;gap:10px">
        <?php if (empty($processed)): ?>
            <div class="card-empty"><i class="fa-solid fa-calendar-xmark" style="font-size:2.5rem;color:#e2e8f0;display:block;margin-bottom:10px"></i><p style="font-weight:800;color:#94a3b8">No reservations yet</p></div>
        <?php else: ?>
            <?php foreach ($processed as $res):
                $s = $res['_status'];
                $isUnclaimed = $res['_unclaimed'];
            ...
            ?>
                <div class="res-card" data-id="<?= $res['id'] ?>" data-status="<?= $s ?>" data-unclaimed="<?= $isUnclaimed?'1':'0' ?>" data-search="<?= strtolower("$name $resource $purpose $email $approverName") ?>" data-date="<?= $rawDate ?>" data-pl-printed="<?= $plPrinted===null?'':($plPrinted?'Yes':'No') ?>" data-pl-pages="<?= $plPrinted?$plPages:'' ?>" data-pl-at="<?= htmlspecialchars($plAt,ENT_QUOTES) ?>" onclick='openDetail(<?= htmlspecialchars($mdata,ENT_QUOTES) ?>)'>
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
                        ...
                        <div style="flex:1;min-width:0"><p style="font-weight:700;font-size:.85rem;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= $name ?></p><?php if ($email): ?><p style="font-size:.7rem;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= $email ?></p><?php endif; ?></div>
...
<script>
    ...
    function refreshCsrf(data) {
        if (data.csrf_hash && data.csrf_token) { csrfToken = data.csrf_hash; csrfName = data.csrf_token; document.querySelector('meta[name="csrf-token"]')?.setAttribute('content', csrfToken); document.querySelector('meta[name="csrf-name"]')?.setAttribute('content', csrfName); }
    }

    const printLogMap = {};
    <?php foreach ($printLogMap as $resId => $pl): ?>
        printLogMap[<?= (int)$resId ?>] = { printed: <?= isset($pl['printed'])?(in_array($pl['printed'],[true,1,'t','true','1'],true)?'true':'false'):'false' ?>, pages: <?= (int)($pl['pages']??0) ?>, at: "<?= !empty($pl['printed_at'])?date('M j · g:i A',strtotime($pl['printed_at'])):'' ?>" };
    <?php endforeach; ?>
    ...
    async function savePrintLog() {
        ...
        btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin text-xs"></i> Saving…'; msg.textContent = ''; msg.style.color = '';
        ...
        try {
            ...
            if (data.ok) {
                ...
                msg.textContent = pages > 0 ? `✓ Saved — ${pages} page${pages!==1?'s':''} printed` : '✓ Saved — no printing logged'; msg.style.color = '#3730a3';
    ...
    function refreshPrintLogStrip(rid) {
        ...
        if (plog.printed) { logText.textContent = `Printed ${plog.pages} page${plog.pages!==1?'s':''}` + (plog.at ? ` · ${plog.at}` : ''); logBadge.textContent = `${plog.pages}pg`; logBadge.className = 'text-[10px] font-black px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700'; }
    ...
    function refreshBothPrintCells(rid, pages) {
        allTableRows.forEach(row => { if (row.dataset.id == rid) { row.cells[7].innerHTML = pages > 0 ? `<span class="print-pill-yes"><i class="fa-solid fa-print" style="font-size:.6rem"></i> ${pages}pg</span>` : `<span class="print-pill-no"><i class="fa-solid fa-xmark" style="font-size:.6rem"></i> No print</span>`; row.dataset.plPrinted = pages > 0 ? 'Yes' : 'No'; row.dataset.plPages = pages > 0 ? pages : ''; } });
    ...
    function exportCSV() {
        ...
        a.href = url; a.download = `sk-reservations-${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
    }
    ...
    function applyFilters() {
        ...
        const matchesFilters = el => { let matchTab = curTab==='all'?true:curTab==='declined'?['declined','canceled'].includes(el.dataset.status):el.dataset.status===curTab; return matchTab && (!q || el.dataset.search.includes(q)) && (!date || el.dataset.date === date); };
        let n = 0; allTableRows.forEach(row => { const show = matchesFilters(row); row.style.display = show?'':'none'; if(show) n++; });
        let cv = 0; allCards.forEach(card => { const show = matchesFilters(card); card.style.display = show?'':'none'; if(show) cv++; });
    ...
    const STATUS_META = {
        ...
        approved: {icon:'fa-circle-check',bg:'#eef2ff', color:'#3730a3',label:'Approved'},
    ...
    function openDetail(d) {
        ...
        document.getElementById('dName').textContent = d.name; document.getElementById('dEmail').textContent = d.email;
        document.getElementById('dResource').textContent = d.resource; document.getElementById('dPc').textContent = d.pc ? 'PC: ' + d.pc : '';
        document.getElementById('dDate').textContent = d.date; document.getElementById('dTime').textContent = d.start + ' – ' + d.end;
        document.getElementById('dPurpose').textContent = d.purpose; document.getElementById('dType').textContent = d.type;
    ...
    function downloadTicket() { const canvas = document.getElementById('qrCanvas'), code = document.getElementById('dTicketCode').textContent, link = document.createElement('a'); link.download = `E-Ticket-${code}.png`; link.href = canvas.toDataURL('image/png'); link.click(); }
    ...
    function closeModal(key) { const el = document.getElementById(modalIds[key]); if (el) { el.classList.remove('open'); document.body.style.overflow = ''; } if (key==='detail') _currentReservationId = null; if (key==='approve') { const b=document.getElementById('confirmApproveBtn'); b.disabled=false; b.innerHTML='<i class="fa-solid fa-check"></i> Approve'; } if (key==='decline') { const b=document.getElementById('confirmDeclineBtn'); b.disabled=false; b.innerHTML='<i class="fa-solid fa-xmark"></i> Decline'; } }
    document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeModal('detail'); closeModal('approve'); closeModal('decline'); } });

    function fixStatGrid() { const g = document.getElementById('statGrid'); if (!g) return; g.style.gridTemplateColumns = window.innerWidth < 640 ? 'repeat(3,minmax(0,1fr))' : window.innerWidth < 900 ? 'repeat(3,minmax(0,1fr))' : 'repeat(6,minmax(0,1fr))'; }
    fixStatGrid(); window.addEventListener('resize', fixStatGrid);
    applyFilters();
    ...
    function updateCountdown() { const el = document.getElementById('refreshCountdown'), dot = document.getElementById('refreshDot'); if (!el) return; if (refreshPaused) { el.textContent = 'Refresh paused'; dot.style.background = '#f59e0b'; } else { el.textContent = `Refresh in ${secondsLeft}s`; dot.style.background = '#6366f1'; } }
    ...
    async function doAutoRefresh(force = false) {
        ...
        try {
            ...
            applyFilters(); secondsLeft = AUTO_REFRESH_INTERVAL; updateCountdown(); if (dot) dot.style.background = '#6366f1';
        } catch (err) { console.warn('Auto-refresh failed:', err.message); const dot = document.getElementById('refreshDot'); if (dot) { dot.style.background = '#ef4444'; setTimeout(() => { dot.style.background = '#6366f1'; }, 3000); } }
    }
