...
foreach (($reservations ?? []) as $res) {
    ...
    elseif ($s === 'approved') { $edt = strtotime(($res['reservation_date']??'').' '.($res['end_time']??'23:59')); if ($edt && $edt < time()) $s = 'unclaimed'; }
    elseif ($s === 'pending')  { $rdt = strtotime($res['reservation_date']??''); if ($rdt && $rdt < strtotime('today')) $s = 'expired'; }
    $res['_status'] = $s; $res['_unclaimed'] = ($s === 'unclaimed'); $processed[] = $res;
}
$counts = [
    'all'       => count($processed),
...
<div id="detailModal" class="overlay" role="dialog" aria-modal="true">
    ...
    <div class="modal-box">
        ...
        <div class="flex items-start justify-between px-7 pt-5 pb-3">
            <div><p id="dId" class="text-xs font-black text-slate-400 font-mono mb-1"></p><h3 class="text-xl font-black text-slate-900">Reservation Details</h3></div>
        ...
        <div id="dUnclaimedBanner" class="unclaimed-banner" style="display:none">
            ...
            <div><p class="font-black text-sm text-orange-700">Not Yet Claimed</p><p class="text-xs text-orange-500 font-medium mt-0.5">Approved but the e-ticket was never scanned.</p></div>
        </div>
        <div class="px-7 pb-2">
        ...
        <div id="dQr" class="mx-7 mb-4 ticket-section" style="display:none">
            ...
            <button onclick="downloadTicket()" class="mt-3 flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded-xl font-bold text-sm hover:bg-green-700 transition"><i class="fa-solid fa-download text-xs"></i> Download E-Ticket</button>
        </div>
        ...
        <div id="dPrintLog" class="mx-7 mb-3 rounded-2xl px-4 py-3 border border-slate-100 bg-slate-50 flex items-center gap-3" style="display:none">
            <div class="w-9 h-9 bg-green-50 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-print text-green-600 text-sm"></i></div>
        ...
        <div id="dPrintLogForm">
            <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2"><i class="fa-solid fa-print text-green-500"></i> Log Print for this Reservation</p>
...
<main class="flex-1 min-w-0 p-4 lg:p-10 pb-32">
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 fade-up">
        <div>
            ...
            <p class="text-slate-400 font-medium text-sm mt-0.5">
                ...
                <?php if ($counts['unclaimed'] > 0): ?> · <span class="text-orange-500 font-bold"><?= $counts['unclaimed'] ?> unclaimed</span><?php endif; ?>
            </p>
        </div>
    ...
    <?php if (session()->getFlashdata('success')): ?><div class="mb-5 px-5 py-4 bg-green-50 border border-green-200 text-green-700 font-bold rounded-2xl flex items-center gap-3 text-sm fade-up"><i class="fa-solid fa-circle-check text-green-500"></i><?= session()->getFlashdata('success') ?></div><?php endif; ?>
    <?php if (session()->getFlashdata('error')): ?><div class="mb-5 px-5 py-4 bg-red-50 border border-red-200 text-red-700 font-bold rounded-2xl flex items-center gap-3 text-sm fade-up"><i class="fa-solid fa-circle-exclamation text-red-500"></i><?= session()->getFlashdata('error') ?></div><?php endif; ?>
    ...
    <div class="bg-white border border-slate-200 rounded-[28px] p-4 lg:p-5 mb-4 shadow-sm">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <button onclick="clearFilters()" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-bold text-sm transition flex items-center gap-2 flex-shrink-0"><i class="fa-solid fa-rotate-left text-xs"></i> Reset</button>
        </div>
                ...
                <tbody id="tableBody">
                    ...
                    <?php else: ?>
                        <?php foreach ($processed as $res):
                            $s=$res['_status']; $isUnclaimed=$res['_unclaimed'];
                        ...
                        <tr class="res-row" data-id="<?= $res['id'] ?>" data-status="<?= $s ?>" data-unclaimed="<?= $isUnclaimed?'1':'0' ?>" data-search="<?= strtolower("$name $resource $purpose $email $approverName") ?>" data-date="<?= $rawDate ?>" data-pl-printed="<?= $plPrinted===null?'':($plPrinted?'Yes':'No') ?>" data-pl-pages="<?= $plPrinted?$plPages:'' ?>" data-pl-at="<?= htmlspecialchars($plAt,ENT_QUOTES) ?>" onclick='openDetail(<?= htmlspecialchars($mdata,ENT_QUOTES) ?>)'>
                            ...
                            <td><p class="font-bold text-sm text-slate-800 leading-tight"><?= $name ?></p><?php if($email):?><p class="text-[11px] text-slate-400 mt-0.5 truncate max-w-[160px]"><?= $email ?></p><?php endif;?></td>
                            <td><p class="font-bold text-sm text-slate-800 leading-tight"><?= $resource ?></p><?php if($pc):?><div class="flex items-center gap-1 mt-0.5"><i class="fa-solid fa-desktop text-[9px] text-slate-400"></i><span class="text-[11px] text-slate-500 font-semibold"><?= $pc ?></span></div><?php endif;?></td>
                            <td><p class="text-sm font-bold text-slate-700"><?= $date ?></p><p class="text-[11px] text-green-500 font-semibold mt-0.5"><?= $start ?> – <?= $end ?></p></td>
                            ...
                            <td onclick="event.stopPropagation()">
                                <?php if($approverName&&in_array($s,['approved','claimed','declined','expired','unclaimed'])):?>
                                    <div class="flex items-center gap-1.5"><div class="w-6 h-6 rounded-lg flex items-center justify-center text-[9px] font-black flex-shrink-0 <?= $s==='declined'?'bg-red-100 text-red-600':'bg-emerald-100 text-emerald-700' ?>"><?= mb_strtoupper(mb_substr($approverName,0,1)) ?></div><div class="min-w-0"><p class="text-xs font-bold text-slate-700 truncate max-w-[110px]"><?= $approverName ?></p><?php if($approvedAt):?><p class="text-[10px] text-slate-400 font-medium truncate"><?= $approvedAt ?></p><?php endif;?></div></div>
                            ...
                            <td class="text-right" onclick="event.stopPropagation()">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <?php elseif($s==='unclaimed'):?><span class="text-[11px] text-orange-500 font-black flex items-center gap-1"><i class="fa-solid fa-ticket"></i> Unclaimed</span>
    ...
    <div id="mobileCardList" class="md:hidden space-y-3">
        <?php if(empty($processed)):?>
            <div class="card-empty"><i class="fa-solid fa-calendar-xmark text-4xl text-slate-200 mb-3 block"></i><p class="font-black text-slate-400">No reservations yet</p></div>
        <?php else:?>
            <?php foreach($processed as $res):
                $s=$res['_status'];$isUnclaimed=$res['_unclaimed'];
                $name=htmlspecialchars($res['visitor_name']??$res['full_name']??'Guest');
                ...
                $isClaimed=in_array($res['claimed']??false,[true,1,'t','true','1'],true);
                $mdata=json_encode(['id'=>$res['id'],'status'=>$s,'name'=>$name,'email'=>$email,'resource'=>$resource,'pc'=>$pc,'date'=>$date,'rawDate'=>$rawDate,'start'=>$start,'end'=>$end,'purpose'=>$purpose,'type'=>$type,'created'=>$created,'code'=>$code,'claimed'=>$isClaimed,'unclaimed'=>$isUnclaimed,'approverName'=>$approverName,'approverEmail'=>$approverEmail,'approvedAt'=>$approvedAt,'plPrinted'=>$plPrinted,'plPages'=>$plPages,'plAt'=>$plAt]);
                $avatarBg=['pending'=>'bg-amber-100 text-amber-700','approved'=>'bg-emerald-100 text-emerald-700','claimed'=>'bg-purple-100 text-purple-700','declined'=>'bg-red-100 text-red-600','canceled'=>'bg-red-100 text-red-600','expired'=>'bg-slate-100 text-slate-500','unclaimed'=>'bg-orange-100 text-orange-700'][$s]??'bg-slate-100 text-slate-500';
            ?>
                <div class="res-card" data-id="<?= $res['id'] ?>" data-status="<?= $s ?>" data-unclaimed="<?= $isUnclaimed?'1':'0' ?>" data-search="<?= strtolower("$name $resource $purpose $email $approverName") ?>" data-date="<?= $rawDate ?>" data-pl-printed="<?= $plPrinted===null?'':($plPrinted?'Yes':'No') ?>" data-pl-pages="<?= $plPrinted?$plPages:'' ?>" data-pl-at="<?= htmlspecialchars($plAt,ENT_QUOTES) ?>" onclick='openDetail(<?= htmlspecialchars($mdata,ENT_QUOTES) ?>)'>
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-2xl <?= $avatarBg ?> flex items-center justify-center font-black text-sm flex-shrink-0"><?= mb_strtoupper(mb_substr(strip_tags($name),0,1)) ?></div>
                        <div class="flex-1 min-w-0"><p class="font-bold text-sm text-slate-800 truncate leading-tight"><?= $name ?></p><?php if($email):?><p class="text-[11px] text-slate-400 truncate"><?= $email ?></p><?php endif;?></div>
                        <span class="badge badge-<?= $s ?> flex-shrink-0"><i class="fa-solid <?= $icon ?> text-[9px]"></i><?= ucfirst($s) ?></span>
                    </div>
                    <div class="flex items-start gap-2 mb-2">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-1.5 mb-1"><i class="fa-solid fa-desktop text-[10px] text-slate-400 flex-shrink-0"></i><p class="text-xs font-bold text-slate-700 truncate"><?= $resource ?><?= $pc?' · '.$pc:'' ?></p></div>
                            <div class="flex items-center gap-1.5"><i class="fa-regular fa-calendar text-[10px] text-slate-400 flex-shrink-0"></i><p class="text-xs text-slate-500 font-semibold"><?= $date ?></p><span class="text-[10px] text-green-500 font-bold"><?= $start ?> – <?= $end ?></span></div>
                        </div>
                        <div class="card-print-pill flex-shrink-0">
                            <?php if($plPrinted===true):?><span class="print-pill-yes"><i class="fa-solid fa-print text-[9px]"></i> <?= $plPages ?>pg</span>
                            <?php elseif($plPrinted===false):?><span class="print-pill-no"><i class="fa-solid fa-xmark text-[9px]"></i> No print</span><?php endif;?>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between gap-2 pt-2.5 border-t border-slate-100">
                        <div class="flex items-center gap-1.5 min-w-0">
                            ...
                            <?php else:?><p class="text-[10px] text-slate-300 font-semibold">#<?= $res['id'] ?></p><?php endif;?>
                        </div>
                        <?php if($s==='pending'):?>
...
async function savePrintLog() {
    ...
    try {
        ...
        let data; try { data = JSON.parse(text); } catch { throw new Error(`Server error (${res.status})`); }
        if (data.ok) {
            refreshCsrf(data);
            const now = new Date();
...
function refreshBothPrintCells(rid, pages) {
    allTableRows.forEach(row => {
        if (row.dataset.id == rid) {
            ...
            cell.innerHTML = pages > 0 ? `<span class="print-pill-yes"><i class="fa-solid fa-print text-[9px]"></i> ${pages}pg</span>` : `<span class="print-pill-no"><i class="fa-solid fa-xmark text-[9px]"></i> No print</span>`;
    ...
    allCards.forEach(card => {
        if (card.dataset.id == rid) {
            ...
            if (wrapper) wrapper.innerHTML = pages > 0 ? `<span class="print-pill-yes"><i class="fa-solid fa-print text-[9px]"></i> ${pages}pg</span>` : `<span class="print-pill-no"><i class="fa-solid fa-xmark text-[9px]"></i> No print</span>`;
...
function applyFilters() {
    ...
    let cardVisible=0; allCards.forEach(card=>{const show=matchesFilters(card);card.style.display=show?'':'none';if(show)cardVisible++;});
...
function openDetail(d) {
    ...
    saveBtn.disabled = false; saveBtn.innerHTML = '<i class="fa-solid fa-floppy-disk text-xs"></i> Save';
...
function triggerApprove(id,name){approveTargetId=id;document.getElementById('approveConfirmName').textContent=name?`"${name}"`:'';}
function triggerDecline(id,name){declineTargetId=id;document.getElementById('declineConfirmName').textContent=name?`"${name}"`:'';}
...
async function doAutoRefresh(force=false) {
    ...
    try {
        ...
        if(dot){dot.style.background='#ef4444';setTimeout(()=>{dot.style.background='#22c55e';},3000);}
    }
}
