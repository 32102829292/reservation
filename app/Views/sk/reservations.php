...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* ── Mobile nav ── */
        .mobile-nav-pill { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); width: 92%; max-width: 600px; background: rgba(20,83,45,0.98); backdrop-filter: blur(12px); border-radius: 24px; padding: 6px; z-index: 100; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.3); }
...
foreach (($reservations ?? []) as $res) {
    ...
    if (!empty($res['claimed']) && $res['claimed'] == 1) $s = 'claimed';
    if ($s === 'approved') {
...
<main class="flex-1 min-w-0 p-4 lg:p-10 pb-32">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 fade-up">
        <div>
            ...
            <p class="text-slate-400 font-medium text-sm mt-0.5"><?= $counts['all'] ?> total record<?= $counts['all'] != 1 ? 's' : '' ?> in the system</p>
        </div>
    ...
    <!-- ── Stat cards: 1 col on xs, 3 col on sm, 5 col on md ── -->
    <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-3 mb-6">
        <?php foreach ([
                    ...
                    <?php if (empty($processed)): ?>
                        <tr><td colspan="9">
                            <div class="empty-state">
                                ...
                                <p class="text-slate-300 text-sm mt-1">Reservation requests will appear here once submitted.</p>
                            </div>
                        </td></tr>
                    <?php else: ?>
                        <?php foreach ($processed as $res):
                            ...
                            $mdata = json_encode([
                                ...
                                'resource'=>$resource,  'pc'=>$pc,
                                'date'=>$date,          'rawDate'=>$rawDate,
                                'start'=>$start,        'end'=>$end,
                                'purpose'=>$purpose,    'type'=>$type,
                                'created'=>$created,    'code'=>$code,
                                'claimed'=>!empty($res['claimed']) && $res['claimed'] == 1,
                        ...
                        ?>
                            <tr class="res-row"
                                ...
                                data-resource="<?= htmlspecialchars($resource, ENT_QUOTES) ?>"
                                data-date-fmt="<?= $date ?>"
                                data-start="<?= $start ?>"
                                data-end="<?= $end ?>"
                                data-purpose="<?= htmlspecialchars($purpose, ENT_QUOTES) ?>"
                                data-type="<?= htmlspecialchars($type, ENT_QUOTES) ?>"
                                data-approver="<?= htmlspecialchars($approverName, ENT_QUOTES) ?>"
                                ...
                                <td onclick="event.stopPropagation()">
                                    <?php if ($approverName && in_array($s, ['approved','claimed','declined','expired'])): ?>
                                        <div class="flex items-center gap-1.5">
                                            <div class="w-6 h-6 rounded-lg flex items-center justify-center text-[9px] font-black flex-shrink-0 <?= $s === 'declined' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700' ?>">
                                                <?= mb_strtoupper(mb_substr($approverName, 0, 1)) ?>
                                            </div>
                                            <div class="min-w-0">
    ...
    <div id="mobileCardList" class="md:hidden space-y-3">
        ...
        <?php else: ?>
            <?php foreach ($processed as $res):
                ...
                // Avatar initial color per status
                $avatarBg = [
            ...
            ?>
                <div class="res-card"
                    ...
                    <!-- Footer: approver OR action buttons -->
                    <div class="flex items-center justify-between gap-2 pt-2.5 border-t border-slate-100">
                        <div class="flex items-center gap-1.5 min-w-0">
                            <?php if ($approverName && in_array($s, ['approved','claimed','declined','expired'])): ?>
                                <div class="w-5 h-5 rounded-md flex items-center justify-center text-[8px] font-black flex-shrink-0 <?= $s === 'declined' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700' ?>">
                                    <?= mb_strtoupper(mb_substr($approverName, 0, 1)) ?>
                                </div>
                        ...
                        <?php if ($s === 'pending'): ?>
                            <div class="flex items-center gap-1.5 flex-shrink-0" onclick="event.stopPropagation()">
                                <button onclick="triggerApprove(<?= $res['id'] ?>, '<?= addslashes($name) ?>')" class="h-7 px-2.5 rounded-xl bg-green-100 hover:bg-green-600 hover:text-white text-green-700 font-bold text-xs transition flex items-center gap-1">
                                    <i class="fa-solid fa-check text-[10px]"></i> Approve
                                </button>
                                <button onclick="triggerDecline(<?= $res['id'] ?>, '<?= addslashes($name) ?>')" class="h-7 px-2 rounded-xl bg-red-100 hover:bg-red-500 hover:text-white text-red-600 font-bold text-xs transition flex items-center">
    ...
    <!-- Mobile no-results empty state (hidden by default, shown by JS) -->
...
function refreshBothPrintCells(rid, pages) {
    ...
    allCards.forEach(c => {
        if (c.dataset.id == rid) {
            // Find the print pill in resource+schedule row
...
/* ──────────────────────────────────────────
   CSV EXPORT  (uses visible rows in both views)
────────────────────────────────────────── */
function exportCSV() {
    ...
    visibleRows.forEach(row => {
        try {
            ...
            lines.push([d.id ?? '', d.name ?? '', d.email ?? '', d.resource ?? '', d.pc ?? '', d.date ?? '', d.start ?? '', d.end ?? '', d.purpose ?? '', d.type ?? '', d.status ?? '', d.approverName ?? '', d.approvedAt ?? '', row.dataset.plPrinted ?? '', row.dataset.plPages ?? '', row.dataset.plAt ?? '', d.created ?? ''].map(escape).join(','));
        } catch (e) {}
    });
    const blob = new Blob([lines.join('\r\n')], { type: 'text/csv;charset=utf-8;' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `sk-reservations-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
...
function openDetail(d) {
    ...
    // Approved / Declined By row
    const approverRow   = document.getElementById('dApprovedByRow');
    const approverLabel = document.getElementById('dApprovedByLabel');
    const approverIcon  = document.getElementById('dApprovedByIcon');
    if (d.approverName && ['approved','claimed','declined','expired'].includes(d.status)) {
        ...
        approverLabel.textContent = isDeclined ? 'Declined By' : 'Approved By';
        approverIcon.className    = `dicon ${isDeclined ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-600'}`;
        approverIcon.innerHTML    = `<i class="fa-solid ${isDeclined ? 'fa-user-xmark' : 'fa-user-check'}"></i>`;
    ...
    if (d.status === 'pending') {
        acts.innerHTML = `
            ...
            <button onclick="triggerDecline(${d.id},'${d.name.replace(/'/g,"\\'")}');closeModal('detail');" class="btn-confirm-decline flex-1"><i class="fa-solid fa-xmark"></i> Decline</button>`;
    } else {
