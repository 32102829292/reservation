...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-nav { flex: 1; overflow-y: auto; overflow-x: hidden; padding: 10px; scrollbar-width: thin; scrollbar-color: #e2e8f0 transparent; }
        ...
        .sidebar-item { transition: all .18s; display: flex; align-items: center; gap: 14px; padding: 12px 18px; border-radius: 16px; font-weight: 600; font-size: .875rem; text-decoration: none; color: #64748b; }
        .sidebar-item:hover { background: #f0fdf4; color: #16a34a; }
        .sidebar-item.active { background: #16a34a; color: white; box-shadow: 0 8px 20px -4px rgba(22,163,74,.35); }
        ...
        /* ── MODAL ── */
        .modal-backdrop { display: none; position: fixed; inset: 0; background: rgba(15,23,42,.55); backdrop-filter: blur(6px); z-index: 200; padding: 1.5rem; overflow-y: auto; align-items: center; justify-content: center; }
        .modal-backdrop.show { display: flex; animation: fadeIn .15s ease; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .modal-box { background: white; border-radius: 32px; width: 100%; max-width: 460px; padding: 2.5rem; margin: auto; animation: slideUp .2s ease; }
        @keyframes slideUp { from { transform: translateY(16px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }

        /* ── FORM ── */
        .field-input { width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: .875rem 1rem; font-family: 'Plus Jakarta Sans', sans-serif; font-size: .92rem; font-weight: 700; color: #1e293b; transition: all .2s; }
        .field-input:focus { outline: none; border-color: #16a34a; background: white; box-shadow: 0 0 0 4px rgba(22,163,74,.08); }
        .field-label { display: block; font-size: .68rem; font-weight: 800; text-transform: uppercase; letter-spacing: .1em; color: #94a3b8; margin-bottom: .4rem; }
    </style>
</head>
<body>
    ...
    <aside class="sidebar-col">
        <div class="sidebar-card">
            <div class="sidebar-header">
                ...
                <h1 class="text-2xl font-extrabold text-slate-800 mt-0.5">my<span class="text-green-600">Space.</span></h1>
            </div>
            <nav class="sidebar-nav space-y-1">
                <?php foreach ($navItems as $item):
                    $a = ($page == $item['key']) ? 'active' : '';
                ?>
                    <a href="<?= base_url($item['url']) ?>" class="sidebar-item <?= $a ?>">
            ...
            <div class="sidebar-footer">
                <a href="<?= base_url('/logout') ?>" class="flex items-center gap-4 px-5 py-4 rounded-2xl text-red-500 font-bold hover:bg-red-50 transition-all text-sm">
    ...
    <div class="main-col">
        <main class="w-full max-w-screen-xl mx-auto px-4 lg:px-8 pt-6 pb-36">
            ...
            <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <span class="bg-green-50 text-green-600 px-4 py-2 rounded-xl font-bold text-xs uppercase tracking-widest border border-green-100 flex-shrink-0">
                    Resident
                </span>
            </header>
            ...
            <div class="max-w-4xl mx-auto">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-[32px] border border-slate-200 shadow-sm p-6 text-center lg:sticky lg:top-8">
                            <div class="relative inline-block mb-5">
                                <div class="w-20 h-20 lg:w-24 lg:h-24 bg-gradient-to-tr from-green-600 to-green-400 rounded-3xl flex items-center justify-center text-white text-3xl shadow-xl shadow-green-200 mx-auto">
                            ...
                            <?php if (!empty($user['phone'])): ?>
                                <p class="text-xs text-slate-400 mb-3"><i class="fa-solid fa-phone mr-1"></i><?= esc($user['phone']) ?></p>
                            <?php endif; ?>
                            <div class="inline-block px-4 py-1.5 bg-green-50 text-green-700 rounded-full text-[10px] font-black uppercase tracking-widest border border-green-100 mb-6">
                                Resident User
                            </div>
                            <button onclick="openModal()" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold rounded-2xl py-3.5 transition shadow-lg shadow-green-200/50 flex items-center justify-center gap-2 text-sm">
                    ...
                    <div class="lg:col-span-2 space-y-5">
                        ...
                        <!-- Info banner -->
                        <div class="bg-gradient-to-br from-green-600 to-green-500 rounded-[32px] p-6 lg:p-8 text-white flex items-center gap-4 lg:gap-6">
    ...
    <div class="modal-box">
        ...
        <form action="<?= base_url('profile/update') ?>" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="phone" value="<?= esc($user['phone'] ?? '') ?>" class="field-input" placeholder="e.g. +63 912 345 6789">
            </div>
            <div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-[2] py-4 bg-green-600 text-white rounded-2xl font-bold hover:bg-green-700 transition text-sm shadow-lg shadow-green-200/50">Save Changes</button>
            </div>
        </form>
    </div>
</div>
