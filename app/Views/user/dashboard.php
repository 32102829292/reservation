...
if ($pending > 0) {
    ...
    $nextAction = ['type'=>'upcoming','msg'=>"Your approved slot is coming up. Download your e-ticket from My Reservations and scan it at the entrance when you arrive.",'cta'=>'Get E-Ticket','url'=>'/reservation-list','color'=>'green'];
...
$nextColors = [
    ...
    'green'  => ['bg'=>'#f0fdf4','border'=>'#bbf7d0','icon_bg'=>'#dcfce7','icon_fg'=>'#16a34a','btn_bg'=>'#16a34a','icon'=>'fa-ticket'],
    ...
    'blue'   => ['bg'=>'#eff6ff','border'=>'#bfdbfe','icon_bg'=>'#dbeafe','icon_fg'=>'#2563eb','btn_bg'=>'#2563eb','icon'=>'fa-hand-wave'],
];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .mobile-nav-pill { position:fixed; bottom:20px; left:50%; transform:translateX(-50%); width:92%; max-width:480px; background:rgba(20,83,45,0.97); backdrop-filter:blur(16px); border-radius:24px; padding:6px; z-index:100; box-shadow:0 20px 40px -10px rgba(0,0,0,0.4); }
        ...
        .genre-fiction::before { background:#3b82f6; } .genre-fantasy::before { background:#8b5cf6; } .genre-poetry::before { background:#ec4899; } .genre-humor::before { background:#f59e0b; } .genre-history::before { background:#78716c; } .genre-science::before { background:#06b6d4; } .genre-default::before { background:#16a34a; }
        ...
        .genre-pill-fiction { background:#dbeafe; color:#1e40af; } .genre-pill-fantasy { background:#ede9fe; color:#5b21b6; } .genre-pill-poetry { background:#fce7f3; color:#9d174d; } .genre-pill-humor { background:#fef3c7; color:#92400e; } .genre-pill-default { background:#f0fdf4; color:#166534; }
        ...
        .how-step-num { width:28px; height:28px; border-radius:10px; background:var(--green); color:white; font-size:0.72rem; font-weight:900; display:flex; align-items:center; justify-content:center; flex-shrink:0; margin-top:1px; }
            ...
            <?php if (isset($remainingReservations)): ?>
                <div class="mx-4 mb-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div class="fairness-bar">
                        <!-- ★ Bar turns red when full, amber when 1 left -->
    ...
    <main class="flex-1 min-w-0 p-4 lg:p-10 pb-32" style="height:100vh;overflow-y:auto;">
        ...
        <header class="flex items-start justify-between mb-8 gap-4">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                <a href="<?= base_url('/reservation') ?>" class="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-green-200"><i class="fa-solid fa-plus"></i> Reserve</a>
                <div class="notif-bell" onclick="toggleNotifications()">
                    <div class="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-slate-200 hover:border-green-300 transition"><i class="fa-regular fa-bell text-slate-600"></i></div>
        ...
        <div id="notificationDropdown" class="notif-dropdown">
            <div class="p-3 border-b border-slate-100 flex justify-between items-center"><span class="font-extrabold text-sm text-slate-800">Notifications</span><button onclick="markAllAsRead()" class="text-xs text-green-600 hover:text-green-700 font-bold">Mark all read</button></div>
            <div id="notificationList" class="max-h-80 overflow-y-auto"></div>
        </div>

        <!-- Flash -->
        <?php if (session()->getFlashdata('success')): ?>
            <div class="mb-6 px-5 py-4 bg-green-50 border border-green-200 text-green-700 font-bold rounded-2xl flex items-center gap-3 text-sm fade-in"><i class="fa-solid fa-circle-check text-green-500"></i><?= session()->getFlashdata('success') ?></div>
        <?php endif; ?>

        <!-- ★ NEW: What to do next card -->
        ...
        <?php if ($upcoming): ?>
            <div class="upcoming-pill mb-6 fade-in">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="font-bold text-green-900 text-sm truncate"><?= esc($upcoming['resource_name'] ?? 'Resource') ?><?php if (!empty($upcoming['pc_number'])): ?>· <span class="font-normal"><?= esc($upcoming['pc_number']) ?></span><?php endif; ?></p>
        ...
        <!-- ★ ENHANCED Stat Cards — 4th slot shows unclaimed if any, else claimed, else declined -->
                        ...
                        <div class="space-y-1">
                            ...
                            ?>
                                <a href="<?= base_url('/reservation-list') ?>" class="booking-item">
                                    ...
                                    <!-- ★ Now correctly shows unclaimed as No-show -->
        ...
        <!-- ★ NEW: How It Works + Status Guide (shown for new users or users with issues) -->
        <?php if (empty($reservations) || $unclaimedCount > 0 || $pending > 0): ?>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div class="dash-card p-5">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-8 h-8 bg-green-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-list-check text-green-600 text-xs"></i></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="flex flex-col gap-5">
                <div class="dash-card p-5">
                    <div class="flex items-center justify-between mb-4"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Available Now</h3><a href="<?= base_url('/books') ?>" class="text-[10px] font-black text-green-600 hover:text-green-700 uppercase tracking-wider">All →</a></div>
                    <?php if (!empty($featuredBooks)): ?>
                        <div class="space-y-2">
                            ...
                            <a href="<?= base_url('/books') ?>" class="book-spine-card <?= $genreClass ?>">
                                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-base" style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);color:#166634;"><?= mb_strtoupper(mb_substr($book['title'],0,1)) ?></div>
                ...
                <div class="dash-card p-5">
                    <div class="flex items-center justify-between mb-4"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">My Borrows</h3><a href="<?= base_url('/books') ?>#mine" class="text-[10px] font-black text-green-600 hover:text-green-700 uppercase tracking-wider">All →</a></div>
                    ...
                    <?php else: ?>
                        <div class="text-center py-5"><i class="fa-regular fa-bookmark text-2xl text-slate-200 mb-2 block"></i><p class="text-xs text-slate-400 font-medium">No active borrows</p><a href="<?= base_url('/books') ?>" class="inline-flex items-center gap-1 mt-2 text-[10px] font-bold text-green-600 hover:text-green-700"><i class="fa-solid fa-book-open text-[9px]"></i> Borrow a book</a></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
