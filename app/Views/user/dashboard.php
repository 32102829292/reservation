...
// ★ Determine contextual "What to do next" message
...
<head>
    ...
    <style>
        :root { --blue:#2563eb; --blue-light:#eff6ff; --blue-dark:#1e3a8a; --slate-bg:#f8fafc; }
        * { box-sizing:border-box; }
        ...
        .mobile-nav-pill { position:fixed; bottom:20px; left:50%; transform:translateX(-50%); width:92%; max-width:480px; background:rgba(30,58,138,0.97); backdrop-filter:blur(16px); border-radius:24px; padding:6px; z-index:100; box-shadow:0 20px 40px -10px rgba(0,0,0,0.4); }
        ...
        .stat-total::after { background:#3b82f6; } .stat-pending::after { background:#f59e0b; } .stat-approved::after { background:#10b981; } .stat-declined::after { background:#f43f5e; }
        ...
        .fc-button-primary { background:var(--blue)!important; border-color:var(--blue)!important; border-radius:10px!important; font-family:'Plus Jakarta Sans',sans-serif!important; font-weight:700!important; font-size:0.75rem!important; padding:0.3rem 0.65rem!important; }
        ...
        .booking-item:hover { background:var(--blue-light); }
        ...
        .quick-action:hover { border-color:var(--blue); background:var(--blue-light); color:var(--blue); transform:translateY(-1px); }
        .fairness-bar { height:6px; border-radius:999px; background:#e2e8f0; overflow:hidden; }
        .fairness-fill { height:100%; border-radius:999px; background:var(--blue); transition:width 0.6s cubic-bezier(0.34,1.56,0.64,1); }
        ...
        .upcoming-pill { background:linear-gradient(135deg,#eff6ff,#dbeafe); border:1px solid #bfdbfe; border-radius:20px; padding:1rem 1.25rem; display:flex; align-items:center; gap:12px; }
        ...
        .tag-pending { background:#fef3c7; color:#92400e; } .tag-approved { background:#dcfce7; color:#166534; } .tag-claimed { background:#f3e8ff; color:#6b21a8; } .tag-declined,.tag-cancelled { background:#fee2e2; color:#991b1b; } .tag-expired { background:#f1f5f9; color:#475569; } .tag-unclaimed { background:#fff7ed; color:#c2410c; border:1px dashed #fdba74; }
        .books-banner { background:linear-gradient(135deg,#0f172a 0%,#1e3a8a 50%,#1e40af 100%); border-radius:28px; padding:1.75rem; position:relative; overflow:hidden; border:1px solid #1d4ed8; }
        ...
        .genre-fiction::before { background:#3b82f6; } .genre-fantasy::before { background:#8b5cf6; } .genre-poetry::before { background:#ec4899; } .genre-humor::before { background:#f59e0b; } .genre-history::before { background:#78716c; } .genre-science::before { background:#06b6d4; } .genre-default::before { background:#2563eb; }
        .avail-dot { width:8px; height:8px; border-radius:50%; flex-shrink:0; }
        .avail-dot.on { background:#3b82f6; box-shadow:0 0 0 3px rgba(59,130,246,0.2); }
        ...
        .rag-search-input:focus { border-color:#2563eb; background:white; box-shadow:0 0 0 3px rgba(37,99,235,0.1); }
        ...
        .ai-suggestion-box { background:linear-gradient(135deg,#eff6ff,#dbeafe); border:1.5px solid #bfdbfe; border-radius:16px; padding:0.875rem 1rem; margin-top:0.75rem; display:none; animation:fadeIn 0.3s ease; }
        ...
        .genre-pill-fiction { background:#dbeafe; color:#1e40af; } .genre-pill-fantasy { background:#ede9fe; color:#5b21b6; } .genre-pill-poetry { background:#fce7f3; color:#9d174d; } .genre-pill-humor { background:#fef3c7; color:#92400e; } .genre-pill-default { background:#eff6ff; color:#1e40af; }
        .borrow-tag-pending { background:#fef3c7; color:#92400e; } .borrow-tag-approved { background:#dcfce7; color:#166534; } .borrow-tag-returned { background:#dbeafe; color:#1e40af; } .borrow-tag-rejected { background:#fee2e2; color:#991b1b; }
        ...
        .next-step-card { border-radius:20px; padding:1rem 1.25rem; border:1.5px solid; display:flex; align-items:flex-start; gap:12px; }
        .next-step-card a { font-size:0.72rem; font-weight:800; text-decoration:none; display:inline-flex; align-items:center; gap:4px; margin-top:6px; padding:5px 12px; border-radius:8px; }
        .how-step { display:flex; align-items:flex-start; gap:12px; padding:0.85rem 0; border-bottom:1px solid #f1f5f9; }
        .how-step:last-child { border-bottom:none; }
        .how-step-num { width:28px; height:28px; border-radius:10px; background:var(--blue); color:white; font-size:0.72rem; font-weight:900; display:flex; align-items:center; justify-content:center; flex-shrink:0; margin-top:1px; }
        .status-guide-row { display:flex; align-items:center; gap:10px; padding:0.55rem 0; border-bottom:1px solid #f8fafc; }
...
<body>
        ...
        <div class="sidebar-card">
            ...
            <nav class="sidebar-nav space-y-1">
                <?php foreach ($navItems as $item):
                    $active = ($page == $item['key']) ? 'bg-blue-600 text-white shadow-lg shadow-blue-200/50' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600';
                ?>
            ...
            <?php if (isset($remainingReservations)): ?>
                <div class="mx-4 mb-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-xs font-black <?= $remaining===0?'text-red-500':($remaining===1?'text-amber-500':'text-blue-600') ?>"><?= $usedSlots ?>/<?= $maxSlots ?></span>
                    </div>
                    ...
                    <p class="text-[10px] mt-1.5 font-medium <?= $remaining===0?'text-red-500 font-bold':($remaining===1?'text-amber-500 font-semibold':'text-slate-400') ?>">
                        <?php if ($remaining === 0): ?>⚠ No slots left this month<?php
                        elseif ($remaining === 1): ?>⚡ Only 1 slot remaining<?php
                        else: ?><?= $remaining ?> slot<?= $remaining!=1?'s':'' ?> remaining this month<?php endif; ?>
                    </p>
                </div>
            <?php endif; ?>
    ...
    <div id="dateModal" class="modal-backdrop" onclick="handleModalBackdrop(event)">
        <div class="modal-card">
            <div class="flex justify-between items-center mb-5">
                <div><h3 class="text-lg font-black text-slate-900" id="modalDateTitle"></h3><p class="text-xs text-slate-400 font-medium mt-0.5" id="modalDateSub"></p></div>
                <button onclick="closeDateModal()" class="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div id="modalReservationsList" class="space-y-1"></div>
            <div class="mt-4 text-center text-sm text-slate-400 hidden" id="modalEmptyMessage"><i class="fa-regular fa-calendar-xmark text-2xl block mb-2 text-slate-300"></i>No reservations for this date.</div>
    ...
    <div id="loginToast" class="login-toast">
        ...
        <div class="flex-1 min-w-0"><p class="font-black text-sm leading-tight" id="toastTitle"></p><p class="text-xs text-white/70 mt-0.5" id="toastBody"></p></div>
    ...
    <main class="flex-1 min-w-0 p-4 lg:p-10 pb-32" style="height:100vh;overflow-y:auto;">
        ...
        <header class="flex items-start justify-between mb-8 gap-4">
            <div class="fade-in-up">
                <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1"><?php $hour=(int)date('H'); echo $hour<12?'Good morning':($hour<17?'Good afternoon':'Good evening'); ?></p>
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                <a href="<?= base_url('/reservation') ?>" class="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-blue-200"><i class="fa-solid fa-plus"></i> Reserve</a>
                <div class="notif-bell" onclick="toggleNotifications()">
                    <div class="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-slate-200 hover:border-blue-300 transition"><i class="fa-regular fa-bell text-slate-600"></i></div>
        ...
        <div id="notificationDropdown" class="notif-dropdown">
            <div class="p-3 border-b border-slate-100 flex justify-between items-center"><span class="font-extrabold text-sm text-slate-800">Notifications</span><button onclick="markAllAsRead()" class="text-xs text-blue-600 hover:text-blue-700 font-bold">Mark all read</button></div>
            <div id="notificationList" class="max-h-80 overflow-y-auto"></div>
        </div>

        <!-- Flash -->
        <?php if (session()->getFlashdata('success')): ?>
            <div class="mb-6 px-5 py-4 bg-blue-50 border border-blue-200 text-blue-700 font-bold rounded-2xl flex items-center gap-3 text-sm fade-in"><i class="fa-solid fa-circle-check text-blue-500"></i><?= session()->getFlashdata('success') ?></div>
        <?php endif; ?>

        <!-- ★ What to do next card -->
        <?php if ($nextAction): $nc = $nextColors[$nextAction['color']]; ?>
            <div class="next-step-card mb-5 fade-in" style="background:<?= $nc['bg'] ?>;border-color:<?= $nc['border'] ?>">
                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style="background:<?= $nc['icon_bg'] ?>;color:<?= $nc['icon_fg'] ?>">
                    <i class="fa-solid <?= $nc['icon'] ?> text-sm"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-[10px] font-black uppercase tracking-widest mb-0.5" style="color:<?= $nc['icon_fg'] ?>">What to do next</p>
                    <p class="text-sm font-medium text-slate-700 leading-snug"><?= $nextAction['msg'] ?></p>
                    <a href="<?= base_url($nextAction['url']) ?>" style="background:<?= $nc['btn_bg'] ?>;color:white">
                        <?= $nextAction['cta'] ?> <i class="fa-solid fa-arrow-right text-[9px]"></i>
                    </a>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div id="timerBanner" class="timer-banner">
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div class="flex-1 min-w-0"><p class="font-black text-sm leading-tight" id="timerTitle">Your reservation ends soon</p><p class="text-[11px] font-medium opacity-75 mt-0.5" id="timerSub"></p></div>
            ...
            <div class="timer-progress-wrap" id="timerProgressWrap" style="display:none"><div class="timer-progress-fill" id="timerProgressFill" style="width:0%"></div></div>
        </div>

        <!-- Upcoming banner -->
        <?php if ($upcoming): ?>
            <div class="upcoming-pill mb-6 fade-in">
                <div class="w-10 h-10 bg-blue-600 rounded-2xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-ticket text-white text-sm"></i></div>
                <div class="flex-1 min-w-0">
                    <p class="text-[10px] font-black text-blue-700 uppercase tracking-widest">Upcoming Reservation</p>
                    <p class="font-bold text-blue-900 text-sm truncate"><?= esc($upcoming['resource_name'] ?? 'Resource') ?><?php if (!empty($upcoming['pc_number'])): ?>· <span class="font-normal"><?= esc($upcoming['pc_number']) ?></span><?php endif; ?></p>
                    <p class="text-xs text-blue-700 font-medium"><?= date('M j, Y',strtotime($upcoming['reservation_date'])) ?> &nbsp;·&nbsp; <?= date('g:i A',strtotime($upcoming['start_time'])) ?> – <?= date('g:i A',strtotime($upcoming['end_time'])) ?></p>
                </div>
                <a href="<?= base_url('/reservation-list') ?>" class="text-xs font-black text-blue-700 bg-white px-3 py-1.5 rounded-xl border border-blue-200 hover:bg-blue-50 transition flex-shrink-0">View →</a>
            </div>
        <?php endif; ?>
        ...
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
            <div class="stat-card stat-total">
                <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-blue-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-layer-group text-blue-500 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Total</span></div>
                <p class="text-3xl font-black text-slate-800 stat-num"><?= $total ?></p><p class="text-xs text-slate-400 font-medium mt-0.5">All time</p>
            </div>
            <div class="stat-card stat-pending">
                <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-amber-50 rounded-xl flex items-center justify-center"><i class="fa-regular fa-clock text-amber-500 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Pending</span></div>
                <p class="text-3xl font-black text-amber-600 stat-num"><?= $pending ?></p><p class="text-xs text-slate-400 font-medium mt-0.5">Awaiting review</p>
            </div>
            <div class="stat-card stat-approved">
                <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-emerald-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-circle-check text-emerald-500 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Approved</span></div>
                <p class="text-3xl font-black text-emerald-600 stat-num"><?= $approved ?></p><p class="text-xs text-slate-400 font-medium mt-0.5">Ready to use</p>
            </div>
            <?php if ($unclaimedCount > 0): ?>
                <div class="stat-card" style="border-color:#fed7aa">
                    <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-orange-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-ticket text-orange-400 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">No-show</span></div>
                    <p class="text-3xl font-black text-orange-500 stat-num"><?= $unclaimedCount ?></p><p class="text-xs text-orange-400 font-semibold mt-0.5">Slot<?= $unclaimedCount>1?'s':'' ?> missed</p>
                </div>
            <?php elseif ($claimedCount > 0): ?>
                <div class="stat-card">
                    <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-purple-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-check-double text-purple-400 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Claimed</span></div>
                    <p class="text-3xl font-black text-purple-500 stat-num"><?= $claimedCount ?></p><p class="text-xs text-slate-400 font-medium mt-0.5">Tickets used</p>
                </div>
            <?php else: ?>
                <div class="stat-card stat-declined">
                    <div class="flex items-start justify-between mb-3"><div class="w-9 h-9 bg-rose-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-ban text-rose-400 text-sm"></i></div><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Declined</span></div>
                    <p class="text-3xl font-black text-rose-500 stat-num"><?= $declined ?></p><p class="text-xs text-slate-400 font-medium mt-0.5">Not approved</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="flex flex-col gap-5">
                <div class="dash-card p-5">
                    ...
                    <div class="space-y-2">
                        <a href="<?= base_url('/reservation') ?>" class="quick-action"><div class="w-8 h-8 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-plus text-blue-600 text-xs"></i></div><span>New Reservation</span><i class="fa-solid fa-chevron-right text-xs text-slate-300 ml-auto"></i></a>
                        <a href="<?= base_url('/reservation-list') ?>" class="quick-action"><div class="w-8 h-8 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-regular fa-calendar text-indigo-500 text-xs"></i></div><span>My Reservations</span><i class="fa-solid fa-chevron-right text-xs text-slate-300 ml-auto"></i></a>
                ...
                <div class="dash-card p-5 flex-1">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="<?= base_url('/reservation-list') ?>" class="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-wider">View all →</a>
                    </div>
                        ...
                        <div class="space-y-1">
                            ...
                            ?>
                                <a href="<?= base_url('/reservation-list') ?>" class="booking-item">
                                    ...
                                    <span class="tag tag-<?= $s ?> flex-shrink-0"><?= $s==='unclaimed'?'No-show':ucfirst($s) ?></span>
                                </a>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <div class="text-center py-8">
                            ...
                            <a href="<?= base_url('/reservation') ?>" class="inline-flex items-center gap-1 mt-3 text-xs font-bold text-blue-600 hover:text-blue-700"><i class="fa-solid fa-plus text-[10px]"></i> Make your first reservation</a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div class="dash-card p-5">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-8 h-8 bg-blue-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-list-check text-blue-600 text-xs"></i></div>
                    <div><h3 class="font-extrabold text-slate-800 text-sm">How to Reserve</h3><p class="text-[10px] text-slate-400 font-medium">Step-by-step guide</p></div>
                </div>
                <?php $step=1; foreach ([
                    ...
                    <div class="how-step">
                        ...
                        <div><p class="font-bold text-slate-800 text-sm"><?= $title ?></p><p class="text-xs text-slate-400 font-medium mt-0.5"><?= $body ?></p></div>
                    </div>
                <?php endforeach; ?>
            </div>
            <div class="dash-card p-5">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-8 h-8 bg-blue-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-circle-info text-blue-500 text-xs"></i></div>
                    <div><h3 class="font-extrabold text-slate-800 text-sm">What Each Status Means</h3><p class="text-[10px] text-slate-400 font-medium">So you always know where you stand</p></div>
                </div>
                <?php foreach ([
                    ...
                    <div class="status-guide-row">
                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex-shrink-0 w-24 justify-center" style="background:<?= $bg ?>;color:<?= $fg ?>">
                            <i class="fa-solid <?= $icon ?> text-[8px]"></i><?= $label ?>
                        </span>
                        <p class="text-xs text-slate-500 font-medium"><?= $desc ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 flex flex-col gap-5">
                <div class="books-banner">
                    <div class="relative z-10 flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            <p class="text-[10px] font-black tracking-[0.2em] text-blue-300 uppercase mb-1">Community Library</p>
                            <h3 class="text-xl font-black text-white leading-tight mb-1"><?= $availableCount ?> books available</h3>
                            <p class="text-blue-300 text-xs font-medium"><?= $totalBooks ?> total titles in the collection</p>
                        </div>
                        <a href="<?= base_url('/books') ?>" class="flex items-center gap-2 px-5 py-2.5 bg-white/15 hover:bg-white/25 text-white border border-white/20 rounded-2xl font-bold text-sm transition backdrop-blur-sm flex-shrink-0"><i class="fa-solid fa-book-open text-xs"></i> Browse All</a>
                    </div>
                    <div class="relative z-10 flex gap-4 mt-4 pt-4 border-t border-white/10">
                        <div class="flex items-center gap-2"><div class="w-7 h-7 bg-white/10 rounded-xl flex items-center justify-center"><i class="fa-solid fa-bookmark text-blue-300 text-xs"></i></div><div><p class="text-[10px] font-black text-blue-300 uppercase tracking-wider">My Borrows</p><p class="text-sm font-black text-white"><?= count($myBorrowings) ?></p></div></div>
                        <div class="flex items-center gap-2"><div class="w-7 h-7 bg-white/10 rounded-xl flex items-center justify-center"><i class="fa-solid fa-hourglass-half text-amber-300 text-xs"></i></div><div><p class="text-[10px] font-black text-blue-300 uppercase tracking-wider">Pending</p><p class="text-sm font-black text-white"><?= count(array_filter($myBorrowings,fn($b)=>($b['status']??'')==='pending')) ?></p></div></div>
                        <div class="flex items-center gap-2"><div class="w-7 h-7 bg-white/10 rounded-xl flex items-center justify-center"><i class="fa-solid fa-circle-check text-sky-300 text-xs"></i></div><div><p class="text-[10px] font-black text-blue-300 uppercase tracking-wider">Active</p><p class="text-sm font-black text-white"><?= count(array_filter($myBorrowings,fn($b)=>($b['status']??'')==='approved')) ?></p></div></div>
                    </div>
                </div>
                <div class="dash-card p-5">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-8 h-8 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl flex items-center justify-center"><i class="fa-solid fa-wand-magic-sparkles text-blue-600 text-xs"></i></div>
                    ...
                    <div class="rag-search-wrap"><i class="fa-solid fa-magnifying-glass rag-search-icon"></i><input type="text" id="dashRagInput" class="rag-search-input" placeholder="e.g. Filipino history, funny stories, adventure for kids…" onkeydown="if(event.key==='Enter') dashRagSearch()"></div>
                    <div id="dashRagSkel" style="display:none;margin-top:.75rem"><div class="shimmer" style="width:90%"></div><div class="shimmer" style="width:72%"></div><div class="shimmer" style="width:55%"></div></div>
                    <div class="ai-suggestion-box" id="dashRagResult">
                        <div class="flex items-start gap-2 mb-2"><i class="fa-solid fa-robot text-blue-600 text-xs mt-0.5 flex-shrink-0"></i><p class="text-[10px] font-black text-blue-700 uppercase tracking-wider">Librarian Suggestion</p></div>
                        <p class="text-sm text-blue-900 font-medium leading-relaxed" id="dashRagText"></p>
                    ...
                    <div class="flex items-center justify-between mt-3">
                        <button onclick="dashRagSearch()" id="dashRagBtn" class="flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs transition"><i class="fa-solid fa-wand-magic-sparkles text-[10px]"></i> Find Books</button>
                        <a href="<?= base_url('/books') ?>" class="text-xs font-bold text-blue-600 hover:text-blue-700">See full library →</a>
                    </div>
                </div>
            </div>
            <div class="flex flex-col gap-5">
                <div class="dash-card p-5">
                    <div class="flex items-center justify-between mb-4"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Available Now</h3><a href="<?= base_url('/books') ?>" class="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-wider">All →</a></div>
                    <?php if (!empty($featuredBooks)): ?>
                        <div class="space-y-2">
                            <?php foreach (array_slice($featuredBooks,0,5) as $book):
                                $genre=$book['genre']??'default'; $genre=strtolower($genre);
                            ...
                            <a href="<?= base_url('/books') ?>" class="book-spine-card <?= $genreClass ?>">
                                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-base" style="background:linear-gradient(135deg,#eff6ff,#dbeafe);color:#1e40af;"><?= mb_strtoupper(mb_substr($book['title'],0,1)) ?></div>
                                <div class="flex-1 min-w-0"><p class="font-bold text-xs text-slate-800 truncate leading-tight"><?= esc($book['title']) ?></p><p class="text-[10px] text-slate-400 font-medium truncate"><?= esc($book['author']??'Unknown') ?></p><span class="genre-pill <?= $gpill ?> mt-0.5 inline-block"><?= esc($book['genre']??'General') ?></span></div>
                                <div class="flex flex-col items-end gap-1 flex-shrink-0"><div class="avail-dot <?= $available?'on':'off' ?>"></div><span class="text-[9px] text-slate-400 font-bold"><?= (int)($book['available_copies']??0) ?> left</span></div>
                            </a>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?><div class="text-center py-6"><i class="fa-solid fa-book text-3xl text-slate-200 mb-2 block"></i><p class="text-xs text-slate-400 font-medium">No books yet</p></div><?php endif; ?>
                </div>
                <div class="dash-card p-5">
                    <div class="flex items-center justify-between mb-4"><h3 class="text-xs font-black uppercase tracking-widest text-slate-400">My Borrows</h3><a href="<?= base_url('/books') ?>#mine" class="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-wider">All →</a></div>
                    ...
                    <?php if (!empty($activeBorrows)): ?>
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-50 border border-slate-100">
                                <div class="w-8 h-8 bg-white rounded-xl flex items-center justify-center flex-shrink-0 border border-slate-200 font-black text-sm text-blue-700"><?= mb_strtoupper(mb_substr($borrow['title']??'B',0,1)) ?></div>
                                <div class="flex-1 min-w-0"><p class="font-bold text-xs text-slate-800 truncate"><?= esc($borrow['title']??'Unknown Book') ?></p><?php if(!empty($borrow['due_date'])&&$bs==='approved'):?><p class="text-[10px] text-slate-400 font-medium">Due <?= date('M j',strtotime($borrow['due_date'])) ?></p><?php endif;?></div>
                                <span class="tag borrow-tag-<?= $bs ?> text-[9px] flex-shrink-0"><?= ucfirst($bs) ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <div class="text-center py-5"><i class="fa-regular fa-bookmark text-2xl text-slate-200 mb-2 block"></i><p class="text-xs text-slate-400 font-medium">No active borrows</p><a href="<?= base_url('/books') ?>" class="inline-flex items-center gap-1 mt-2 text-[10px] font-bold text-blue-600 hover:text-blue-700"><i class="fa-solid fa-book-open text-[9px]"></i> Borrow a book</a></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function loadNotifications(){const seen=getNotifiedIds();notifications=reservations.filter(r=>r.status==='approved').map(r=>({id:parseInt(r.id),title:'Reservation Approved',msg:`${r.resource_name||'Resource'} · ${new Date(r.reservation_date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}`,time:r.updated_at||r.created_at||new Date().toISOString(),read:seen.includes(parseInt(r.id))}));updateBadge();renderNotifs();}
        ...
        function renderNotifs(){const list=document.getElementById('notificationList');if(!notifications.length){list.innerHTML=`<div class="text-center py-8 px-4"><i class="fa-regular fa-bell-slash text-3xl text-slate-200 mb-2 block"></i><p class="text-sm text-slate-400 font-medium">All caught up!</p></div>`;return;}list.innerHTML=notifications.sort((a,b)=>new Date(b.time)-new Date(a.time)).map(n=>`<div class="notif-item ${!n.read?'unread':''}" onclick="markAsRead(${n.id})"><div class="flex items-start gap-3"><div class="w-8 h-8 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5"><i class="fa-solid fa-check text-blue-600 text-xs"></i></div><div class="flex-1 min-w-0"><p class="font-bold text-sm text-slate-800">${n.title}</p><p class="text-xs text-slate-500 truncate">${n.msg}</p><p class="text-[10px] text-slate-400 mt-1">${timeAgo(n.time)}</p></div>${!n.read?'<span class="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0 mt-1.5"></span>':''}</div></div>`).join('');}
        ...
        function openDateModal(date,items){const d=new Date(date+'T00:00:00');document.getElementById('modalDateTitle').textContent=d.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});document.getElementById('modalDateSub').textContent=items.length?`${items.length} reservation${items.length>1?'s':''}`:'';;const list=document.getElementById('modalReservationsList'),empty=document.getElementById('modalEmptyMessage');list.innerHTML='';if(items.length){empty.classList.add('hidden');items.sort((a,b)=>(a.start_time||'').localeCompare(b.start_time||'')).forEach(r=>{const isClaimed=r.claimed==1,s=isClaimed?'claimed':(r.status||'pending').toLowerCase();const colorMap={approved:'bg-emerald-100 text-emerald-700',pending:'bg-amber-100 text-amber-700',declined:'bg-rose-100 text-rose-700',canceled:'bg-rose-100 text-rose-700',claimed:'bg-purple-100 text-purple-700'};const col=colorMap[s]||'bg-slate-100 text-slate-600';const t1=r.start_time?r.start_time.substring(0,5):'All day',t2=r.end_time?` – ${r.end_time.substring(0,5)}`:'';const row=document.createElement('div');row.className='date-row';row.innerHTML=`<div class="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-regular fa-calendar text-slate-400 text-xs"></i></div><div class="flex-1 min-w-0"><p class="font-bold text-sm text-slate-800">${r.resource_name||'Reserved'}</p><p class="text-xs text-slate-400 mt-0.5">${t1}${t2}</p></div><span class="text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${col}">${s.charAt(0).toUpperCase()+s.slice(1)}</span>`;list.appendChild(row);});}else{empty.classList.remove('hidden');}document.getElementById('dateModal').classList.add('show');document.body.style.overflow='hidden';}
        ...
        document.addEventListener('DOMContentLoaded',()=>{
            ...
            loadNotifications();initCountdownBanner();showLoginToast();
            const byDate={};allReservationsData.forEach(r=>{if(!r.reservation_date)return;if(!byDate[r.reservation_date])byDate[r.reservation_date]=[];byDate[r.reservation_date].push(r);});
            ...
            const cal=new FullCalendar.Calendar(document.getElementById('calendar'),{initialView:'dayGridMonth',headerToolbar:{left:'prev,next',center:'title',right:'today'},events,height:380,eventDisplay:'block',eventMaxStack:2,dateClick:info=>openDateModal(info.dateStr,byDate[info.dateStr]||[]),eventClick:info=>{const d=info.event.startStr.split('T')[0];openDateModal(d,byDate[d]||[]);},dayCellDidMount:info=>{const d=info.date.toISOString().split('T')[0];const res=byDate[d];if(res?.length){const badge=document.createElement('div');badge.style.cssText='font-size:9px;font-weight:800;color:white;background:#2563eb;border-radius:999px;width:16px;height:16px;display:flex;align-items:center;justify-content:center;margin-left:auto;margin-right:4px;margin-bottom:2px;';badge.textContent=res.length;info.el.querySelector('.fc-daygrid-day-top')?.appendChild(badge);}}});
            cal.render();
        });
        function initCountdownBanner(){const banner=document.getElementById('timerBanner'),titleEl=document.getElementById('timerTitle'),subEl=document.getElementById('timerSub'),hEl=document.getElementById('tdHv'),mEl=document.getElementById('tdMv'),sEl=document.getElementById('tdSv'),iconEl=document.getElementById('timerIcon'),progressWrap=document.getElementById('timerProgressWrap'),progressFill=document.getElementById('timerProgressFill');function findTarget(){const now=Date.now();let active=null,upcoming=null;_approved.forEach(r=>{if(!r.reservation_date||!r.start_time||!r.end_time)return;const start=new Date(r.reservation_date+'T'+r.start_time).getTime(),end=new Date(r.reservation_date+'T'+r.end_time).getTime(),minsToStart=(start-now)/60000,minsToEnd=(end-now)/60000;if(now>=start&&now<end&&!active)active={r,start,end,mode:'active',minsLeft:minsToEnd};if(!upcoming&&minsToStart>0&&minsToStart<=30)upcoming={r,start,end,mode:'upcoming',minsLeft:minsToStart};});return active||upcoming||null;}function tick(){const target=findTarget();if(!target){banner.style.display='none';return;}const{r,start,end,mode,minsLeft}=target,now=Date.now(),diff=Math.max(0,(mode==='active'?end:start)-now),h=Math.floor(diff/3600000),m=Math.floor((diff%3600000)/60000),s=Math.floor((diff%60000)/1000);hEl.textContent=String(h).padStart(2,'0');mEl.textContent=String(m).padStart(2,'0');sEl.textContent=String(s).padStart(2,'0');banner.classList.remove('urgent','warning','safe');if(mode==='active'){if(minsLeft<=10){banner.classList.add('urgent');iconEl.innerHTML='<i class="fa-solid fa-triangle-exclamation"></i>';}else if(minsLeft<=20){banner.classList.add('warning');iconEl.innerHTML='<i class="fa-solid fa-hourglass-half"></i>';}else{banner.classList.add('safe');iconEl.innerHTML='<i class="fa-solid fa-hourglass-start"></i>';}titleEl.textContent=minsLeft<=10?'⚠️ Your reservation ends very soon!':'Your reservation is active';subEl.textContent=`${r.resource_name||'Resource'} · Ends at ${r.end_time?.substring(0,5)}`;const pct=Math.min(100,Math.max(0,((now-start)/(end-start))*100));progressWrap.style.display='block';progressFill.style.width=pct.toFixed(1)+'%';}else{banner.classList.add('safe');iconEl.innerHTML='<i class="fa-solid fa-bell"></i>';titleEl.textContent='Your reservation starts soon';subEl.textContent=`${r.resource_name||'Resource'} · Starts at ${r.start_time?.substring(0,5)}`;progressWrap.style.display='none';}banner.style.display='block';}tick();setInterval(tick,1000);}
        function showLoginToast(){const key='toast_shown_<?= session()->get('user_id') ?>_'+new Date().toDateString();if(sessionStorage.getItem(key))return;sessionStorage.setItem(key,'1');const now=Date.now();let toastData=null;_approved.forEach(r=>{if(!r.reservation_date||!r.start_time||!r.end_time)return;const start=new Date(r.reservation_date+'T'+r.start_time).getTime(),end=new Date(r.reservation_date+'T'+r.end_time).getTime(),minsToStart=(start-now)/60000,today=new Date().toDateString(),resDay=new Date(r.reservation_date+'T00:00:00').toDateString();if(now>=start&&now<end&&!toastData)toastData={icon:'<i class="fa-solid fa-circle-play" style="color:#3b82f6"></i>',bg:'#2563eb',title:'Active reservation now!',body:`${r.resource_name||'Resource'} ends at ${r.end_time?.substring(0,5)} — don't forget!`};if(!toastData&&resDay===today&&minsToStart>0&&minsToStart<=120)toastData={icon:'<i class="fa-solid fa-bell" style="color:#f59e0b"></i>',bg:'#d97706',title:`Reservation in ${Math.round(minsToStart)} min`,body:`${r.resource_name||'Resource'} · ${r.start_time?.substring(0,5)} – ${r.end_time?.substring(0,5)}`};if(!toastData&&resDay===today){const fmt=t=>{const[h,m]=t.split(':');const hr=+h;return`${hr%12||12}:${m} ${hr<12?'AM':'PM'}`;};toastData={icon:'<i class="fa-solid fa-calendar-check" style="color:#60a5fa"></i>',bg:'#2563eb',title:'You have a reservation today',body:`${r.resource_name||'Resource'} · ${fmt(r.start_time)} – ${fmt(r.end_time)}`};}});if(!toastData)return;const toast=document.getElementById('loginToast');document.getElementById('toastIcon').innerHTML=toastData.icon;document.getElementById('toastIcon').style.background=toastData.bg+'33';document.getElementById('toastTitle').textContent=toastData.title;document.getElementById('toastBody').textContent=toastData.body;setTimeout(()=>toast.classList.add('show'),900);setTimeout(()=>toast.classList.remove('show'),7900);}
        function dismissToast(){document.getElementById('loginToast').classList.remove('show');}
        async function dashRagSearch(){const query=document.getElementById('dashRagInput').value.trim();if(query.length<2)return;const skel=document.getElementById('dashRagSkel'),res=document.getElementById('dashRagResult'),err=document.getElementById('dashRagErr'),btn=document.getElementById('dashRagBtn');res.classList.remove('show');err.style.display='none';skel.style.display='block';btn.disabled=true;try{const r=await fetch('/rag/suggest',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},body:JSON.stringify({query})});const d=await r.json();skel.style.display='none';btn.disabled=false;if(d.message&&!d.suggestion){err.textContent=d.message;err.style.display='block';return;}if(d.error&&!d.books){err.textContent=d.error;err.style.display='block';return;}document.getElementById('dashRagText').textContent=d.suggestion||'';const booksRow=document.getElementById('dashRagBooks');booksRow.innerHTML='';(d.books||[]).slice(0,4).forEach(b=>{const avail=(b.available_copies||0)>0;const chip=document.createElement('a');chip.href='/books';chip.className='inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-bold border transition '+(avail?'bg-white border-blue-200 text-blue-700 hover:bg-blue-50':'bg-slate-50 border-slate-200 text-slate-400');chip.innerHTML=`<i class="fa-solid fa-book text-[9px]"></i>${b.title}${avail?'':' <span class="text-[9px]">(out)</span>'}`;booksRow.appendChild(chip);});res.classList.add('show');}catch(e){skel.style.display='none';btn.disabled=false;err.textContent='Network error. Try again.';err.style.display='block';}}
    </script>
