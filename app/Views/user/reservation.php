...
<head>
    ...
    <style>
        ...
        .page-title { font-size: 1.75rem; font-weight: 800; color: var(--text); letter-spacing: -.04em; line-height: 1.1; }
        .page-sub { font-size: .8rem; color: var(--text-sub); margin-top: 4px; font-weight: 500; }

        .back-btn { display: inline-flex; align-items: center; gap: 7px; padding: 9px 16px; background: var(--card); border: 1px solid var(--border); border-radius: var(--r-sm); font-size: .8rem; font-weight: 700; color: var(--text-muted); text-decoration: none; transition: all var(--ease); box-shadow: var(--shadow-sm); }
        ...
        /* ── Form card ── */
        .form-card { background: var(--card); border-radius: var(--r-xl); border: 1px solid var(--border); box-shadow: var(--shadow-sm); padding: 28px; max-width: 760px; margin: 0 auto; transition: background var(--ease), border-color var(--ease); }
        ...
        input, select, textarea {
            ...
            transition: all var(--ease); background: var(--input-bg);
        ...
        /* ── Submit button ── */
        .btn-primary { background: var(--indigo); color: white; border: none; padding: .85rem 1.75rem; border-radius: var(--r-md); font-weight: 700; font-size: .88rem; cursor: pointer; transition: all var(--ease); font-family: var(--font); display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 4px 12px rgba(55,48,163,.28); touch-action: manipulation; }
...
<main class="main-area">
    ...
    <?php if (isset($isBlocked) && $isBlocked): ?>
        <div class="flash flash-err">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
            ...
            <div style="margin-bottom:24px;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px;">
                    <div class="section-icon">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </div>
                    <div>
                ...
                <div id="pcSection" class="hidden" style="margin-bottom:16px;">
                    <div class="pc-section">
                        <label class="pc-section-lbl">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="display:inline;margin-right:5px;"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
...
function checkAvailability() {
    ...
    fetch('<?= base_url("reservation/check-availability") ?>', {
        ...
        m.style.cssText=''; m.textContent=data.message; m.classList.add(data.available?'available':'unavailable');
...
function previewReservation() {
    ...
    if (!rid)  { alert('Please select a resource'); return; }
    if (hasPc && selectedPcs.length === 0) { alert('Please select at least one workstation'); return; }
    if (!date) { alert('Please select a date'); return; }
    if (!st)   { alert('Please enter start time'); return; }
    if (!et)   { alert('Please enter end time'); return; }
    if (!pv)   { alert('Please select a purpose'); return; }
    ...
    fetch('<?= base_url("reservation/check-new-approvals") ?>', {
        ...
        if (data.new_approvals?.length) {
            data.new_approvals.forEach(a => {
                ...
                notifications.unshift(n); unreadCount++; updateBadge(); renderNotifications(); showPush(n); showToast(n);
            });
...
function markAsRead(id) {
    ...
    if (n&&!n.read) { n.read=true; unreadCount=Math.max(0,unreadCount-1); updateBadge(); renderNotifications(); }
}
function updateBadge() {
    const b=document.getElementById('notificationBadge');
    if (b) { if (unreadCount>0) { b.style.display='block'; b.textContent=unreadCount>9?'9+':unreadCount; } else b.style.display='none'; }
}
