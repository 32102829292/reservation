...
<body class="flex">
    <?php $navItems = [['url' => '/dashboard', 'icon' => 'fa-house', 'label' => 'Dashboard', 'key' => 'dashboard'], ['url' => '/reservation', 'icon' => 'fa-plus', 'label' => 'New Reservation', 'key' => 'reservation'], ['url' => '/reservation-list', 'icon' => 'fa-calendar', 'label' => 'My Reservations', 'key' => 'reservation-list'], ['url' => '/books', 'icon' => 'fa-book-open', 'label' => 'Library', 'key' => 'books'], ['url' => '/profile', 'icon' => 'fa-regular fa-user', 'label' => 'Profile', 'key' => 'profile']];
    $page = 'books';
    $booksJson = json_encode(array_map(fn($b) => ['id' => (int)($b['id'] ?? 0), 'title' => $b['title'] ?? '', 'author' => $b['author'] ?? 'Unknown', 'genre' => $b['genre'] ?? '', 'preface' => $b['preface'] ?? '', 'published_year' => $b['published_year'] ?? '', 'cover_image' => $b['cover_image'] ?? '', 'isbn' => $b['isbn'] ?? '', 'call_number' => $b['call_number'] ?? '', 'available_copies' => (int)($b['available_copies'] ?? 0), 'total_copies' => (int)($b['total_copies'] ?? 1)], $books ?? [])); ?>
    <!-- Sidebar -->
        ...
        <div class="sidebar-card">
            ...
            <nav class="sidebar-nav space-y-1">
                <?php foreach ($navItems as $item): $active = ($page === $item['key']) ? 'active' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600'; ?>
                    <a href="<?= base_url($item['url']) ?>" class="sidebar-item flex items-center gap-4 px-5 py-3.5 rounded-2xl font-semibold text-sm <?= $active ?>"><i class="fa-solid <?= $item['icon'] ?> w-5 text-center text-lg"></i><?= $item['label'] ?></a>
                <?php endforeach; ?>
            </nav>
    ...
    <nav class="lg:hidden mobile-nav-pill">
        <div class="mobile-scroll-container text-white px-2">
            <?php foreach ($navItems as $item): $cls = ($page === $item['key']) ? 'bg-blue-700 font-semibold' : 'hover:bg-blue-500/30'; ?>
                <a href="<?= base_url($item['url']) ?>" class="flex flex-col items-center justify-center py-2 px-3 min-w-[68px] rounded-xl transition flex-shrink-0 <?= $cls ?>"><i class="fa-solid <?= $item['icon'] ?> text-base"></i><span class="text-[9px] mt-0.5 text-center leading-tight whitespace-nowrap"><?= $item['label'] ?></span></a>
            <?php endforeach; ?>
            <a href="<?= base_url('/logout') ?>" class="flex flex-col items-center justify-center py-2 px-3 min-w-[68px] rounded-xl transition flex-shrink-0 hover:bg-red-500/30 text-red-400"><i class="fa-solid fa-arrow-right-from-bracket text-base"></i><span class="text-[9px] mt-0.5 text-center leading-tight whitespace-nowrap">Logout</span></a>
        </div>
    </nav>
    <!-- Book Detail Modal -->
    <div class="modal-backdrop" id="bookDetailModal" onclick="onDetailBackdrop(event)">
        <div class="detail-card">
            ...
            <div class="detail-cover" id="detailCover"><span class="detail-cover-ph" id="detailCoverPh"></span></div>
            <div class="detail-body">
                <div class="flex items-start justify-between gap-3 mb-4">
                    <div class="flex-1 min-w-0">
                        <p id="detailGenrePill" class="text-[10px] font-black uppercase tracking-widest text-blue-600 mb-1"></p>
                    ...
                    <div class="flex flex-col items-end gap-2 flex-shrink-0"><button onclick="closeDetailModal()" class="w-9 h-9 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition"><i class="fa-solid fa-xmark"></i></button><span id="detailAvailTag" class="tag"></span></div>
                </div>
    ...
    <div class="modal-backdrop" id="borrowModal" onclick="onBorrowBackdrop(event)">
        <div class="modal-card">
            ...
            <div class="flex justify-between items-center mb-5">
                ...
                </div><button onclick="closeBorrowModal()" class="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div class="flex items-center gap-4 p-4 bg-blue-50 rounded-2xl border border-blue-100 mb-5">
                <div class="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-book-open text-blue-600"></i></div>
                <div>
            ...
            <div class="flex gap-3"><button onclick="closeBorrowModal()" class="flex-1 py-3 bg-slate-100 rounded-2xl font-bold text-slate-600 hover:bg-slate-200 transition text-sm">Cancel</button>
                <form id="borrowForm" method="post" action="" class="flex-1"><?= csrf_field() ?><button type="submit" class="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-blue-200">Yes, Borrow</button></form>
            </div>
        </div>
    </div>
    <!-- Main -->
    <main class="flex-1 min-w-0 p-4 lg:p-10 pb-32">
        <header class="flex items-start justify-between mb-8 gap-4 fade-in-up">
            <div>
                ...
                <h2 class="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight leading-tight">Community <span class="text-blue-600">Library</span></h2>
                <p class="text-slate-400 font-medium text-sm mt-1">Browse, search, and borrow books available to all residents</p>
            </div>
        </header>
        <?php if (session()->getFlashdata('success')): ?><div class="mb-6 px-5 py-4 bg-blue-50 border border-blue-200 text-blue-700 font-bold rounded-2xl flex items-center gap-3 text-sm"><i class="fa-solid fa-circle-check text-blue-500"></i><?= session()->getFlashdata('success') ?></div><?php endif; ?>
        ...
        <div class="dash-card p-5 mb-6">
            <div class="flex items-center gap-2 mb-3">
                <div class="w-7 h-7 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center"><i class="fa-solid fa-bolt text-xs"></i></div><span class="text-xs font-black uppercase tracking-widest text-slate-400">AI Smart Suggestion</span>
            </div>
            <div class="rag-input-row"><input id="ragQuery" type="text" class="flex-1 min-w-0 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition" placeholder="e.g. adventure for kids, Philippine history…" onkeydown="if(event.key==='Enter')doRag()"><button id="ragBtn" onclick="doRag()" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-sm transition shadow-sm shadow-blue-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap flex-shrink-0"><i class="fa-solid fa-magnifying-glass text-xs"></i> Find for Me</button></div>
            ...
            <div id="ragRes" class="hidden mt-4">
                <div class="p-4 bg-blue-50 border border-blue-100 rounded-2xl mb-3">
                    <p class="text-[10px] font-black uppercase tracking-widest text-blue-600 mb-1.5 flex items-center gap-1"><i class="fa-solid fa-star text-[9px]"></i> Librarian Suggestion</p>
        ...
        <div class="flex gap-3 items-center flex-wrap mb-6">
            <div class="relative flex-1 min-w-[160px]"><i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none"></i><input id="searchInput" type="text" placeholder="Search title or author…" class="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition"></div>
            <select id="genreFilter" class="flex-shrink-0 px-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium text-slate-700 outline-none focus:border-blue-400 cursor-pointer w-full sm:w-auto">
            ...
            <div class="flex rounded-2xl border border-slate-200 overflow-hidden bg-white w-full sm:w-auto">
                ...
                <button id="tabMine" onclick="switchTab('mine')" class="flex-1 sm:flex-none px-4 py-2.5 text-sm font-bold transition"><i class="fa-solid fa-clock-rotate-left text-xs mr-1.5"></i>My Borrowings</button>
            </div>
        </div>
        <!-- Browse Tab -->
        <div id="paneBrowse">
            ...
            <?php else: ?>
                <div class="books-grid grid gap-4" id="booksGrid" style="grid-template-columns:repeat(auto-fill,minmax(200px,1fr))">
                    <?php foreach ($books as $book): $available = (int)($book['available_copies'] ?? 0) > 0; ?>
                        <div class="book-card" id="book-<?= (int)$book['id'] ?>" data-title="<?= strtolower(htmlspecialchars($book['title'], ENT_QUOTES, 'UTF-8')) ?>" data-author="<?= strtolower(htmlspecialchars($book['author'] ?? '', ENT_QUOTES, 'UTF-8')) ?>" data-genre="<?= htmlspecialchars($book['genre'] ?? '', ENT_QUOTES, 'UTF-8') ?>" onclick="openBookDetail(<?= (int)$book['id'] ?>)">
                            <div class="book-cover">
                                <?php if (!empty($book['cover_image'])): ?><img src="<?= esc($book['cover_image']) ?>" alt="<?= esc($book['title']) ?>"><?php else: ?><span class="cover-ph"><?= mb_strtoupper(mb_substr($book['title'], 0, 1)) ?></span><?php endif; ?>
                                <div class="cover-overlay"><span class="cover-overlay-btn"><i class="fa-solid fa-eye text-[10px]"></i> View Details</span></div>
                                <?php if (!empty($book['genre'])): ?><span class="cover-genre-badge px-2 py-0.5 bg-white/90 backdrop-blur text-blue-700 text-[10px] font-black uppercase tracking-wider rounded-full border border-blue-100"><?= esc($book['genre']) ?></span><?php endif; ?>
                            ...
                            <div class="p-3 flex flex-col flex-1">
                                ...
                                <?php if (!empty($book['preface'])): ?><p class="text-xs text-slate-400 italic leading-relaxed mb-2 hidden sm:block" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden"><?= esc($book['preface']) ?></p><?php endif; ?>
                                <div class="flex gap-1.5 flex-wrap mt-auto mb-1.5">
                                    ...
                                    <?php if (!empty($book['published_year'])): ?><span class="px-2 py-0.5 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-500 hidden sm:inline-flex"><?= esc($book['published_year']) ?></span><?php endif; ?>
                                    <?php if (!empty($book['call_number'])): ?><span class="px-2 py-0.5 bg-violet-50 rounded-lg text-[10px] font-bold text-violet-600 font-mono hidden sm:inline-flex"><?= esc($book['call_number']) ?></span><?php endif; ?>
                                </div>
            ...
            <?php else: ?>
                <div class="hidden md:block dash-card overflow-hidden">
                    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                        <div class="w-8 h-8 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center"><i class="fa-solid fa-clock-rotate-left text-xs"></i></div>
                ...
                <div class="md:hidden space-y-3">
                    <div class="flex items-center gap-2.5 mb-1">
                        <div class="w-7 h-7 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-clock-rotate-left text-xs"></i></div>
                    ...
                    <?php foreach ($myBorrowings as $i => $b): $s = strtolower($b['status'] ?? 'pending'); ?>
                        <div class="my-borrow-card">
                            <div class="my-borrow-top">
                                <div class="w-9 h-9 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0"><i class="fa-solid fa-book-open text-blue-500 text-sm"></i></div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2">
                                        <p class="font-black text-sm text-slate-900 leading-tight"><?= esc($b['title']) ?></p><span class="tag tag-<?= $s ?> flex-shrink-0 mt-0.5"><?= ucfirst($s) ?></span>
                                    </div>
                                    <p class="text-xs text-slate-400 font-medium mt-0.5 truncate"><?= esc($b['author'] ?? '') ?></p>
                                </div>
                            </div>
                            <div class="my-borrow-dates"><span class="flex items-center gap-1"><i class="fa-regular fa-calendar text-[10px] text-slate-300"></i>Borrowed: <?= esc($b['borrowed_at'] ?? '—') ?></span><span class="flex items-center gap-1 <?= $s === 'approved' ? 'text-rose-500 font-bold' : '' ?>"><i class="fa-regular fa-calendar-xmark text-[10px]"></i>Due: <?= esc($b['due_date'] ?? '—') ?></span></div>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
    ...
    <script>
        ...
        const ACT = 'bg-blue-600 text-white shadow-sm shadow-blue-200',
            INQ = 'text-slate-500 hover:text-slate-700 hover:bg-slate-50';
        ...
        async function doRag() {
            ...
            const skel = document.getElementById('ragSkel'),
                ...
                res = document.getElementById('ragRes'),
            ...
            try {
                ...
                (d.books || []).forEach(b => {
                    ...
                    chip.className = 'px-3 py-1.5 rounded-xl border font-bold text-xs transition ' + (avail ? 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100' : 'bg-slate-50 border-slate-200 text-slate-400 border-dashed');
