...
<head>
    ...
    <style>
        ...
        .reservation-row[data-status="unclaimed"] td { opacity:.75; }
        ...
        .res-card[data-status="unclaimed"] { opacity:.8; }
        ...
        .unclaimed-notice { background:#fff7ed; border:1.5px dashed #fdba74; border-radius:16px; padding:1.25rem; display:flex; align-items:flex-start; gap:12px; }
...
// ★ Pre-process statuses once so PHP and the banner share the same logic
$processed = [];
foreach (($reservations ?? []) as $res) {
    ...
    if ($status === 'approved') {
        ...
        if ($edt < time()) $status = 'unclaimed';  // approved + time passed = no-show
    } elseif ($status === 'pending') {
        $rdt = strtotime($res['reservation_date'] ?? '');
        if ($rdt && $rdt < strtotime('today')) $status = 'expired'; // pending + date past = never approved
    }
    $res['_status'] = $status;
    $processed[] = $res;
}
...
$statusIcons = [
    ...
    'declined'  => 'fa-xmark',
    'cancelled' => 'fa-ban',
    'expired'   => 'fa-regular fa-hourglass',
    'unclaimed' => 'fa-ticket',
];
?>
    ...
    <div class="modal-card">
        ...
        <div id="unclaimedNotice" class="hidden unclaimed-notice mb-4">
            ...
            <div>
                ...
                <p class="text-xs text-orange-500 mt-0.5">Your reservation was approved but the e-ticket was never scanned. The time slot has passed.</p>
...
<main class="flex-1 min-w-0 p-4 lg:p-12 pb-32">
    ...
    <!-- ★ NEW: Unclaimed warning banner -->
    <?php if ($unclaimedCount > 0): ?>
        <div class="mb-6 px-5 py-4 bg-orange-50 border border-orange-200 rounded-2xl flex items-start gap-3">
            <div class="w-9 h-9 bg-orange-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                <i class="fa-solid fa-ticket text-orange-500 text-sm"></i>
            </div>
            <div>
                <p class="font-bold text-orange-700 text-sm">
                    You have <?= $unclaimedCount ?> unclaimed reservation<?= $unclaimedCount > 1 ? 's' : '' ?>
                </p>
                <p class="text-xs text-orange-500 mt-0.5">
                    <?= $unclaimedCount > 1 ? 'These were' : 'This was' ?> approved but the e-ticket was never scanned at the venue.
                    Repeated no-shows may affect your future booking priority.
                </p>
            </div>
        </div>
    <?php endif; ?>
                            ...
                            <tr class="reservation-row"
                                ...
                                <td>
                                    <span class="status-badge status-<?= $status ?>">
                                        <i class="fa-solid <?= $statusIcons[$status] ?? 'fa-circle' ?> text-[9px]"></i>
...
function viewDetails(id) {
    // ★ Use pre-processed _status from PHP so JS matches PHP exactly
    ...
    if (isClaimed && res.claimed_at) {
        const fc = new Date(res.claimed_at).toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric', hour:'2-digit', minute:'2-digit' });
        html += `<div class="detail-row"><span class="detail-label">Claimed On</span><span class="detail-value">${fc}</span></div>`;
    }
    document.getElementById('modalBody').innerHTML = html;

    // Hide all notices
    ['pendingNotice','rejectedNotice','qrSection','claimedNotice','expiredNotice','unclaimedNotice'].forEach(i => {
    ...
    if (isPending) {
        ...
        document.getElementById('rejectedText').textContent = status === 'declined'
            ? 'This reservation was declined by an SK officer.'
