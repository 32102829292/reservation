...
<head>
    ...
    <style>
        ...
        .btn-primary:hover {
            ...
            box-shadow: 0 12px 20px -5px rgba(37, 99, 235, .35);
        }
...
<body class="flex">
    <?php $navItems = [['url' => '/dashboard', 'icon' => 'fa-house', 'label' => 'Dashboard', 'key' => 'dashboard'], ['url' => '/reservation', 'icon' => 'fa-plus', 'label' => 'New Reservation', 'key' => 'reservation'], ['url' => '/reservation-list', 'icon' => 'fa-calendar', 'label' => 'My Reservations', 'key' => 'reservation-list'], ['url' => '/profile', 'icon' => 'fa-regular fa-user', 'label' => 'Profile', 'key' => 'profile']]; ?>
    <aside class="hidden lg:flex flex-col w-80 p-6">
        <div class="sidebar-card">
            ...
            <nav class="sidebar-nav space-y-1">
                <?php foreach ($navItems as $item): $active = (isset($page) && $page == $item['key']) ? 'active' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600'; ?>
            ...
            <div class="sidebar-footer">
                <a href="<?= base_url('/logout') ?>" class="flex items-center gap-4 px-5 py-4 rounded-2xl text-red-500 font-bold hover:bg-red-50 transition-all"><i class="fa-solid fa-arrow-right-from-bracket w-5 text-center"></i> Logout</a>
            </div>
        </div>
    </aside>
    <nav class="lg:hidden mobile-nav-pill">
        <div class="mobile-scroll-container text-white px-2">
            <?php foreach ($navItems as $item): $cls = (isset($page) && $page == $item['key']) ? 'bg-blue-700 font-semibold' : 'hover:bg-blue-500/30'; ?>
            ...
            <a href="<?= base_url('/logout') ?>" class="flex flex-col items-center justify-center py-2 px-3 min-w-[75px] rounded-xl transition flex-shrink-0 hover:bg-red-500/30 text-red-400">
                <i class="fa-solid fa-arrow-right-from-bracket text-lg"></i><span class="text-[10px] mt-1 text-center leading-tight whitespace-nowrap">Logout</span>
            </a>
        </div>
    </nav>
    <main class="flex-1 p-6 lg:p-12 pb-32 flex items-start justify-center">
        <div class="success-card">
            <div class="pending-icon"><i class="fa-regular fa-hourglass-half text-3xl text-yellow-500"></i></div>
            ...
            <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 mb-6">
                <div class="flex items-center gap-2 mb-4">
                    <div class="w-7 h-7 bg-amber-100 rounded-lg flex items-center justify-center"><i class="fa-solid fa-receipt text-amber-600 text-xs"></i></div>
                ...
                <div class="detail-row"><span class="detail-label">Reservation ID</span><span class="detail-value font-mono text-slate-500">#<?= $reservation['id'] ?></span></div>
                ...
                <?php if (!empty($reservation['pc_number'])): ?><div class="detail-row"><span class="detail-label">Workstation</span><span class="detail-value"><?= esc($reservation['pc_number']) ?></span></div><?php endif; ?>
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-100 mb-6">
                <div class="flex items-center gap-2 mb-4">
                    <div class="w-7 h-7 bg-blue-50 rounded-lg flex items-center justify-center"><i class="fa-solid fa-list-check text-blue-600 text-xs"></i></div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
                ...
                <a href="<?= base_url('/dashboard') ?>" class="btn-primary"><i class="fa-solid fa-house"></i> Go to Dashboard</a>
            </div>
            <div class="flex justify-center gap-5 text-xs text-slate-400 pt-2 border-t border-slate-100">
                <a href="<?= base_url('/reservation') ?>" class="hover:text-blue-600 transition flex items-center gap-1 font-semibold"><i class="fa-solid fa-plus"></i> New Reservation</a>
                <span class="text-slate-200">|</span>
                <a href="<?= base_url('/reservation-list') ?>" class="hover:text-blue-600 transition flex items-center gap-1 font-semibold"><i class="fa-regular fa-clock"></i> Check Status</a>
            </div>
        </div>
    </main>
</body>
