...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    /* ── Wrapper — flex column so back btn stays left ── */
    .login-wrapper {
      width: 100%;
      max-width: 440px;
      position: relative;
...
<body>
      ...
      <div class="modal-body">
        ...
        <div class="fp-panel active" id="fp1">
          ...
          <div style="margin-bottom:1rem">
            ...
            <div class="fp-err" id="fpEmailErr">
              <i class="fa-solid fa-circle-exclamation" style="margin-right:4px"></i>
              <span id="fpEmailMsg">Please enter a valid email.</span>
            </div>
          </div>
          <div class="fp-info">
            <i class="fa-solid fa-circle-info"></i>
        ...
        <div class="fp-panel" id="fp2">
          ...
          <div class="fp-err" id="fpOtpErr" style="text-align:center">
            <i class="fa-solid fa-circle-exclamation" style="margin-right:4px"></i>
            <span id="fpOtpMsg">Invalid or expired code.</span>
          </div>
          <div style="text-align:center">
            <div class="timer-pill" id="timerPill">
              <i class="fa-regular fa-clock"></i> Expires in <span id="fpTimer">10:00</span>
            </div>
          </div>
          <div class="resend-row">
            Didn't receive it?
            <button class="resend-btn" id="fpResendBtn" onclick="fpResend()" disabled>Resend Code</button>
          </div>
        </div>
        ...
        <div class="fp-panel" id="fp3">
          ...
          <div style="margin-bottom:1rem">
            ...
            <div style="position:relative">
              ...
              <button type="button" class="eye-btn" onclick="fpToggle('fpPwd','fpEye1')">
                <i class="fa-regular fa-eye" id="fpEye1"></i>
              </button>
            </div>
            <div class="str-row">
          ...
          <div style="margin-bottom:1rem">
            ...
            <div style="position:relative">
              ...
              <button type="button" class="eye-btn" onclick="fpToggle('fpConf','fpEye2')">
                <i class="fa-regular fa-eye" id="fpEye2"></i>
              </button>
            </div>
            <div class="fp-err" id="fpConfErr">
              <i class="fa-solid fa-circle-exclamation" style="margin-right:4px"></i>
    ...
    <div class="modal-box">
      ...
      <div class="modal-body" id="termsBody">
        ...
        <div class="ts">
          ...
          <p>The administration reserves the right to suspend or terminate accounts that violate these terms, including repeated no-shows, misuse of equipment, or providing false information.</p>
        </div>
        <div class="ts">
          <div class="ts-head">
  ...
  <script>
    ...
    document.getElementById('loginForm').addEventListener('submit', function(e) {
      ...
      btn.disabled = true;
      btn.innerHTML = '<div class="spin"></div> Signing in&hellip;';
    });
    ...
    async function fpS1() {
      ...
      fpLoad(true, 'Sending Code&hellip;');
      try {
    ...
    async function fpS3() {
      ...
      if (pwd.length < 8) {
        fpErr('fpPwdErr', true);
        ok = false;
      } else fpErr('fpPwdErr', false);
      if (pwd !== conf || !conf) {
        fpErr('fpConfErr', true);
        ok = false;
      } else fpErr('fpConfErr', false);
      if (!ok) return;
      fpLoad(true, 'Updating Password&hellip;');
      try {
        const r = await fetch('/forgot-password/reset-password', {
          method: 'POST',
          headers: _hdrs(),
          body: JSON.stringify({
            reset_token: fpToken,
    ...
    function fpStartTimer() {
      ...
      fpTi = setInterval(() => {
        ...
        if (fpSec <= 0) {
          ...
          if (p) {
            p.style.background = 'rgba(239,68,68,.1)';
            p.style.borderColor = 'rgba(239,68,68,.3)';
    ...
    function fpStrength() {
      ...
      const lc = ['', '#fca5a5', '#fdba74', '#fde047', '#86efac'];
      [1, 2, 3, 4].forEach(j => {
        document.getElementById('ss' + j).style.background = j <= sc ? c[sc] : 'var(--border)';
      });
      const lb = document.getElementById('fpStrLbl');
      lb.textContent = p ? l[sc] : '';
      lb.style.color = p ? lc[sc] : 'var(--text-dimmer)';
    }

    function fpToggle(id, ic) {
      const i = document.getElementById(id);
      const ico = document.getElementById(ic);
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.getElementById('fpPwd')?.addEventListener('keydown', e => {
        if (e.key === 'Enter') document.getElementById('fpConf').focus();
      });
      document.getElementById('fpConf')?.addEventListener('keydown', e => {
        if (e.key === 'Enter') fpNext();
      });
    });
