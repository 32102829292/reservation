...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .flash-warning { background: rgba(245,158,11,.1);         border: 1px solid rgba(245,158,11,.25); color: #fcd34d; }
    ...
    .modal-box { background: var(--navy-card); border-radius: var(--r); border: 1px solid var(--border); box-shadow: 0 32px 80px rgba(0,0,0,.6), 0 0 0 1px rgba(99,132,255,.05); width: 100%; max-width: 560px; max-height: 90vh; display: flex; flex-direction: column; animation: modalUp .25s cubic-bezier(.34,1.4,.64,1); overflow: hidden; }
...
function fpStartTimer() {
  ...
  fpTi = setInterval(() => {
    ...
    if (fpSec <= 0) {
      ...
      if (p) { p.style.background = 'rgba(239,68,68,.1)'; p.style.borderColor = 'rgba(239,68,68,.3)'; p.style.color = '#fca5a5'; }
    }
  }, 1000);
  document.getElementById('fpResendBtn').disabled = true;
  clearTimeout(fpRt);
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { closeFP(); ['terms', 'privacy'].forEach(closeModalT); }
});
